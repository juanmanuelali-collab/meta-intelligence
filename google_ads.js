...
async function fetchCampaignMetricsRange(customerId, since, until) {
  const accessToken = await getGoogleAdsAccessToken();
  try {
    const rows = await runGaqlSearch(customerId, accessToken, buildCampaignRangeQuery(since, until, true));
    console.log(`[google-ads][diag] fetchCampaignMetricsRange OK con impression share — ${rows.length} filas para ${customerId} (${since} a ${until})`);
    return rows;
  } catch (e) {
    console.warn(`[google-ads] impression share no disponible para ${customerId} (rango), reintentando sin esos campos:`, e.message);
    const rows = await runGaqlSearch(customerId, accessToken, buildCampaignRangeQuery(since, until, false));
    console.log(`[google-ads][diag] fetchCampaignMetricsRange OK SIN impression share (fallback) — ${rows.length} filas para ${customerId} (${since} a ${until})`);
    return rows;
  }
}
...
// ── Palabras clave (Search): rendimiento por keyword para un rango de fechas ──
// Se usa en el Informe (Docta Nexus Reporte) del perfil "leads" — dato en vivo,
// no se guarda histórico en Supabase, mismo criterio que fetchShoppingProducts.
// Solo aplica a campañas de Search (keyword_view no trae filas de PMax/Display).
async function fetchTopKeywords(customerId, since, until, limit = 15) {
  const accessToken = await getGoogleAdsAccessToken();
  const query = `
    SELECT
  ...
  // keyword_view devuelve una fila por keyword+campaña+día en algunos casos —
  // se agrega acá por texto+match_type (misma keyword en distintas campañas
  // se suma, ya que al operador le interesa el término, no dónde vive).
  const MATCH_TYPE_ES = { EXACT: 'Exacta', PHRASE: 'Frase', BROAD: 'Amplia' };
  const byKeyword = {};
  for (const r of results) {
    const kw = r.adGroupCriterion?.keyword || r.ad_group_criterion?.keyword || {};
    const m = r.metrics || {};
    const text = kw.text || '(sin texto)';
    const matchType = kw.matchType || kw.match_type || 'UNSPECIFIED';
    const key = `${text}::${matchType}`;
    if (!byKeyword[key]) {
      byKeyword[key] = {
        text, match_type: MATCH_TYPE_ES[matchType] || matchType,
        impressions: 0, clicks: 0, cost: 0, conversions: 0, conversion_value: 0,
      };
    }
    const k = byKeyword[key];
    k.impressions += Number(m.impressions || 0);
    k.clicks += Number(m.clicks || 0);
    k.cost += Number(m.costMicros || m.cost_micros || 0) / 1_000_000;
    k.conversions += Number(m.conversions || 0);
    k.conversion_value += Number(m.conversionsValue || m.conversions_value || 0);
  }

  return Object.values(byKeyword)
    .map(k => ({
      ...k,
      cost: +k.cost.toFixed(0),
      conversions: +k.conversions.toFixed(1),
      ctr: k.impressions > 0 ? +(k.clicks / k.impressions * 100).toFixed(2) : null,
      cpc: k.clicks > 0 ? +(k.cost / k.clicks).toFixed(0) : null,
      cpa: k.conversions > 0 ? +(k.cost / k.conversions).toFixed(0) : null,
      conversion_rate: k.clicks > 0 ? +(k.conversions / k.clicks * 100).toFixed(2) : null,
    }))
    .sort((a, b) => b.clicks - a.clicks)
    .slice(0, limit);
}

// ── Objetivos (acciones de conversión): cantidad y valor por objetivo
// configurado en la cuenta — ej. "Compra", "Formulario de contacto",
// "Llamada", cada uno con su propio conteo y valor, para un rango de fechas.
// Se usa en el Informe Frío. Dato en vivo, no se guarda histórico.
async function fetchConversionActionsSummary(customerId, since, until) {
  const accessToken = await getGoogleAdsAccessToken();
  const query = `
    SELECT
  ...
  const CATEGORY_ES = {
    PURCHASE: 'Compra', LEAD: 'Lead', SIGNUP: 'Registro', SUBMIT_LEAD_FORM: 'Formulario',
    PHONE_CALL_LEAD: 'Llamada', BOOK_APPOINTMENT: 'Turno/Cita', REQUEST_QUOTE: 'Cotización',
    GET_DIRECTIONS: 'Cómo llegar', OUTBOUND_CLICK: 'Clic saliente', CONTACT: 'Contacto',
    ADD_TO_CART: 'Agregado al carrito', BEGIN_CHECKOUT: 'Checkout iniciado', PAGE_VIEW: 'Vista de página',
  };
  const byAction = {};
  for (const r of results) {
    const s = r.segments || {};
    const m = r.metrics || {};
    const name = s.conversionActionName || s.conversion_action_name || '(sin nombre)';
    const category = s.conversionActionCategory || s.conversion_action_category || null;
    const key = name;
    if (!byAction[key]) {
      byAction[key] = { name, category: CATEGORY_ES[category] || category || null, conversions: 0, value: 0 };
    }
    byAction[key].conversions += Number(m.conversions || 0);
    byAction[key].value += Number(m.conversionsValue || m.conversions_value || 0);
  }

  return Object.values(byAction)
    .filter(a => a.conversions > 0)
    .map(a => ({ ...a, conversions: +a.conversions.toFixed(1), value: +a.value.toFixed(0) }))
    .sort((a, b) => b.conversions - a.conversions);
}

// ── Estructura de campañas (sin depender de gasto/fecha) — a diferencia de
// fetchCampaignMetricsRange, esto consulta el recurso "campaign" directo,
// sin joinear métricas ni filtrar por rango de fechas. Trae TODAS las
// campañas que existen en la cuenta (activas o pausadas, con o sin gasto
// todavía) — se usa para el matching del Plan de Campañas, donde una
// campaña recién creada sin historial no debe quedar invisible.
async function fetchAllCampaignsStructure(customerId) {
  const accessToken = await getGoogleAdsAccessToken();
  const query = `
...
// Moneda real de la cuenta — se usa para mostrar cada canal en su propia
// moneda en el Plan de Campañas (nunca se asume ARS/USD, se trae real).
async function fetchAccountCurrency(customerId) {
  const accessToken = await getGoogleAdsAccessToken();
  const query = `SELECT customer.currency_code FROM customer LIMIT 1`;
  const results = await runGaqlSearch(customerId, accessToken, query);
  return results?.[0]?.customer?.currencyCode || results?.[0]?.customer?.currency_code || null;
}

// ── Conversiones por día de la semana — Google Ads soporta combinar
// segments.day_of_week directo en la consulta, a diferencia de Meta que
// necesita cruzarse a mano. Trae conversiones totales (todas las acciones
// de conversión configuradas) agrupadas por día — el llamador decide si
// mostrarlas como "Compras" o algo más específico según report_profile.
async function fetchConversionsByWeekday(customerId, since, until) {
  const accessToken = await getGoogleAdsAccessToken();
  const query = `
    SELECT segments.day_of_week, metrics.conversions, metrics.all_conversions
    FROM customer
    WHERE segments.date BETWEEN '${since}' AND '${until}'
  `;
  const rows = await runGaqlSearch(customerId, accessToken, query);
  const order = ['MONDAY','TUESDAY','WEDNESDAY','THURSDAY','FRIDAY','SATURDAY','SUNDAY'];
  const totals = {};
  order.forEach(d => totals[d] = { conversions: 0, all_conversions: 0 });
  rows.forEach(r => {
    const day = r.segments?.dayOfWeek || r.segments?.day_of_week;
    if (day && totals[day] !== undefined) {
      totals[day].conversions += Number(r.metrics?.conversions || 0);
      totals[day].all_conversions += Number(r.metrics?.allConversions || r.metrics?.all_conversions || 0);
    }
  });
  ...
  return order.map(d => ({ label: LABELS[d], conversions: +totals[d].conversions.toFixed(1), all_conversions: +totals[d].all_conversions.toFixed(1) }));
}

module.exports = {
  getGoogleAdsAccessToken, fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts,
  fetchCampaignMetricsRange, aggregateCampaignRows, fetchTopKeywords, fetchConversionActionsSummary,
  fetchAllCampaignsStructure, fetchAccountCurrency, fetchConversionsByWeekday,
};
