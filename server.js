...
async function aggregateChannelPeriod(slug, source, start, end) {
  try {
    if (source === 'meta') {
      const creds = await getMetaCredentials(slug);
      if (!creds || !creds.access_token || !creds.ad_account_id) return { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null };
    ...
    if (source === 'google_ads') {
      const customerId = await getGoogleAdsAccountId(slug);
      if (!customerId) return { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null };
    ...
    return { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null };
  } catch (e) {
...
async function fetchReportFunnel(slug, profile, start, end) {
  if (profile !== 'ecommerce') return null;
  try {
    const propertyId = await getGA4PropertyId(slug);
...
async function computeReportFacts(client, clientId, slug, profile, periodType) {
  ...
  const [metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail] = await Promise.all([
    ...
    profile === 'ecommerce' ? fetchEcommerce12MonthEvolution(clientId) : Promise.resolve([]),
  ...
  if (profile === 'leads' && targets.costo_lead_max != null) {
    const combSpend = (metaCur.spend || 0) + (googleCur.spend || 0);
  ...
  return { range, facts, metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail };
}
...
async function runReportAgent(client, profile, factsData, contextNote) {
  const { facts, range, metaCur, metaPrev, googleCur, googlePrev, funnel, ecomCur, ecomPrev } = factsData;
  const negocio = profile === 'ecommerce' ? 'un eCommerce' : 'una empresa de generación de leads (servicios)';
  ...
  if (!facts.length) {
    return {
      ...
      funnel_narrative: '', conversion_insight: '',
      actions: [
  ...
  const funnelText = (profile === 'ecommerce' && funnel && funnel.length)
  ...
  const convText = (profile === 'ecommerce' && ecomCur && ecomCur.tasa_conversion_ecommerce != null)
...
Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","channel_narratives":{"meta":"...","google":"..."},"stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]${funnelText ? ',"funnel_narrative":"..."' : ''}${convText ? ',"conversion_insight":"..."' : ''}}`;
...
app.post('/api/:slug/report/generate', async (req, res) => {
  ...
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['ecommerce', 'leads'].includes(profile)) return res.status(400).json({ error: 'Perfil inválido' });
...
// ── Endpoint público: metadata del informe (para la pantalla de código de acceso, sin exponer datos) ──
