...
async function computeReportFacts(client, clientId, slug, periodType) {
  ...
  const [
    ...
    fetchAnalyticsDetail(slug, true, range.start, range.end),
  ]);
  ...
  return { range, facts, metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail, reachData, messagesData };
}
...
async function runReportAgent(client, factsData, contextNote, panoramaTheme) {
  const { facts, range, metaCur, metaPrev, googleCur, googlePrev, funnel, ecomCur, ecomPrev, reachData, messagesData } = factsData;
...
Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","channel_narratives":{"meta":"...","google":"..."},"stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]${funnelText ? ',"funnel_narrative":"..."' : ''}${convText ? ',"conversion_insight":"..."' : ''}${reachText ? ',"reach_narrative":"..."' : ''}${messagesText ? ',"messages_narrative":"..."' : ''}}`;
...
async function generateClientReport(slug, periodType, options = {}) {
  ...
  const reportData = {
    profile: 'ecommerce', period_type: periodType, period_label: factsData.range.label,
  ...
  const { data, error } = await supabase.from('client_reports').insert({
    client_id: clientDB.id, token, access_code: accessCode, profile: 'ecommerce',
