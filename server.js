...
function calcKpis(data) {
  ...
  // Meta reporta la donación repartida en hasta 5 canales según cómo
  // convirtió el usuario. donate_total es el agregado oficial de Meta
  // (cuando viene), pero si no viene se arma sumando los 4 canales —
  // son mutuamente excluyentes, así que sumarlos no duplica:
  //  - donate_website     → formulario de donación en el sitio (pixel)
  //  - donate_on_facebook → on-Facebook (fundraiser / botón nativo)
  //  - donate_mobile_app  → donación hecha desde app mobile
  //  - donate_offline     → conversión offline cargada manualmente
  //  - onsite_conversion.donate → variante legacy del botón nativo, como último fallback
  ...
  const donations      = donateTotalRaw || donateChannelSum || onsiteDonate || getAction(actions, 'donate');
  ...
  const donationValue  = getActionValue(actionValues, 'donate_total') ||
