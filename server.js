...
function calcKpis(data) {
  ...
  // ── DONACIONES CONSOLIDADAS (pixel + onsite + agrupado "donate_total") ──
  // Meta separa Compras y Donaciones como eventos distintos desde siempre
  // (onsite_conversion.donate / donate_total); antes solo se leía la
  // variante onsite. Se agrega acá sin tocar el cálculo de "purchases".
  const pixelDonate  = getAction(actions, 'offsite_conversion.fb_pixel_donate');
  const donations      = pixelDonate || onsiteDonate ||
                        getAction(actions, 'donate_total') || getAction(actions, 'donate');
