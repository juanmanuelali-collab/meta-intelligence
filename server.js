...
async function runAnalysis(client, metricsData) {
  ...
  const thresholds = client.alert_thresholds || {};
...
- Fases del algoritmo: Aprendizaje (<50 conv/semana), Estabilización, Fatiga
- Advantage+: cómo Meta optimiza automáticamente audiencias, placements y creatividades
- Señales de calidad: relevance score, engagement rate, CTR post-click
- Subasta: cómo el bid, el presupuesto y la calidad del anuncio determinan el delivery
- Fatiga creativa: señales tempranas y tardías, cómo impacta el ROAS y el CPM

## Métricas clave y sus relaciones
- CPM alto + CTR bajo = problema de creatividad o audiencia saturada
- Frecuencia >3.5 = señal de fatiga, especialmente en remarketing
- CPA creciente + impresiones estables = fatiga o competencia aumentó
- ROAS bajo en fase de aprendizaje es normal, no pausar antes de 50 conv
- CTR del 1-3% es saludable para feed; stories/reels >1.5%

## Propuestas accionables
Siempre terminás con 3-5 acciones concretas priorizadas por impacto potencial.
Las acciones deben ser específicas: "pausar campaña X", "aumentar presupuesto de conjunto Y un 20%", "refrescar creatividad del anuncio Z".

Respondé en español, con tono profesional pero directo. Sin rodeos.`;

  const prompt = `Analizá el estado de las campañas de Meta Ads para el cliente "${client.name}".

OBJETIVOS DEL CLIENTE:
- CPA objetivo: ${targets.target_cpa ? '$' + targets.target_cpa : 'no definido'}
- ROAS objetivo: ${targets.target_roas || 'no definido'}
- CTR objetivo: ${targets.target_ctr ? targets.target_ctr + '%' : 'no definido'}
- Objetivo de campaña: ${client.objective || 'CONVERSIONS'}
- Presupuesto diario: ${targets.daily_budget ? '$' + targets.daily_budget : 'no definido'}

MÉTRICAS ACTUALES (últimos 7 días):
${JSON.stringify(metricsData.account, null, 2)}

ESTADO POR CAMPAÑA:
${JSON.stringify(metricsData.campaigns, null, 2)}

TOP ANUNCIOS (por gasto):
${JSON.stringify(metricsData.ads.slice(0, 10), null, 2)}

${history.length > 1 ? 'TENDENCIA (últimos ' + history.length + ' días de datos):' : ''}
${history.length > 1 ? JSON.stringify(history.map(h => ({ date: h.date, account: h.account })), null, 2) : ''}

Generá un análisis completo con:
1. DIAGNÓSTICO GENERAL (2-3 párrafos)
2. ESTADO DEL ALGORITMO (fase detectada y por qué)
3. CAMPAÑAS CRÍTICAS (las que necesitan atención urgente)
4. ANÁLISIS DE CREATIVIDADES (qué está funcionando y qué está en fatiga)
5. ACCIONES RECOMENDADAS (3-5 acciones concretas y priorizadas)

Respondé en JSON con esta estructura:
{
  "summary": "diagnóstico general en 2-3 párrafos",
  "algorithm_phase": "learning|stable|fatigue|mixed",
  "algorithm_note": "explicación de la fase detectada",
  "health_score": 0-100,
  "critical_campaigns": [{"name":"","issue":"","action":""}],
  "creative_insights": "análisis de creatividades",
  "recommendations": [{"priority":1,"action":"","impact":"alto|medio|bajo","detail":""}],
  "alerts": [{"type":"warning|critical","metric":"","current":"","target":"","message":""}]
}`;
  ...
  catch { return { summary: text, health_score: 50, recommendations: [], alerts: [] }; }
}
...
async function sendReport(client, metrics, analysis) {
  if (!process.env.EMAIL_USER || !client.email_to?.length) return;
...
async function sendAlert(client, alerts, metrics) {
  if (!process.env.EMAIL_USER || !client.email_to?.length || !alerts?.length) return;
  const transporter = createTransporter();
...
app.post('/api/:slug/send-report', async (req, res) => {
  ...
  try {
    ...
    res.json({ ok: true, message: 'Reporte enviado a ' + client.email_to.join(', ') });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
