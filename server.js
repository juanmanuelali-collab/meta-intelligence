...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    // Etiquetas legibles del período anterior
    const prevLabel = period === 'yesterday'
      ? 'anteayer'
      : period === '30d'
      ? `${fmt(daysAgo(60))} – ${fmt(daysAgo(31))}`
      : `${fmt(daysAgo(14))} – ${fmt(daysAgo(8))}`;

    // Log para debug
    console.log(`[metrics] ${period} | current: ${data.data?.length} rows, spend: ${data.data?.[0]?.spend} | prev: ${prevData.data?.length} rows, spend: ${prevData.data?.[0]?.spend}`);
