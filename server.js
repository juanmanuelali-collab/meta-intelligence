...
const PLAN_COLUMNS = {
  ...
  'start date': 'start_date', 'end date': 'end_date', 'objective': 'objective',
  'budget diario': 'budget_diario', 'días': 'dias', 'dias': 'dias',
  'budget mensual': 'budget_mensual',
  'status': 'status', 'kpi 1': 'kpi_1', 'kpi 2': 'kpi_2', 'kpi 3': 'kpi_3', 'notas': 'notas',
  // Legacy — Sheets viejos que todavía no migraron a la estructura nueva de arriba
  'budget %': 'budget_pct', 'target audience': 'target_audience', 'creative': 'creative',
...
async function writeCampaignPlanDraftToSheet(sheetId, tabName, rows) {
  ...
  const header = ['Campaign Name', 'Channel', 'Start Date', 'End Date', 'Objective', 'Budget Diario', 'Días', 'Budget Mensual', 'Status', 'KPI 1', 'KPI 2', 'KPI 3', 'Notas'];
  const values = [header, ...rows.map(r => [
    r.campaign_name || '', r.channel || '', r.start_date || '', r.end_date || '', r.objective || '',
    r.budget_diario ?? '', r.dias ?? '', r.budget_mensual ?? '',
    r.status || 'Proposed', r.kpi_1 || '', r.kpi_2 || '', r.kpi_3 || '', r.notas || '',
  ])];
...
async function updateCampaignPlanRowInSheet(sheetId, tabName, rowIndex, fields) {
  ...
  if (!sheets) throw new Error('No se pudo autenticar con Google Sheets — revisar GA4_SERVICE_ACCOUNT/GOOGLE_SERVICE_ACCOUNT.');
  const values = [[
    fields.campaign_name || '', fields.channel || '', fields.start_date || '', fields.end_date || '', fields.objective || '',
    fields.budget_diario ?? '', fields.dias ?? '', fields.budget_mensual ?? '',
    fields.status || '', fields.kpi_1 || '', fields.kpi_2 || '', fields.kpi_3 || '', fields.notas || '',
  ]];
  await sheets.spreadsheets.values.update({
    spreadsheetId: sheetId, range: `'${tabName}'!A${rowIndex}:M${rowIndex}`, valueInputOption: 'USER_ENTERED',
    requestBody: { values },
  });
}
...
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    ...
    const planText = rowsWithActuals.map((r, i) => {
      ...
      if (r.budget_mensual) parts.push(`presupuesto mensual planeado: $${r.budget_mensual}${r.budget_diario ? ` ($${r.budget_diario}/día × ${r.dias || '?'} días)` : ''}`);
    ...
    const prompt = `Sos un estratega de Paid Media de la agencia Docta Nexus. Tenés el plan de campañas actual de "${client.name}" y cómo viene rindiendo cada una este mes (datos reales, no inventes números nuevos). Tu trabajo es proponer el plan de ${nextMonthLabel} — NO es copiar el plan actual tal cual: para cada campaña con datos reales, decidí explícitamente si sube, baja, se pausa o sigue igual el presupuesto mensual según cómo rindió, y reflejá esa decisión en el número que devolvés (ajustá budget_mensual y, si corresponde, budget_diario y dias de forma consistente entre sí). Si falta algo evidente, sugerí una campaña nueva de tipo similar a las que ya existen (sin inventar plataformas o audiencias que no tengan sentido con el resto del plan).
...
${planText}
${liveText}${kpiText}
Respondé SOLO en JSON, un array de campañas propuestas para ${nextMonthLabel}, con este formato exacto (mismas columnas que el Sheet real: Campaign Name, Channel, Start Date, End Date, Objective, Budget Diario, Días, Budget Mensual, Status, KPI 1, KPI 2, KPI 3, Notas — kpi_2/kpi_3 pueden quedar vacíos si la campaña solo tiene un KPI relevante):
[{"campaign_name":"...","channel":"Meta|Google|otro","start_date":"YYYY-MM-DD","end_date":"YYYY-MM-DD","objective":"...","budget_diario":0,"dias":0,"budget_mensual":0,"kpi_1":"...","kpi_2":"","kpi_3":"","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
    ...
    const proposedRows = proposed.map(p => ({
      ...
      end_date: p.end_date || '', objective: p.objective || '',
      budget_diario: p.budget_diario ?? '', dias: p.dias ?? '', budget_mensual: p.budget_mensual ?? '',
      kpi_1: p.kpi_1 || '', kpi_2: p.kpi_2 || '', kpi_3: p.kpi_3 || '', notas: p.notas || '', status: 'Proposed',
    }));
