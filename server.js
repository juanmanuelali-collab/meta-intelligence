...
const { loadClientDB, loadClientDBAny, verifyPassword, checkPassword, getMetaCredentials, updateMetaToken, getGoogleAdsAccountId, getGA4PropertyId, getSearchConsoleProperty, setSearchConsoleProperty, getSheetsId, setSheetsId, createClientRecord, setMetaCredentials, setGoogleAdsAccountId, setGA4PropertyId, updateClientRecord, getClientCredentialsSummary, setCredentialError, getAllCredentialsHealth, getAiUsage, incrementAiUsage, supabase } = require('./db'); // Sub-pasos 3.1/3.2/3.3 — auth + credenciales vía Supabase
...
const DEFAULT_ENABLED_FEATURES = {
  ...
  informe: true, plan_campanias: true, nexus_reporte: true, reporte_frio: true,
...
app.post('/api/admin/clients/:slug/logo', async (req, res) => {
  ...
  const authorized = checkAdminPassword(password) || await checkPassword(slug, password);
...
const KPI_METRIC_DEFS = [
  ...
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_frequency', label: 'Frecuencia', format: 'number2', accessor: a => a.meta?.frequency },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_messages', label: 'Mensajes', format: 'number', accessor: a => a.meta?.messages },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_cpmsg', label: 'CPA de los Mensajes', format: 'money', accessor: a => a.meta?.cpmsg },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_purchases', label: 'Compras', format: 'number', accessor: a => a.meta?.conversions },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_cpa', label: 'Coste Conversión Compras', format: 'money', accessor: a => a.meta?.cpa },
  ...
  { group: 'google', group_label: 'Google Ads', key: 'google_cpa', label: 'CPA', format: 'money', accessor: a => a.google?.cpa },
  { group: 'google', group_label: 'Google Ads', key: 'google_ctr', label: 'CTR', format: 'percent', accessor: a => a.google?.ctr },
  { group: 'google', group_label: 'Google Ads', key: 'google_cpc', label: 'CPC', format: 'money', accessor: a => (a.google?.spend && a.google?.clicks) ? a.google.spend / a.google.clicks : null },
  ...
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_bounce', label: 'Tasa de Rebote', format: 'percent', accessor: a => a.analytics?.bounce_rate },
...
app.get('/api/:slug/kpi-targets', async (req, res) => {
  ...
  try {
    ...
    const [{ data: targetsRows }, { data: labelsRows }, actuals] = await Promise.all([
      ...
      computeMonthlyKpiActuals(slug, clientDB.id, targetPeriod),
    ]);

    const customLabels = {};
    (labelsRows || []).forEach(r => { customLabels[r.slot_key] = r.label; });

    const { start, end } = periodToMonthRange(targetPeriod);
    const customActuals = await computeCustomKpiActuals(slug, start, end, customLabels);
    ...
    const metrics = KPI_METRIC_DEFS
      ...
      actual: def.custom ? (customActuals[def.key] ?? null) : def.accessor(actuals),
      target_global: targetsGlobal[def.key] ?? null,
      target_monthly: targetsMonthly[def.key] ?? null,
    }));

    res.json({ period: targetPeriod, client_type: clientDB.client_type || null, metrics });
  } catch (e) {
...
// Cálculo de "mejoró/empeoró" genérico, reusado por Score y Alerts — nunca
// compara contra un benchmark externo, solo actual vs. período anterior.
function nexusPctChange(curr, prev) {
  if (curr == null || prev == null || prev === 0) return null;
  return +(((curr - prev) / prev) * 100).toFixed(1);
}

// ── Nexus Score — auto-referencial, 0-100 por área + general. Cada área
// suma 1 si el indicador clave mejoró vs. el período anterior, 0 si
// empeoró o no hay dato — nunca compara contra un número inventado.
function computeNexusScore(data) {
  const areas = {};
  const gp = data.google?.previous, mp = data.meta?.previous;
  areas.paid_media = [
    nexusPctChange(data.meta?.spend > 0 ? (data.meta.revenue / data.meta.spend) : null, mp && mp.spend > 0 ? (mp.revenue / mp.spend) : null),
    nexusPctChange(data.google?.conversions, gp?.conversions),
  ];
  areas.acquisition = [
    nexusPctChange(data.analytics?.top_site?.current?.total_users, data.analytics?.top_site?.previous?.total_users),
    nexusPctChange(data.analytics?.top_site?.current?.views, data.analytics?.top_site?.previous?.views),
  ];
  areas.seo = [nexusPctChange(data.search_console?.clicks, data.search_console?.previous?.clicks)];
  areas.ecommerce = [nexusPctChange(data.ecommerce?.facturacion, null)]; // sin histórico directo disponible hoy — queda N/D si no hay previo

  const scoreArea = vals => {
    const valid = vals.filter(v => v != null);
    if (!valid.length) return null;
    const improved = valid.filter(v => v > 0).length;
    return Math.round((improved / valid.length) * 100);
  };
  const scores = {};
  let sum = 0, count = 0;
  for (const [area, vals] of Object.entries(areas)) {
    const s = scoreArea(vals);
    scores[area] = s;
    if (s != null) { sum += s; count++; }
  }
  scores.general = count ? Math.round(sum / count) : null;
  return scores;
}

// ── Nexus Alerts — automático, sin botón. Marca cambios grandes (>30%)
// en los indicadores ya calculados, sin inventar causas.
function computeNexusAlerts(data) {
  const alerts = [];
  const check = (label, curr, prev, area) => {
    const chg = nexusPctChange(curr, prev);
    if (chg != null && Math.abs(chg) >= 30) {
      alerts.push({ area, label, change_pct: chg, severity: Math.abs(chg) >= 60 ? 'alta' : 'media' });
    }
  };
  const gp = data.google?.previous, mp = data.meta?.previous;
  check('Inversión Meta', data.meta?.spend, mp?.spend, 'Meta Ads');
  check('Conversiones Google', data.google?.conversions, gp?.conversions, 'Google Ads');
  check('Usuarios totales', data.analytics?.top_site?.current?.total_users, data.analytics?.top_site?.previous?.total_users, 'Analytics');
  check('% de rebote', data.analytics?.top_site?.current?.bounce_rate, data.analytics?.top_site?.previous?.bounce_rate, 'Analytics');
  check('Clics orgánicos', data.search_console?.clicks, data.search_console?.previous?.clicks, 'SEO');
  return alerts;
}

// ── Data Quality — detecta inconsistencias sin corregirlas solo, avisa.
function computeDataQuality(data) {
  const issues = [];
  const funnel = data.analytics?.funnel;
  if (funnel) {
    const order = ['view_item', 'add_to_cart', 'view_cart', 'begin_checkout', 'add_shipping_info', 'add_payment_info', 'purchase'];
    let lastCount = Infinity;
    for (const step of order) {
      const c = funnel[step]?.users;
      if (c == null) continue;
      if (c > lastCount * 1.05) issues.push(`El funnel muestra más usuarios en "${step}" (${c}) que en el paso anterior (${lastCount}) — revisar el tracking de eventos de eCommerce.`);
      lastCount = c;
    }
  }
  if (data.ecommerce?.pedidos && data.meta?.purchases && data.google?.conversions) {
    const attributed = (data.meta.purchases || 0) + (data.google.conversions || 0);
    if (attributed > data.ecommerce.pedidos * 1.5) issues.push(`Las conversiones atribuidas por Meta+Google (${Math.round(attributed)}) superan bastante los pedidos reales de eCommerce (${data.ecommerce.pedidos}) — puede haber doble atribución entre plataformas.`);
  }
  return issues;
}

app.get('/api/:slug/nexus-report', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const pt = ['mes_anterior', 'mes_actual', 'ultimos_90_dias'].includes(period_type) ? period_type : 'mes_actual';
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    const data = await computeReportFrioData(client, clientDB.id, slug, pt);
    const periodKey = data.range.start.slice(0, 7);
    const { data: drafts } = await supabase.from('nexus_report_drafts').select('*').eq('client_id', clientDB.id).eq('period', periodKey);
    const draftsBySection = {};
    (drafts || []).forEach(dr => { draftsBySection[dr.section_key] = dr; });

    const sections = {};
    for (const [key, def] of Object.entries(NEXUS_SECTIONS)) {
      sections[key] = { title: def.title, data: def.extract(data), draft: draftsBySection[key] || null };
    }
    res.json({
      period_key: periodKey, period_label: data.range.label,
      sections, score: computeNexusScore(data), alerts: computeNexusAlerts(data), data_quality: computeDataQuality(data),
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/:slug/nexus-report/analyze', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type, section_key } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const def = NEXUS_SECTIONS[section_key];
  if (!def) return res.status(400).json({ error: 'Sección desconocida' });
  const pt = ['mes_anterior', 'mes_actual', 'ultimos_90_dias'].includes(period_type) ? period_type : 'mes_actual';
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    const data = await computeReportFrioData(client, clientDB.id, slug, pt);
    const sectionData = def.extract(data);
    const periodKey = data.range.start.slice(0, 7);

    const prompt = `Sos un analista de marketing digital senior de la agencia Docta Nexus, escribiendo el análisis de la sección "${def.title}" para "${client.name}" — período ${data.range.label}.

Reglas estrictas (nunca las rompas):
- Nunca inventes causalidad que no puedas justificar con el dato — si no hay evidencia suficiente para explicar un cambio, decilo explícitamente ("requiere revisión, no hay dato suficiente para explicarlo con certeza").
- Nunca compares contra un benchmark de industria — no tenés esa información, solo tenés los datos reales de este cliente.
- Si un dato falta o es null, decí que falta — no lo completes con un supuesto.
- Sé concreto y breve, sin relleno.

Datos reales de esta sección:
${JSON.stringify(sectionData, null, 2)}

Respondé en español, en JSON, con este formato exacto:
{"que_paso":"2-3 oraciones, los hechos, sin interpretar","por_que_importa":"1-2 oraciones, el impacto de negocio","que_puede_explicarlo":"1-2 oraciones — hipótesis razonable basada en el dato, o 'requiere revisión' si no hay evidencia suficiente","que_recomendamos":"1-2 oraciones, acción concreta y accionable"}`;
    ...
    const d = await r.json();
    if (!r.ok || d.error) throw new Error('Agente Nexus Report (OpenAI): ' + (d.error?.message || r.status));
    logAiCall({ slug, model: 'gpt-4o', callType: 'nexus_report_section', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 }).catch(() => {});
    const parsed = JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
    const aiText = `Qué pasó: ${parsed.que_paso || ''}\n\nPor qué importa: ${parsed.por_que_importa || ''}\n\nQué puede explicarlo: ${parsed.que_puede_explicarlo || ''}\n\nQué recomendamos: ${parsed.que_recomendamos || ''}`;

    const { data: saved, error } = await supabase.from('nexus_report_drafts')
      .upsert({ client_id: clientDB.id, period: periodKey, section_key, ai_text: aiText, updated_at: new Date().toISOString() }, { onConflict: 'client_id,period,section_key' })
      .select().single();
    if (error) throw new Error(error.message);
...
app.post('/api/:slug/nexus-report/draft', async (req, res) => {
  const slug = req.params.slug;
  const { password, period, section_key, edited_text, included } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    const update = { client_id: clientDB.id, period, section_key, updated_at: new Date().toISOString() };
    if (edited_text !== undefined) update.edited_text = edited_text;
    if (included !== undefined) update.included = included;
    const { data: saved, error } = await supabase.from('nexus_report_drafts')
      .upsert(update, { onConflict: 'client_id,period,section_key' }).select().single();
    if (error) throw new Error(error.message);
    res.json({ draft: saved });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.post('/api/:slug/nexus-report/generate', async (req, res) => {
  const slug = req.params.slug;
  ...
  if (!client || !clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    const { data: drafts } = await supabase.from('nexus_report_drafts').select('*').eq('client_id', clientDB.id).eq('period', period).eq('included', true);
    if (!drafts?.length) return res.status(400).json({ error: 'No hay ninguna sección tildada para incluir en el informe final.' });

    const included = drafts.map(dr => ({
      section_key: dr.section_key, title: NEXUS_SECTIONS[dr.section_key]?.title || dr.section_key,
      text: dr.edited_text || dr.ai_text || '',
    }));

    const synthPrompt = `Sos un estratega de Docta Nexus armando el resumen ejecutivo final del informe de "${client.name}" (${period}), a partir de los análisis YA APROBADOS de cada sección (no inventes nada nuevo, solo sintetizá lo que ya está escrito abajo).

Secciones aprobadas:
${included.map(s => `### ${s.title}\n${s.text}`).join('\n\n')}

Respondé en español, en JSON:
{"executive_summary":"4-6 oraciones sintetizando el mes","recommendations":"3-5 viñetas con '- ', accionables","next_30_days":"3-5 viñetas con '- ', qué hacer en el próximo mes"}`;

    const r = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + OPENAI_KEY },
      body: JSON.stringify({ model: 'gpt-4o', max_tokens: 1200, messages: [{ role: 'user', content: synthPrompt }] }),
    });
    const d = await r.json();
    if (!r.ok || d.error) throw new Error('Agente de síntesis Nexus (OpenAI): ' + (d.error?.message || r.status));
    logAiCall({ slug, model: 'gpt-4o', callType: 'nexus_report_synthesis', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 }).catch(() => {});
    const synth = JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());

    const { data: clientRow } = await supabase.from('clients').select('color_primary, logo_url').eq('id', clientDB.id).single();
    const { data: agencyRow } = await supabase.from('agency_settings').select('logo_url').eq('id', 1).single();

    const reportData = {
      period_label: periodToLabel(period), sections: included,
      executive_summary: synth.executive_summary || '', recommendations: synth.recommendations || '', next_30_days: synth.next_30_days || '',
      color_primary: clientRow?.color_primary || null, client_logo_url: clientRow?.logo_url || null, agency_logo_url: agencyRow?.logo_url || null,
      data_retrieved_at: new Date().toISOString(),
    };
    const token = crypto.randomBytes(8).toString('hex');
    const accessCode = String(crypto.randomInt(100000, 999999));
    const expiresAt = new Date(); expiresAt.setUTCDate(expiresAt.getUTCDate() + 7);
    const { data: row, error } = await supabase.from('nexus_reports').insert({
      client_id: clientDB.id, period, token, access_code: accessCode, report_data: reportData, expires_at: expiresAt.toISOString(),
    }).select().single();
    if (error) throw new Error(error.message);
    res.json({ token: row.token, access_code: row.access_code, url: `/nr/${row.token}`, expires_at: row.expires_at });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.get('/nr/:token', (req, res) => res.sendFile(path.join(__dirname, 'public', 'nexus-report.html')));

app.get('/api/nexus-report/:token/meta', async (req, res) => {
  try {
    const { data, error } = await supabase.from('nexus_reports').select('token, expires_at, report_data, clients(name)').eq('token', req.params.token).limit(1);
    if (error || !data || !data.length) return res.status(404).json({ error: 'No encontrado' });
    const row = data[0];
    const expired = new Date(row.expires_at) < new Date();
    res.json({ expired, expires_at: row.expires_at, period_label: row.report_data?.period_label, business_name: row.clients?.name || '' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/nexus-report/:token/unlock', async (req, res) => {
  const { code } = req.body;
  try {
    const { data, error } = await supabase.from('nexus_reports').select('*, clients(name)').eq('token', req.params.token).limit(1);
    if (error || !data || !data.length) return res.status(404).json({ error: 'No encontrado' });
    const row = data[0];
    ...
    supabase.from('nexus_reports').update({ view_count: (row.view_count || 0) + 1, last_viewed_at: new Date().toISOString() }).eq('token', row.token).then(() => {});
    res.json({ ...row.report_data, business_name: row.clients?.name || '', expires_at: row.expires_at });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  try {
    ...
    const enriched = await Promise.all((items || []).map(async item => {
      ...
      let adsets = null;
      // Modo ABO (presupuesto por grupo de anuncios, solo Meta): el Budget
      // Diario/Mensual de la campaña deja de ser el valor guardado y pasa a
      // ser la SUMA de sus grupos de anuncios — nunca se confía en un valor
      // manual que quedó desactualizado respecto a los grupos reales.
...
app.post('/api/:slug/campaign-plans/period/:sourcePeriodId/copy-to', async (req, res) => {
  ...
  try {
    ...
    for (let i = 0; i < (insertedItems || []).length; i++) {
      ...
      if (item.budget_mode !== 'adset') continue;
...
async function buildMasterImportItems(slug, masterRows, monthStart, monthEnd, existingCodes) {
  ...
  for (const mRow of masterRows) {
    if (existingCodes.has(mRow.plan_code)) continue;
    ...
    toInsert.push({
      ...
      budget_mode: hasAdsets ? 'adset' : 'campaign',
...
app.post('/api/:slug/campaign-plans/period/:periodId/import-from-master', async (req, res) => {
  ...
  try {
    ...
    const toInsert = await buildMasterImportItems(slug, masterRows || [], monthStart, monthEnd, existingCodes);
    ...
    const masterIdByCode = new Map(toInsert.map(it => [it.plan_code, it._master_id]));
    ...
    if (error) throw new Error(error.message);
    await cloneMasterAdsetsToItems(inserted.map(it => ({ ...it, _master_id: masterIdByCode.get(it.plan_code) })), monthStart, monthEnd);
