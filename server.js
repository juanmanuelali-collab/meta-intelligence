...
app.get('/api/:slug/ga4/acquisition-history', async (req, res) => {
  ...
  // Agregar por canal, sumando los días de la semana
  const byChannel = {};
  (data || []).forEach(r => {
    if (!byChannel[r.channel]) byChannel[r.channel] = { channel: r.channel, sessions: 0, users: 0, new_users: 0, engaged_sessions: 0, conversions: 0 };
    const c = byChannel[r.channel];
    c.sessions += r.sessions || 0;
    c.users += r.users || 0;
    c.new_users += r.new_users || 0;
    c.engaged_sessions += r.engaged_sessions || 0;
    c.conversions += r.conversions || 0;
  });
  const channels = Object.values(byChannel)
    .map(c => ({ ...c, engagement_rate: c.sessions ? +(c.engaged_sessions / c.sessions * 100).toFixed(1) : 0 }))
...
// tener histórico, y se agrega por semana para mostrar en el panel.
...
// ── Top URLs y Top Artículos — en vivo, para la semana seleccionada (sin guardar histórico) ──
app.get('/api/:slug/ga4/top-pages', async (req, res) => {
  ...
  try {
    ...
    const sunday = new Date(thursday); sunday.setUTCDate(thursday.getUTCDate() + 3);
    const fmt = d => d.toISOString().split('T')[0];
    const pages = await fetchGA4TopPages(propertyId, fmt(monday), fmt(sunday), 10);
    res.json({ pages });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/:slug/ga4/top-items', async (req, res) => {
  ...
  try {
    ...
    const sunday = new Date(thursday); sunday.setUTCDate(thursday.getUTCDate() + 3);
    const fmt = d => d.toISOString().split('T')[0];
    const items = await fetchGA4TopItems(propertyId, fmt(monday), fmt(sunday), 10);
    res.json({ items });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
// En vivo para la semana seleccionada, con tasa de conversión entre cada paso.
app.get('/api/:slug/ga4/funnel', async (req, res) => {
  ...
  try {
    ...
    const sunday = new Date(thursday); sunday.setUTCDate(thursday.getUTCDate() + 3);
    const fmt = d => d.toISOString().split('T')[0];
    const counts = await fetchGA4FunnelCounts(propertyId, fmt(monday), fmt(sunday));
...
app.post('/api/:slug/intelligence/analyze-ga4-metrics', async (req, res) => {
  ...
  try {
    ...
    const { data: siteRows } = await supabase
      ...
      .eq('client_id', clientDB.id).gte('date', fmt(monday)).lte('date', fmt(sunday));
    ...
    const [topPages, topItems] = await Promise.all([
      fetchGA4TopPages(propertyId, fmt(monday), fmt(sunday), 5),
      fetchGA4TopItems(propertyId, fmt(monday), fmt(sunday), 5),
    ]);
    ...
    await supabase.from('ai_insights').insert({
      client_id: clientDB.id, period_type: 'week', iso_year: Number(iso_year), iso_week: Number(iso_week),
