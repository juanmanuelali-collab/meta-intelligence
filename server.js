...
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  try {
    ...
    const enriched = await Promise.all((items || []).map(async item => {
      totalPlanned += Number(item.budget_mensual) || 0;
      totalPlannedDaily += Number(item.budget_diario) || 0;
      if (item.channel) byChannelPlanned[item.channel] = (byChannelPlanned[item.channel] || 0) + (Number(item.budget_mensual) || 0);
    ...
    (items || []).forEach(it => {
      ...
      if (!it.budget_mensual) completeness.issues.push({ campaign_name: it.campaign_name, issue: 'Sin presupuesto cargado' });
