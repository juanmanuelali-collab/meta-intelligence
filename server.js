...
const FRIO_ANCHOR_KEYS = [
  ...
  'google_rendimiento', 'google_impression_share', 'google_campanas', 'google_pmax_productos', 'google_keywords',
...
async function computeReportFrioData(client, clientId, slug, periodType) {
  ...
  let googleTotals = null, googleCampaigns = [], pmaxProducts = [], keywords = null;
  if (customerId) {
    const [rawRows, products, kw] = await Promise.all([
  ...
  return { range, meta: metaTotals, meta_campaigns: metaCampaigns, google: googleTotals, google_campaigns: googleCampaigns, pmax_products: pmaxProducts, keywords, analytics, ecommerce };
}
...
async function generateClientReportFrio(slug, periodType, options = {}) {
  ...
  const reportData = {
    ...
    pmax_products: data.pmax_products, keywords: data.keywords,
...
app.post('/api/:slug/report-frio/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type, anchors } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['semana', 'mes_anterior', 'mes_actual'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const report = await generateClientReportFrio(slug, period_type, { anchors });
