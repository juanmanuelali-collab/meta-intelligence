...
const { fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts, fetchCampaignMetricsRange, aggregateCampaignRows, fetchTopKeywords } = require('./google_ads'); // Fase 1 — conector Google Ads
...
// KPI Principal, Notas.
const PLAN_COLUMNS = {
  'campaign name': 'campaign_name', 'channel': 'channel',
  'start date': 'start_date', 'end date': 'end_date', 'budget %': 'budget_pct',
  'objective': 'objective', 'target audience': 'target_audience', 'creative': 'creative',
  'status': 'status', 'kpi principal': 'kpi_principal', 'notas': 'notas',
};

// Lee la primera hoja de la planilla del cliente y devuelve las filas como
// objetos {campaign_name, channel, ...}, mapeando por el nombre de columna
...
async function readCampaignPlanFromSheet(sheetId) {
  ...
  const rows = values.slice(1)
    ...
    .map((r, i) => {
      const row = { _row_index: i + 2 }; // fila real en el Sheet (1-indexed + header)
...
async function writeCampaignPlanDraftToSheet(sheetId, tabName, rows) {
  ...
  const header = ['Campaign Name', 'Channel', 'Start Date', 'End Date', 'Budget %', 'Objective', 'Target Audience', 'Creative', 'Status', 'KPI Principal', 'Notas'];
  const values = [header, ...rows.map(r => [
    r.campaign_name || '', r.channel || '', r.start_date || '', r.end_date || '',
    r.budget_pct ?? '', r.objective || '', r.target_audience || '', r.creative || '',
    r.status || 'Proposed', r.kpi_principal || '', r.notas || '',
  ])];
...
async function loadClient(slug) {
  ...
  return {
    ...
    report_profile: (clientDB.report_profile === 'leads') ? 'leads' : 'ecommerce',
...
async function computeReportFacts(client, clientId, slug, periodType) {
  ...
  const isLeads = client.report_profile === 'leads';
  ...
  const [
    ...
    isLeads ? Promise.resolve(null) : fetchEcommercePeriodMetrics(clientId, periodType, range),
    isLeads ? Promise.resolve(null) : fetchEcommercePeriodMetrics(clientId, periodType, { start: range.prevStart, end: range.prevEnd }),
    isLeads ? Promise.resolve(null) : fetchReportFunnel(slug, range.start, range.end),
    isLeads ? Promise.resolve([]) : fetchEcommerce12MonthEvolution(clientId),
    fetchDailyChannelTotals(clientId, range.start, range.end),
    fetchAnalyticsDetail(slug, !isLeads, range.start, range.end),
    isLeads ? fetchReportKeywords(slug, range.start, range.end) : Promise.resolve(null),
  ]);
  ...
  for (const ch of channels) {
    ...
    if (ch.cur.roas != null && ch.cur.roas < chRoasMin) {
      facts.push({ tipo: 'critico', canal: ch.label, hecho: `ROAS del período: ${ch.cur.roas} (objetivo: ${chRoasMin})${roasDelta != null ? `, ${roasDelta >= 0 ? 'subió' : 'bajó'} ${Math.abs(roasDelta)}% vs. el período anterior` : ''}`, confianza: 'alta', impacto_estimado: null });
    } else if (ch.cur.roas != null && ch.cur.roas >= chRoasMin * 1.3) {
      facts.push({ tipo: 'escalar', canal: ch.label, hecho: `ROAS del período: ${ch.cur.roas}, muy por encima del objetivo (${chRoasMin})`, confianza: 'alta', impacto_estimado: null });
    }
  ...
  if (targets.roas_min_combinado != null) {
    ...
    if (combRoas != null && combRoas < roasMinCombinado) {
      facts.push({ tipo: 'critico', canal: 'General', hecho: `ROAS combinado del período (Meta+Google): ${combRoas} (objetivo: ${roasMinCombinado})`, confianza: 'alta', impacto_estimado: null });
    } else if (combRoas != null && combRoas >= roasMinCombinado * 1.2) {
      facts.push({ tipo: 'oportunidad', canal: 'General', hecho: `ROAS combinado del período (Meta+Google): ${combRoas}, por encima del objetivo (${roasMinCombinado})`, confianza: 'media', impacto_estimado: null });
    }
  }
  ...
  // Comparación entre canales (para las burbujas + oportunidad de reasignar presupuesto) — basada en ROAS si hay, si no en conversiones.
  if (metaCur.spend && googleCur.spend) {
    if (metaCur.roas != null && googleCur.roas != null) {
      ...
      if (worst.roas > 0 && best.roas >= worst.roas * 1.4) {
        facts.push({ tipo: 'oportunidad', canal: 'Meta Ads vs. Google Ads', hecho: `${best.label} rinde ${best.roas}x contra ${worst.roas}x de ${worst.label} en el mismo período — hay margen para reasignar presupuesto hacia ${best.label}`, confianza: 'media', impacto_estimado: null });
      }
    }
  }
  ...
  // Costo por lead combinado vs. su propio objetivo (si está configurado)
  if (targets.costo_lead_max != null) {
    const combSpend = (metaCur.spend || 0) + (googleCur.spend || 0);
  ...
  // Palabras clave de Search (perfil "leads") — hechos accionables: la que más
  ...
  if (keywords && keywords.length) {
    ...
    if (bestConv) {
      facts.push({ tipo: 'oportunidad', canal: 'Google Ads', hecho: `La palabra clave "${bestConv.text}" (${bestConv.match_type}) generó ${bestConv.conversions} conversiones con ${bestConv.clicks} clics — la de mejor desempeño del período`, confianza: 'alta', impacto_estimado: null });
    }
    ...
    if (wasted && wasted.cost >= (cpaMax || 0) * 0.5 && wasted.clicks >= 5) {
      facts.push({ tipo: 'critico', canal: 'Google Ads', hecho: `La palabra clave "${wasted.text}" (${wasted.match_type}) gastó $${wasted.cost.toLocaleString('es-AR')} en ${wasted.clicks} clics sin generar ninguna conversión en el período`, confianza: 'media', impacto_estimado: null });
    }
  }

  return { range, facts, metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail, reachData, messagesData, keywords };
}
...
Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","channel_narratives":{"meta":"...","google":"..."},"stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]${funnelText ? ',"funnel_narrative":"..."' : ''}${convText ? ',"conversion_insight":"..."' : ''}${reachText ? ',"reach_narrative":"..."' : ''}${messagesText ? ',"messages_narrative":"..."' : ''}${keywordsText ? ',"keywords_narrative":"..."' : ''}}`;
...
async function generateClientReport(slug, periodType, options = {}) {
  ...
  const profile = client.report_profile === 'leads' ? 'leads' : 'ecommerce';
  ...
  const reportData = {
    ...
    headline: agent.headline, dek: agent.dek, channel_narratives: agent.channel_narratives, stories: agent.stories, actions: agent.actions,
...
// Trae los vínculos guardados de un cliente, como mapa campaign_name -> vínculo
...
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  try {
    ...
    const enriched = await Promise.all(rows.map(async row => {
      const link = links[row.campaign_name];
...
app.post('/api/:slug/campaign-plans/link', async (req, res) => {
  const slug = req.params.slug;
  const { password, campaign_name, linked_source, linked_campaign_id, linked_campaign_name } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  if (!campaign_name) return res.status(400).json({ error: 'Falta campaign_name' });
  try {
    const { error } = await supabase.from('campaign_plans').upsert(
      { client_id: clientDB.id, nombre_campana: campaign_name, linked_source: linked_source || null, linked_campaign_id: linked_campaign_id || null, linked_campaign_name: linked_campaign_name || null, updated_at: new Date().toISOString() },
...
// Escribe una fila puntual del Sheet (edición inline desde el dashboard) —
// usa _row_index (fila real en la hoja) que ya viene en cada fila leída.
async function updateCampaignPlanRowInSheet(sheetId, tabName, rowIndex, fields) {
  const sheets = await getSheetsClient();
  if (!sheets) throw new Error('No se pudo autenticar con Google Sheets — revisar GA4_SERVICE_ACCOUNT/GOOGLE_SERVICE_ACCOUNT.');
  const values = [[
    fields.campaign_name || '', fields.channel || '', fields.start_date || '', fields.end_date || '',
    fields.budget_pct ?? '', fields.objective || '', fields.target_audience || '', fields.creative || '',
    fields.status || '', fields.kpi_principal || '', fields.notas || '',
  ]];
  await sheets.spreadsheets.values.update({
    spreadsheetId: sheetId, range: `'${tabName}'!A${rowIndex}:K${rowIndex}`, valueInputOption: 'USER_ENTERED',
    requestBody: { values },
  });
}

// Editar una fila existente del plan directamente en el Sheet
app.put('/api/:slug/campaign-plans/row', async (req, res) => {
  const slug = req.params.slug;
  const { password, tab_name, row_index, fields } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const sheetId = await getSheetsId(slug);
  if (!sheetId) return res.status(400).json({ error: 'Este cliente todavía no tiene una planilla configurada.' });
  if (!tab_name || !row_index || !fields) return res.status(400).json({ error: 'Falta tab_name, row_index o fields' });
  try {
    await updateCampaignPlanRowInSheet(sheetId, tab_name, row_index, fields);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    ...
    const range = computeReportPeriodRange('mes_actual');
    const since30 = fmtDate(daysAgoDate(30)), until1 = fmtDate(daysAgoDate(1));

    // Traemos las campañas reales UNA sola vez (se usan para: matching por nombre
    // como respaldo cuando una fila no está vinculada a mano, y para el listado
    // de "campañas activas" que se le pasa a la IA como contexto extra).
    const normalize = s => (s || '').toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z0-9]+/g, ' ').trim();
    let metaRaw = [], googleRaw = [];
    try {
      const metaCreds = await getMetaCredentials(slug);
      if (metaCreds?.access_token && metaCreds?.ad_account_id) metaRaw = await fetchMetaCampaignMetricsForRange(metaCreds.ad_account_id, metaCreds.access_token, since30, until1);
    } catch (e) { console.warn('[campaign-plans] Meta para propuesta omitido:', e.message); }
    try {
      const googleCustomerId = await getGoogleAdsAccountId(slug);
      if (googleCustomerId) googleRaw = await fetchCampaignMetricsRange(googleCustomerId, since30, until1);
    } catch (e) { console.warn('[campaign-plans] Google para propuesta omitido:', e.message); }

    const rowsWithActuals = await Promise.all(currentRows.map(async r => {
      const link = links[r.campaign_name];
      let actual = null;
      if (link?.linked_source && link?.linked_campaign_id) {
        // Vínculo manual explícito — la fuente más confiable.
        actual = await fetchCampaignActuals(slug, link.linked_source, link.linked_campaign_id, range.start, range.end);
      } else {
        // Sin vínculo manual: intentamos emparejar por nombre normalizado contra
        // las campañas activas reales, para no quedarnos sin ningún dato.
        const normPlan = normalize(r.campaign_name);
        const metaMatch = metaRaw.find(m => normalize(m.campaign_name) === normPlan || (normPlan && normalize(m.campaign_name).includes(normPlan)));
        const googleMatch = !metaMatch ? googleRaw.find(g => normalize(g.campaign?.name) === normPlan || (normPlan && normalize(g.campaign?.name).includes(normPlan))) : null;
        if (metaMatch) {
          const k = calcKpis(metaMatch);
          actual = { spend: k.spend || 0, conversions: k.purchases || k.leads || 0, revenue: k.revenue || 0, matched_by: 'nombre (Meta)' };
        } else if (googleMatch) {
          const m = googleMatch.metrics || {};
          actual = { spend: Number(m.costMicros || 0) / 1_000_000, conversions: Number(m.conversions || 0), revenue: Number(m.conversionsValue || m.conversions_value || 0), matched_by: 'nombre (Google)' };
        }
      }
      return { ...r, actual };
    }));
    ...
    const planText = rowsWithActuals.map((r, i) => {
      const parts = [`${i + 1}. "${r.campaign_name}" (${r.channel || 'canal N/D'}, status ${r.status || 'N/D'})`];
      if (r.budget_pct) parts.push(`presupuesto planeado: ${r.budget_pct}% del total`);
      if (r.objective) parts.push(`objetivo: ${r.objective}`);
      if (r.kpi_principal) parts.push(`KPI principal: ${r.kpi_principal}`);
      if (r.actual) parts.push(`REAL este mes (últimos 30 días${r.actual.matched_by ? `, emparejado por ${r.actual.matched_by}` : ''}): gasto $${Math.round(r.actual.spend)}, ${Math.round(r.actual.conversions)} conversiones, ingresos $${Math.round(r.actual.revenue)}`);
      else parts.push('sin datos reales disponibles (no está vinculada ni se encontró una campaña activa con nombre parecido)');
      return parts.join(' — ');
    }).join('\n');
    const anyRealData = rowsWithActuals.some(r => r.actual);

    // Campañas ACTIVAS reales de Meta/Google (últimos 30 días) — para que la IA
    // también pueda notar campañas que están corriendo pero no figuran en el plan.
    let liveText = '';
    try {
      const liveNames = new Set();
      metaRaw.forEach(r => r.campaign_name && liveNames.add(`Meta: ${r.campaign_name}`));
      googleRaw.forEach(r => r.campaign?.name && liveNames.add(`Google: ${r.campaign.name}`));
      if (liveNames.size) liveText = `\nCampañas actualmente activas en las cuentas publicitarias (últimos 30 días, puede haber alguna que no esté en el plan de arriba):\n${[...liveNames].join('\n')}\n`;
    } catch (e) { console.warn('[campaign-plans] live campaigns para propuesta omitido:', e.message); }

    // KPI y Objetivos del mes en curso (si existe esa sub-pestaña generada) — para
    // que la propuesta también tenga en cuenta si se está cumpliendo lo que el
    // operador definió como objetivo, no solo la tendencia mes a mes.
    let kpiText = '';
    try {
      ...
      if (kpiRow?.[0]?.targets && Object.keys(kpiRow[0].targets).length) {
        ...
        if (lines.length) kpiText = `\nKPI y Objetivos definidos por el operador para ${currentPeriod} (usalo para saber si el cliente está cumpliendo lo que se propuso, más allá de la tendencia):\n${lines.join('\n')}\n`;
      }
    } catch (e) { console.warn('[campaign-plans] KPI del mes para propuesta omitido:', e.message); }
    ...
    const prompt = `Sos un estratega de Paid Media de la agencia Docta Nexus. Tenés el plan de campañas actual de "${client.name}" y cómo viene rindiendo cada una este mes (datos reales, no inventes números nuevos). Tu trabajo es proponer el plan de ${nextMonthLabel} — NO es copiar el plan actual tal cual: para cada campaña con datos reales, decidí explícitamente si sube, baja, se pausa o sigue igual el presupuesto (Budget %) según cómo rindió, y reflejá esa decisión en el número que devolvés. Si falta algo evidente, sugerí una campaña nueva de tipo similar a las que ya existen (sin inventar plataformas o audiencias que no tengan sentido con el resto del plan).
${!anyRealData ? '\nIMPORTANTE: ninguna fila del plan tiene datos reales disponibles este mes (ni vinculada a mano ni por nombre parecido a una campaña activa) — en ese caso, no tenés base numérica para ajustar presupuestos, así que mantené el plan estructuralmente similar pero dejalo explícito en las notas de cada fila ("sin datos reales para ajustar este mes, se mantiene el plan"). No inventes un ajuste que no podés justificar.\n' : ''}
Plan actual y su desempeño real de este mes:
${planText}
${liveText}${kpiText}
Respondé SOLO en JSON, un array de campañas propuestas para ${nextMonthLabel}, con este formato exacto (mismas columnas que el Sheet real: Campaign Name, Channel, Start Date, End Date, Budget %, Objective, Target Audience, Creative, Status, KPI Principal, Notas):
[{"campaign_name":"...","channel":"Meta|Google|otro","start_date":"YYYY-MM-DD","end_date":"YYYY-MM-DD","budget_pct":0,"objective":"...","target_audience":"...","creative":"...","kpi_principal":"...","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
    ...
    const proposedRows = proposed.map(p => ({
      campaign_name: p.campaign_name || '(sin nombre)', channel: p.channel || '', start_date: p.start_date || '',
      end_date: p.end_date || '', budget_pct: p.budget_pct ?? '', objective: p.objective || '',
      target_audience: p.target_audience || '', creative: p.creative || '',
      kpi_principal: p.kpi_principal || '', notas: p.notas || '', status: 'Proposed',
    }));
    res.json({ next_month_label: nextMonthLabel, suggested_tab_name: nextMonthLabel, rows: proposedRows, any_real_data: anyRealData });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
