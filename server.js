...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    const [data, campaigns, adsets, ads] = await Promise.all([
      ...
      metaFetch(base + ',adset_id,adset_name,campaign_id,campaign_name&time_range=' + timeRange + '&level=adset&limit=100&', token),
      metaFetch(base + ',ad_id,ad_name,adset_id,adset_name,campaign_id,campaign_name&time_range=' + timeRange + '&level=ad&limit=100&', token),
    ]);

    console.log(`[metrics] ${period} | rows: ${data.data?.length} | camps: ${campaigns.data?.length} | adsets: ${adsets.data?.length} | ads: ${ads.data?.length}`);

    // Procesar con las mismas funciones que fetchAccountMetrics
    // para que el frontend reciba el mismo formato en todos los períodos

    // Account
    const accRaw = data.data?.[0] || {};
    const accKpis = calcKpis(accRaw);

    // Campaigns — procesar igual que campaignList en fetchAccountMetrics
    const campaignList = (campaigns.data || []).map(c => {
      const kpis = calcKpis(c);
      const phase = detectPhase(kpis);
      return {
        id: c.campaign_id, name: c.campaign_name,
        status: c.effective_status || 'ACTIVE',
        objective: c.objective || '',
        _actions: filterActions(c.actions || []),
        ...kpis, phase,
        fatigueScore: calcFatigue(kpis),
      };
    });
    ...
    console.log(`[metrics] ${period} ✓ camps:${campaignList.length} adsets:${adsetList.length} ads:${adList.length}`);

    res.json({
      account:   { ...accKpis, _actions: filterActions(accRaw.actions || []) },
      campaigns: campaignList,
      adsets:    adsetList,
      ads:       adList,
      period,
