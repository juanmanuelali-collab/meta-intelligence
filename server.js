...
app.get('/api/admin/clients/:slug', async (req, res) => {
  ...
  const aiUsage = await getAiUsage(slug, 'meta_analysis');
  const featureUsage = {};
  for (const f of ['google_analysis', 'ecommerce_analysis', 'analytics_analysis', 'creatives_dual']) {
...
app.post('/api/:slug/login', async (req, res) => {
  ...
  const aiUsage = await getAiUsage(slug);
  res.json({
    ok: true,
    name: clientDB.name,
...
app.post('/api/:slug/analyze-only', async (req, res) => {
  ...
  const { password, period } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const quota = await checkAiQuota(slug);
  ...
  try {
    ...
    if (clientDBForUsage) await incrementAiUsage(clientDBForUsage.id).catch(() => {});
    const ai_usage = await getAiUsage(slug);
...
app.post('/api/:slug/analyze-gpt', async (req, res) => {
  ...
  const quota = await checkAiQuota(slug); // mismo cupo compartido que el botón Claude
  if (quota.blocked) return res.status(429).json({ error: quota.message, ai_usage: quota.usage });
...
app.get('/api/:slug/ai-usage-all', async (req, res) => {
  ...
  const { password } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const features = ['meta_analysis', 'creatives_dual', 'google_analysis', 'ecommerce_analysis', 'analytics_analysis'];
