...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    // Calcular fechas — usar UTC para consistencia con Meta
    const now = new Date();
    const fmt = d => {
    ...
    const daysAgo = n => { const d = new Date(now); d.setUTCDate(now.getUTCDate()-n); return d; };

    // Período actual y anterior según selección
    let currentPreset, prevSince, prevUntil;
    if (period === 'yesterday') {
      currentPreset = 'yesterday';
      prevSince = daysAgo(2); prevUntil = daysAgo(2);        // anteayer
    } else if (period === '30d') {
      currentPreset = 'last_30d';
      prevSince = daysAgo(60); prevUntil = daysAgo(31);      // 30d anteriores
    } else {
      currentPreset = 'last_7d';
      prevSince = daysAgo(14); prevUntil = daysAgo(8);       // 7d anteriores
    }

    // time_range para período anterior — correctamente encodeado
    const prevTimeRange = encodeURIComponent(JSON.stringify({
      since: fmt(prevSince),
      until: fmt(prevUntil)
    }));
    ...
    const [data, prevData, campaigns, prevCampaigns] = await Promise.all([
      metaFetch(base + '&date_preset=' + currentPreset + '&level=account&', token),
      metaFetch(base + '&time_range=' + prevTimeRange + '&level=account&', token).catch(()=>({data:[]})),
      metaFetch(base + ',campaign_id,campaign_name&date_preset=' + currentPreset + '&level=campaign&limit=50&', token),
