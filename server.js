...
async function fetchAccountMetrics(client) {
  ...
  const fieldsAccount = fieldsBase; // sin campos directos problemáticos
  ...
  function calcKpis(data) {
    ...
    const videoViews3s  = getAction(actions, 'video_view_3s')  || getAction(actions, 'video_view');
    const videoViews10s = getAction(actions, 'video_view_10s');
    const videoViews15s = getAction(actions, 'video_view_15s');
    const videoViews30s = getAction(actions, 'video_view_30s');
    ...
    return {
      ...
      videoP25, videoP50, videoP75, videoP95, thruPlays,
      // Engagement
...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    let currentPreset, prevRange;

    if (period === 'yesterday') {
      // Ayer vs anteayer
      currentPreset = 'yesterday';
      const anteayer = new Date(now); anteayer.setDate(now.getDate() - 2);
      prevRange = `time_range={"since":"${fmt(anteayer)}","until":"${fmt(anteayer)}"}`;
    } else if (period === '30d') {
      // Últimos 30 días vs 30 días previos
      currentPreset = 'last_30d';
      const since = new Date(now); since.setDate(now.getDate() - 60);
      const until = new Date(now); until.setDate(now.getDate() - 31);
      prevRange = `time_range={"since":"${fmt(since)}","until":"${fmt(until)}"}`;
    } else {
      // Últimos 7 días vs 7 días previos
      currentPreset = 'last_7d';
      const since = new Date(now); since.setDate(now.getDate() - 14);
      const until = new Date(now); until.setDate(now.getDate() - 8);
      prevRange = `time_range={"since":"${fmt(since)}","until":"${fmt(until)}"}`;
    }

    // Fetchear período actual y anterior en paralelo
    const [data, prevData, campaigns, prevCampaigns] = await Promise.all([
      metaFetch('/' + accountId + '/insights?fields=' + fields + '&date_preset=' + currentPreset + '&level=account&', token),
      metaFetch('/' + accountId + '/insights?fields=' + fields + '&' + prevRange + '&level=account&', token),
      metaFetch('/' + accountId + '/insights?fields=' + fields + ',campaign_id,campaign_name&date_preset=' + currentPreset + '&level=campaign&limit=50&', token),
      metaFetch('/' + accountId + '/insights?fields=' + fields + ',campaign_id,campaign_name&' + prevRange + '&level=campaign&limit=50&', token).catch(()=>({data:[]})),
    ]);
...
// Diagnóstico: ver todos los action types que devuelve Meta
