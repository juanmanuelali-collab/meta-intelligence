...
const { loadClientDB, loadClientDBAny, verifyPassword, checkPassword, generateClientToken, generateAdminToken, verifyAdminToken, getMetaCredentials, updateMetaToken, getGoogleAdsAccountId, getGA4PropertyId, getSearchConsoleProperty, setSearchConsoleProperty, getSheetsId, setSheetsId, createClientRecord, setMetaCredentials, setGoogleAdsAccountId, setGA4PropertyId, updateClientRecord, getClientCredentialsSummary, setCredentialError, getAllCredentialsHealth, getAiUsage, incrementAiUsage, supabase } = require('./db'); // Sub-pasos 3.1/3.2/3.3 — auth + credenciales vía Supabase; ahora también tokens de sesión (Fase 1 hardening login)
...
const { fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts, fetchCampaignMetricsRange, aggregateCampaignRows, fetchTopKeywords, fetchConversionActionsSummary, fetchAllCampaignsStructure, fetchAccountCurrency: fetchGoogleAdsAccountCurrency, fetchConversionsByWeekday } = require('./google_ads'); // Fase 1 — conector Google Ads
const { fetchGA4WeeklyMetrics, fetchGA4AcquisitionForDate, fetchGA4SiteMetricsForDate, fetchGA4AcquisitionForRange, fetchGA4SiteMetricsForRange, fetchGA4TopPages, fetchGA4TopItems, fetchGA4FunnelCounts, GA4_FUNNEL_EVENTS, fetchGA4TopItemsDetailed, fetchGA4DeviceBreakdown, fetchGA4Demographics, fetchGA4KeyEventsSummary, fetchGA4BrowserBreakdown, fetchGA4OSBreakdown, fetchGA4TopSiteSummary, fetchGA4ChannelsByUsers, fetchGA4SourceMedium, fetchGA4DailySeries, fetchGA4AnnualUsersByMonth, fetchGA4AnnualChannels, fetchGA4CityBreakdown } = require('./ga4'); // Integración GA4 — tráfico y eventos por semana (post-Fase 5)
...
// Chequea el cupo de consultas IA de un cliente ANTES de gastar la llamada
// real a Claude/GPT. No incrementa el contador acá — eso pasa recién si la
// llamada a la IA sale bien, para no descontarle una consulta a un cliente
// por un error nuestro. Devuelve { blocked, usage, message }.
async function checkAiQuota(slug, feature = 'meta_analysis') {
  const usage = await getAiUsage(slug, feature);
  if (!usage) return { blocked: false, usage: null };
  if (usage.limit !== null && usage.used >= usage.limit) {
    return {
      blocked: true,
      usage,
      message: `Alcanzaste el límite de ${usage.limit} consultas mensuales de esta función de IA. Se renueva el 1° del próximo mes.`,
    };
  }
  return { blocked: false, usage };
}

// Funciones/solapas del menú — default: todo habilitado (cliente sin
// enabled_features configurado explícitamente en Supabase no pierde acceso
// a nada). Las claves matchean el data-feature de cada nav-tab en dashboard.html.
const DEFAULT_ENABLED_FEATURES = {
  kpis: true, meta_ads: true, google_ads: true, ecommerce: true, analytics: true,
  general_analysis: true, creatives: true, search_console: true,
  informe_story: true, informe_frio_general: true, plan_campanias: true, reporte_frio: true,
  ai_meta_claude: true, ai_meta_gpt: true, ai_google_claude: true, ai_google_gpt: true,
  ai_creatives_claude: true, ai_creatives_gpt: true,
};
...
// ── Fase 3 — hardening general ──────────────────────────────────
// contentSecurityPolicy y crossOriginEmbedderPolicy quedan OFF a propósito:
// dashboard.html/admin.html/report*.html son SPAs de un solo archivo con
// <script> inline por todos lados y cargan Chart.js/Tabler Icons/reCAPTCHA
// desde CDNs externos + imágenes de Supabase Storage. Un CSP estricto por
// defecto rompe todo eso. El resto de las protecciones de helmet (noSniff,
// frameguard/X-Frame-Options, hsts, etc.) quedan activas igual. Si más
// adelante se quiere CSP real, hay que armar la whitelist a mano y
// probarla módulo por módulo — no es cosa de un solo paso.
app.use(helmet({
  contentSecurityPolicy: false,
  crossOriginEmbedderPolicy: false,
}));

// CORS explícito — restringido a los dominios propios de Docta Nexus.
// El dashboard funciona same-origin (no necesita esto para andar), pero
// sin esto cualquier origen externo podía golpear la API igual.
const ALLOWED_ORIGINS = (process.env.ALLOWED_ORIGINS || 'https://analytics.doctanexus.com,https://doctanexus.com')
  .split(',').map(o => o.trim()).filter(Boolean);
app.use(cors({
  origin(origin, callback) {
    if (!origin) return callback(null, true); // server-to-server, curl, health checks del propio Render
    if (ALLOWED_ORIGINS.includes(origin)) return callback(null, true);
    callback(new Error('Origen no permitido por CORS: ' + origin));
  },
}));

// Rate limiting — SOLO en los 2 endpoints de login reales, para no afectar
// la UX del resto del dashboard (igual que reCAPTCHA). 5 intentos cada 10
// minutos por IP; ajustar si en el uso real resulta muy estricto/laxo.
...
function calcKpis(data) {
  ...
  const onsiteDonate     = getAction(actions, 'onsite_conversion.donate');
  ...
  // Meta reporta la donación de varias formas según cómo esté
  // configurado el evento en el cliente:
  //  - offsite_conversion.fb_pixel_donate → evento ESTÁNDAR del Pixel
  //    "Donate" (se ve en Events Manager como "Hacer donación" en español).
  //    Este es el más común cuando el cliente tiene el Pixel bien
  //    configurado — se chequea primero.
  //  - donate_website / donate_on_facebook / donate_mobile_app / donate_offline
  //    → desglose por canal que a veces expone la API en paralelo
  //  - donate_total       → agregado oficial de Meta de los 4 anteriores (no siempre viene)
  //  - onsite_conversion.donate → variante legacy del botón nativo, último fallback
  const pixelDonate      = getAction(actions, 'offsite_conversion.fb_pixel_donate');
  const donateWebsite    = getAction(actions, 'donate_website');
  const donateOnFacebook = getAction(actions, 'donate_on_facebook');
  const donateMobileApp  = getAction(actions, 'donate_mobile_app');
  const donateOffline    = getAction(actions, 'donate_offline');
  const donateTotalRaw   = getAction(actions, 'donate_total');
  const donateChannelSum = donateWebsite + donateOnFacebook + donateMobileApp + donateOffline;
  const donations      = pixelDonate || donateTotalRaw || donateChannelSum || onsiteDonate || getAction(actions, 'donate');
  const donateValueChannelSum = getActionValue(actionValues, 'donate_website') +
                        getActionValue(actionValues, 'donate_on_facebook') +
                        getActionValue(actionValues, 'donate_mobile_app') +
                        getActionValue(actionValues, 'donate_offline');
  const donationValue  = getActionValue(actionValues, 'offsite_conversion.fb_pixel_donate') ||
                        getActionValue(actionValues, 'donate_total') ||
                        donateValueChannelSum ||
                        getActionValue(actionValues, 'onsite_conversion.donate') ||
                        getActionValue(actionValues, 'donate');
  ...
  return {
    ...
    // Onsite
    onsitePurchase, onsiteAddToCart, onsiteCheckout, onsiteDonate,
    // Consolidados
    purchases, purchaseValue, addToCart, initiateCheckout, completeReg,
    donations, donationValue,
    // Donaciones — desglose por canal (por si se quiere mostrar aparte)
    donateWebsite, donateOnFacebook, donateMobileApp, donateOffline,
    leads, leadgenLeads,
    // Tráfico
...
// ── Action types CRUDOS de Meta a nivel cuenta, sin pasar por filterActions()
// (que filtra contra VALID_ACTION_TYPES, una lista curada de ~30 tipos).
// Panel operador — mostrar TODO, sin filtrar, para encontrar el action type
// no mapeado que se busca al configurar "Conversión Principal". ──
async function fetchMetaRawActionTypesSummary(accountId, token, since, until) {
  const timeRange = encodeURIComponent(JSON.stringify({ since, until }));
  const path = `/${accountId}/insights?fields=actions,action_values,spend&time_range=${timeRange}&level=account&limit=1&`;
  const result = await metaFetch(path, token);
  const row = (result.data || [])[0];
  if (!row) return [];
  const values = {};
  (row.action_values || []).forEach(v => { values[v.action_type] = Number(v.value || 0); });
  return (row.actions || [])
    .map(a => ({ action_type: a.action_type, count: Number(a.value || 0), value: values[a.action_type] || 0 }))
...
const VALID_ACTION_TYPES = new Set([
  ...
  'offsite_conversion.fb_pixel_donate',
  // Onsite (on-Facebook)
...
Respondé ÚNICAMENTE con este JSON (sin markdown):
{
  ...
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'x-api-key': ANTHROPIC_KEY, 'anthropic-version': '2023-06-01' },
    body: JSON.stringify({ model: 'claude-opus-4-8', max_tokens: 6000, system, messages: [{ role: 'user', content: prompt }] })
  });
  const d = await r.json();
  if (!r.ok) throw new Error(d.error?.message || 'Error Claude');
  logAiCall({ slug: client.slug || 'sistema', model: 'claude-opus-4-8', callType: 'dashboard_analysis', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
...
// Sincroniza el Status del Plan de Campañas (mes actual, todos los clientes)
// con el estado real de Meta/Google una vez al día — de fondo, sin depender
// de que alguien toque el botón "Actualizar match" a mano.
cron.schedule('30 7 * * *', async () => {
  console.log('[CRON] Sincronizando estado de campañas del Plan (mes actual)...');
  const currentPeriod = fmtDate(new Date()).slice(0, 7);
  for (const slug of await listClients()) {
    try {
      const clientDB = await loadClientDB(slug);
      if (!clientDB) continue;
      const { data: periodRows } = await supabase.from('campaign_plan_periods').select('id').eq('client_id', clientDB.id).eq('period', currentPeriod).order('version_number', { ascending: false }).limit(1);
      if (!periodRows?.length) continue;
      const { data: items } = await supabase.from('campaign_plan_items').select('*').eq('plan_period_id', periodRows[0].id);
      for (const item of (items || [])) {
        const match = await matchCampaignByName(slug, item.channel, item.campaign_name);
        const update = buildMatchUpdate(item, match);
        if (Object.keys(update).length) {
          update.updated_at = new Date().toISOString();
          await supabase.from('campaign_plan_items').update(update).eq('id', item.id);
        }
      }
      console.log(`[CRON] Estado del Plan sincronizado: ${slug}`);
    } catch (e) {
      console.error(`[CRON] Error sincronizando estado del Plan en ${slug}:`, e.message);
    }
  }
}, { timezone: TZ });
...
// Usada SOLO en /api/admin/login (real password contra la env var). Antes
// comparaba con === (no timing-safe: un atacante puede medir microsegundos
// de diferencia según cuántos caracteres coinciden e ir adivinando de a
// uno). timingSafeEqual necesita buffers del mismo largo, por eso el chequeo
// de longitud antes — placeholder.length!==real.length ya es información en
// sí misma, pero es solo "sí/no coincide el largo", no de a un caracter.
function checkAdminPassword(password) {
  const real = process.env.ADMIN_PASSWORD;
  if (!real || typeof password !== 'string') return false;
...
// Usada en TODOS los demás endpoints /api/admin/* (los que no son el login
// en sí). Valida el token que emitió /api/admin/login, nunca el password
// real de nuevo.
function checkAdminToken(token) {
  return !!verifyAdminToken(token);
}

// ── reCAPTCHA Enterprise — verificación de token contra Google Cloud ──
// Se usa SOLO en los 2 endpoints de login reales (admin y cliente), no en
// las rutas ya autenticadas del dashboard, para no romper la UX de las
// llamadas que se repiten constantemente durante una sesión ya iniciada.
const RECAPTCHA_PROJECT_ID = process.env.RECAPTCHA_PROJECT_ID || 'contectnexus';
async function verifyCaptcha(token, expectedAction) {
  if (!token) { console.log('[captcha] falta token'); return false; }
  const apiKey = process.env.RECAPTCHA_API_KEY;
  const siteKey = process.env.RECAPTCHA_SITE_KEY;
  if (!apiKey || !siteKey) { console.error('[captcha] RECAPTCHA_API_KEY/RECAPTCHA_SITE_KEY no configuradas en Render'); return false; }
  ...
  try {
    ...
    if (!data.tokenProperties?.valid) {
      console.log('[captcha] token inválido:', data.tokenProperties?.invalidReason || data.error?.message);
      return false;
    }
    if (data.tokenProperties.action !== expectedAction) {
      console.log('[captcha] action no coincide. Esperado:', expectedAction, 'Recibido:', data.tokenProperties.action);
      return false;
    }
    const score = data.riskAnalysis?.score ?? 0;
    console.log('[captcha] score:', score, 'action:', expectedAction, 'reasons:', data.riskAnalysis?.reasons);
    return score >= 0.5;
  } catch (e) {
    console.error('[captcha] error al verificar contra Google:', e.message);
    return false; // si la verificación falla, no dejamos pasar el login
  }
}

app.post('/api/admin/login', loginLimiter, async (req, res) => {
  const { password, captchaToken } = req.body;
  if (!(await verifyCaptcha(captchaToken, 'login_admin'))) return res.status(400).json({ error: 'Verificación de seguridad falló. Recargá la página e intentá de nuevo.' });
  if (!checkAdminPassword(password)) return res.status(401).json({ error: 'Contraseña incorrecta' });
  // El password real viajó UNA sola vez, acá. De ahora en más el frontend
  // usa este token — nunca vuelve a mandar ADMIN_PASSWORD.
...
app.get('/api/admin/clients', async (req, res) => {
  ...
  if (!checkAdminToken(password)) return res.status(401).json({ error: 'No autorizado' });
  const { data, error } = await supabase
    .from('clients')
...
app.post('/api/admin/clients', async (req, res) => {
  ...
  const existing = await loadClientDBAny(slug);
...
app.get('/api/admin/clients/:slug', async (req, res) => {
  ...
  const { password } = req.query;
  if (!checkAdminToken(password)) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDBAny(slug); // sin filtro de activo — el admin tiene que poder ver/reactivar clientes desactivados
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const creds = await getClientCredentialsSummary(slug);
  const featureUsage = {};
  for (const f of ['meta_claude_analysis', 'meta_gpt_analysis', 'google_claude_analysis', 'google_gpt_analysis', 'creatives_claude_analysis', 'creatives_gpt_analysis']) {
    featureUsage[f] = await getAiUsage(slug, f);
  }
  res.json({
    client: {
      ...
      enabled_features: clientDB.enabled_features || DEFAULT_ENABLED_FEATURES,
      ai_monthly_limit: (clientDB.ai_monthly_limit === null || clientDB.ai_monthly_limit === undefined) ? '' : clientDB.ai_monthly_limit,
      ai_feature_limits: clientDB.ai_feature_limits || {},
      logo_url: clientDB.logo_url || null,
      report_frio_config: clientDB.report_frio_config || null,
    },
    credentials: creds,
    ai_feature_usage: featureUsage,
  });
});
...
app.put('/api/admin/clients/:slug', async (req, res) => {
  const slug = req.params.slug;
  const { password, name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit, aiFeatureLimits, reportFrioConfig } = req.body;
  if (!checkAdminToken(password)) return res.status(401).json({ error: 'No autorizado' });
  // aiMonthlyLimit llega como '' (string vacío) desde el input del admin cuando el operador
  // quiere "ilimitado" — se guarda como null en la columna, nunca como 0 (0 bloquearía todo).
  const parsedLimit = aiMonthlyLimit === undefined ? undefined : (aiMonthlyLimit === '' || aiMonthlyLimit === null ? null : Number(aiMonthlyLimit));
  const ok = await updateClientRecord(slug, { name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit: parsedLimit, aiFeatureLimits, reportFrioConfig });
...
// ── Logos del Informe Frío — subida real de imagen a Supabase Storage
// (bucket público "report-logos"), no solo pegar una URL. Acepta la
// imagen en base64 desde el frontend (input type=file → FileReader).
async function uploadLogoToStorage(base64DataUrl, filenamePrefix) {
  const match = base64DataUrl.match(/^data:(image\/[a-zA-Z0-9+.-]+);base64,(.+)$/);
  if (!match) throw new Error('Formato de imagen inválido — tiene que ser un data URL base64.');
  const [, mimeType, base64Data] = match;
  const ext = mimeType.split('/')[1].replace('jpeg', 'jpg');
  const fileName = `${filenamePrefix}-${Date.now()}.${ext}`;
  const buffer = Buffer.from(base64Data, 'base64');
  const { error } = await supabase.storage.from('report-logos').upload(fileName, buffer, { contentType: mimeType, upsert: true });
  if (error) throw new Error(error.message);
  const { data } = supabase.storage.from('report-logos').getPublicUrl(fileName);
  return data.publicUrl;
}

app.get('/api/admin/agency-logo', async (req, res) => {
  const { password } = req.query;
  if (!checkAdminToken(password)) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data } = await supabase.from('agency_settings').select('logo_url').eq('id', 1).single();
    res.json({ logo_url: data?.logo_url || null });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/admin/agency-logo', async (req, res) => {
  const { password, image_base64 } = req.body;
  if (!checkAdminToken(password)) return res.status(401).json({ error: 'No autorizado' });
  if (!image_base64) return res.status(400).json({ error: 'Falta la imagen' });
  try {
    const url = await uploadLogoToStorage(image_base64, 'agency');
    const { error } = await supabase.from('agency_settings').update({ logo_url: url, updated_at: new Date().toISOString() }).eq('id', 1);
    if (error) throw new Error(error.message);
...
app.post('/api/admin/clients/:slug/logo', async (req, res) => {
  const slug = req.params.slug;
  const { password, image_base64 } = req.body;
  // Este logo es específico de un cliente — lo puede subir tanto desde /admin
  // (contraseña de admin) como desde el propio dashboard de ese cliente
  // (su contraseña de sesión, ya que quien la tiene ya tiene acceso completo
  // a los datos de ese cliente igual).
  const authorized = checkAdminToken(password) || await checkPassword(slug, password);
  if (!authorized) return res.status(401).json({ error: 'No autorizado' });
  if (!image_base64) return res.status(400).json({ error: 'Falta la imagen' });
  try {
    const url = await uploadLogoToStorage(image_base64, `client-${slug}`);
    const clientDB = await loadClientDB(slug);
    if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
    const { error } = await supabase.from('clients').update({ logo_url: url }).eq('id', clientDB.id);
    if (error) throw new Error(error.message);
    res.json({ ok: true, logo_url: url });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.post('/api/:slug/login', loginLimiter, async (req, res) => {
  const slug = req.params.slug;
  const { password, captchaToken } = req.body;

  if (!(await verifyCaptcha(captchaToken, 'login_client'))) return res.status(400).json({ error: 'Verificación de seguridad falló. Recargá la página e intentá de nuevo.' });
  ...
  res.json(await buildClientLoginPayload(slug, clientDB, generateClientToken(slug)));
});

// Arma la respuesta que espera el frontend tanto en el login real como en
// el resumen silencioso de sesión (/session) — para no duplicar el bloque.
async function buildClientLoginPayload(slug, clientDB, token) {
  const aiUsageClaude = await getAiUsage(slug, 'meta_claude_analysis');
  const aiUsageGpt = await getAiUsage(slug, 'meta_gpt_analysis');
  return {
    ok: true,
    // El password real del cliente viajó UNA sola vez, en /login. De ahora
    // en más el frontend usa este token en el resto de los ~104 endpoints —
    // nunca vuelve a mandar el password real (ni en query string ni en body).
    token,
    name: clientDB.name,
    colorPrimary: clientDB.color_primary || '#1B17FF',
    colorAccent: clientDB.color_accent || '#FF751F',
    colorBtnText: clientDB.color_btn_text || '#ffffff',
    kpi_targets: clientDB.kpi_targets,
    ...
    logo_url: clientDB.logo_url || null,
    enabled_features: clientDB.enabled_features || DEFAULT_ENABLED_FEATURES,
    ai_usage_claude: aiUsageClaude,
    ai_usage_gpt: aiUsageGpt,
    report_profile: clientDB.report_profile || 'ecommerce',
    has_report_frio_config: !!clientDB.report_frio_config,
  };
}

// Resumen silencioso de sesión — reemplaza el autoLogin viejo que releía
// el password guardado en localStorage y volvía a pegarle a /login. Ahora
// el navegador solo tiene el token (30 días de vida); esto lo valida y
// devuelve el mismo payload que un login real, SIN pedir captcha ni
// password de nuevo. Si el token venció o es inválido, 401 — el frontend
// vuelve a mostrar el form de login normal, ahí sí con captcha.
app.get('/api/:slug/session', async (req, res) => {
  const slug = req.params.slug;
  const token = (req.headers.authorization || '').replace(/^Bearer\s+/i, '') || req.query.token;
  if (!(await checkPassword(slug, token))) return res.status(401).json({ error: 'Sesión vencida' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  res.json(await buildClientLoginPayload(slug, clientDB, token));
});
...
app.get('/api/:slug/dashboard', async (req, res) => {
  ...
  try {
    ...
    const liveCurrency = await getAccountCurrency(slug, 'meta');
    res.json({ ...result, history, currency: liveCurrency || client.currency || 'ARS', report_profile: client.report_profile });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/refresh-metrics', async (req, res) => {
  ...
  try {
    ...
    await saveDataDB(slug, result);
    const liveCurrency = await getAccountCurrency(slug, 'meta');
    res.json({ ...result, history: await loadHistoryDB(slug, 14), currency: liveCurrency || client.currency || 'ARS' });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
function computeReportPeriodRange(periodType) {
  ...
  if (periodType === 'ultimos_90_dias') {
    const end = new Date(today); end.setUTCDate(today.getUTCDate() - 1);
    const start = new Date(end); start.setUTCDate(end.getUTCDate() - 89);
    const prevEnd = new Date(start); prevEnd.setUTCDate(start.getUTCDate() - 1);
    const prevStart = new Date(prevEnd); prevStart.setUTCDate(prevEnd.getUTCDate() - 89);
    return {
      start: fmt(start), end: fmt(end), prevStart: fmt(prevStart), prevEnd: fmt(prevEnd),
      label: `últimos 90 días (${start.getUTCDate()} ${MESES_ES[start.getUTCMonth()]} al ${end.getUTCDate()} ${MESES_ES[end.getUTCMonth()]})`,
    };
  }
...
async function aggregateChannelPeriod(slug, source, start, end) {
  const EMPTY = { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null, reach: 0, impressions: 0, clicks: 0, frequency: null, engagement: 0, messages: 0, cpmsg: null, ctr: null, conversionRate: null, depth2: 0, depth3: 0, depth5: 0 };
  try {
    if (source === 'meta') {
      const creds = await getMetaCredentials(slug);
      ...
      let spend = 0, conversions = 0, revenue = 0, reach = 0, impressions = 0, clicks = 0, engagement = 0, messages = 0, freqWeighted = 0, depth2 = 0, depth3 = 0, depth5 = 0;
      for (const row of raw) {
        ...
        depth2 += (k.msgDepth2 || 0);
        depth3 += (k.msgDepth3 || 0);
        depth5 += (k.msgDepth5 || 0);
      ...
      return {
        ...
        depth2: Math.round(depth2), depth3: Math.round(depth3), depth5: Math.round(depth5),
      };
    }
...
// ── Definición de las métricas marcables como "Conversión Principal" en el
// rediseño de KPI y Objetivos. accessor() lee el valor actual desde el
// resultado de computeMonthlyKpiActuals() — los 4 slots "custom" no tienen
// accessor fijo, se resuelven aparte contra el label que cargó el operador. ──
const KPI_METRIC_DEFS = [
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'roas_global', label: 'ROAS Global', format: 'ratio', accessor: a => a.ecommerce?.roas, profiles: ['ecommerce','hibrido'] },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'pedidos', label: 'Pedidos', format: 'number', accessor: a => a.ecommerce?.pedidos, profiles: ['ecommerce','hibrido'] },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'facturacion', label: 'Facturación', format: 'money', accessor: a => a.ecommerce?.facturacion, profiles: ['ecommerce','hibrido'] },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'tasa_conversion_ecommerce', label: 'Tasa de Conversión (eCommerce)', format: 'percent', accessor: a => a.ecommerce?.tasa_conversion_ecommerce, profiles: ['ecommerce','hibrido'] },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'ticket_promedio', label: 'Ticket Promedio', format: 'money', accessor: a => a.ecommerce?.ticket_promedio, profiles: ['ecommerce','hibrido'] },

  { group: 'meta', group_label: 'Meta Ads', key: 'meta_roas', label: 'ROAS', format: 'ratio', accessor: a => a.meta?.roas },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_ctr', label: 'CTR', format: 'percent', accessor: a => a.meta?.ctr },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_frequency', label: 'Frecuencia', format: 'number2', accessor: a => a.meta?.frequency, lower_is_better: true },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_messages', label: 'Mensajes', format: 'number', accessor: a => a.meta?.messages },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_cpmsg', label: 'CPA de los Mensajes', format: 'money', accessor: a => a.meta?.cpmsg, lower_is_better: true },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_purchases', label: 'Compras', format: 'number', accessor: a => a.meta?.conversions },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_cpa', label: 'Coste Conversión Compras', format: 'money', accessor: a => a.meta?.cpa, lower_is_better: true },
  { group: 'meta', group_label: 'Meta Ads', key: 'meta_revenue', label: 'Valor de Conversión', format: 'money', accessor: a => a.meta?.revenue },

  { group: 'google', group_label: 'Google Ads', key: 'google_conversions', label: 'Conversiones', format: 'number', accessor: a => a.google?.conversions },
  { group: 'google', group_label: 'Google Ads', key: 'google_roas', label: 'ROAS', format: 'ratio', accessor: a => a.google?.roas },
  { group: 'google', group_label: 'Google Ads', key: 'google_cpa', label: 'CPA', format: 'money', accessor: a => a.google?.cpa, lower_is_better: true },
  { group: 'google', group_label: 'Google Ads', key: 'google_ctr', label: 'CTR', format: 'percent', accessor: a => a.google?.ctr },
  { group: 'google', group_label: 'Google Ads', key: 'google_cpc', label: 'CPC', format: 'money', accessor: a => (a.google?.spend && a.google?.clicks) ? a.google.spend / a.google.clicks : null, lower_is_better: true },
  { group: 'google', group_label: 'Google Ads', key: 'google_custom_1', label: null, format: 'number', custom: true },
  { group: 'google', group_label: 'Google Ads', key: 'google_custom_2', label: null, format: 'number', custom: true },

  { group: 'analytics', group_label: 'Analytics', key: 'analytics_users', label: 'Total de Usuarios', format: 'number', accessor: a => a.analytics?.users },
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_conv_rate', label: 'Tasa de Conversión del Sitio', format: 'percent', accessor: a => a.ecommerce?.tasa_conversion_ecommerce },
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_engagement', label: 'Tiempo de Interacción', format: 'seconds', accessor: a => a.analytics?.engagement_seconds },
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_bounce', label: 'Tasa de Rebote', format: 'percent', accessor: a => a.analytics?.bounce_rate, lower_is_better: true },
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_custom_1', label: null, format: 'number', custom: true },
  { group: 'analytics', group_label: 'Analytics', key: 'analytics_custom_2', label: null, format: 'number', custom: true },
];
...
async function computeMonthlyKpiActuals(slug, clientId, period) {
  ...
  return {
    ...
    google: googleC.spend ? {
      ...
      ctr: googleC.ctr, cpa: googleC.cpa, spend: googleC.spend, clicks: googleC.clicks,
    } : null,
    analytics,
  };
}

// ── Promedio de los últimos 12 meses (o los que haya con datos) para la
// columna "KPI Global" de Conversión Principal — a pedido del usuario,
// reemplaza el valor de un solo mes por un promedio más representativo
// del baseline real. Se trae en vivo de las APIs cada vez (más lento mes
// a mes que un solo período, pero es la precisión que se pidió). Cada
// mes se resuelve en paralelo y los que fallan o no tienen dato para una
// métrica puntual simplemente no cuentan para el promedio de esa métrica
// — nunca se rellena con 0 ni se asume.
function last12PeriodsEnding(period) {
  const [y, m] = period.split('-').map(Number);
  const out = [];
  for (let i = 0; i < 12; i++) {
    let yy = y, mm = m - i;
...
async function computeKpiActuals12moAverage(slug, clientId, customLabels) {
  const periods = last12PeriodsEnding(fmtDate(new Date()).slice(0, 7));
  const perMonth = await Promise.all(periods.map(async period => {
    try {
      const { start, end } = periodToMonthRange(period);
      const [actuals, customActuals] = await Promise.all([
        computeMonthlyKpiActuals(slug, clientId, period),
        computeCustomKpiActuals(slug, start, end, customLabels),
      ]);
      return { actuals, customActuals };
    } catch (e) {
      console.warn(`[kpi-targets] promedio 12m — mes ${period} omitido:`, e.message);
      return null;
    }
  }));
  ...
  const addValue = (key, val) => {
    ...
    sums[key] = (sums[key] || 0) + Number(val);
    counts[key] = (counts[key] || 0) + 1;
  };

  perMonth.filter(Boolean).forEach(({ actuals, customActuals }) => {
    KPI_METRIC_DEFS.forEach(def => {
      if (def.custom) addValue(def.key, customActuals[def.key]);
      else addValue(def.key, def.accessor(actuals));
    });
  });

  const avg = {};
  Object.keys(counts).forEach(key => { avg[key] = { value: +(sums[key] / counts[key]).toFixed(2), months_used: counts[key] }; });
  return avg;
}
// ── Actual de los 4 slots custom (google_custom_1/2, analytics_custom_1/2) —
// se resuelve buscando por nombre exacto (label cargado a mano por el
// operador) dentro de los objetivos de Google Ads / eventos de Analytics del
// período, reusando las mismas funciones que alimentan los paneles nuevos. ──
async function computeCustomKpiActuals(slug, since, until, customLabels) {
  const out = {};
  const googleSlots = ['google_custom_1', 'google_custom_2'].filter(k => customLabels[k]);
  const analyticsSlots = ['analytics_custom_1', 'analytics_custom_2'].filter(k => customLabels[k]);

  if (googleSlots.length) {
    try {
      const customerId = await getGoogleAdsAccountId(slug);
      if (customerId) {
        const objetivos = await fetchConversionActionsSummary(customerId, since, until);
        googleSlots.forEach(slot => {
          const match = objetivos.find(o => o.name === customLabels[slot]);
          out[slot] = match ? match.conversions : null;
        });
      }
    } catch (e) { console.warn('[kpi-targets] actual custom Google omitido:', e.message); }
  }
  if (analyticsSlots.length) {
    try {
      const propertyId = await getGA4PropertyId(slug);
      if (propertyId) {
        const eventos = await fetchGA4KeyEventsSummary(propertyId, since, until);
        analyticsSlots.forEach(slot => {
          const match = eventos.find(e => e.event_name === customLabels[slot]);
          out[slot] = match ? match.count : null;
        });
      }
    } catch (e) { console.warn('[kpi-targets] actual custom Analytics omitido:', e.message); }
  }
  return out;
}

app.get('/api/:slug/kpi-targets', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period } = req.query; // period = 'YYYY-MM'
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const targetPeriod = period || fmtDate(new Date()).slice(0, 7);
    const [{ data: targetsRows }, { data: labelsRows }] = await Promise.all([
      supabase.from('client_kpi_targets').select('metric_key, month, target_value').eq('client_id', clientDB.id),
      supabase.from('client_kpi_custom_labels').select('slot_key, label').eq('client_id', clientDB.id),
    ]);

    const customLabels = {};
    (labelsRows || []).forEach(r => { customLabels[r.slot_key] = r.label; });

    // "KPI Global" — promedio de los últimos 12 meses. Ya NO se calcula en
    // vivo en cada carga (era muy lento, hasta 12x llamadas a las APIs por
    // cada vez que se abría la pestaña) — ahora se lee de una caché que el
    // operador actualiza a demanda con el botón "Actualizar promedios"
    // (POST /kpi-targets/refresh-averages).
    const avg12mo = clientDB.kpi_avg_cache?.metrics || {};
    const avgComputedAt = clientDB.kpi_avg_cache?.computed_at || null;

    const targetsGlobal = {}, targetsMonthly = {};
    (targetsRows || []).forEach(r => {
      if (r.month === null) targetsGlobal[r.metric_key] = r.target_value;
      else if (r.month === targetPeriod) targetsMonthly[r.metric_key] = r.target_value;
    });

    const profile = clientDB.report_profile || 'ecommerce';
    const metrics = KPI_METRIC_DEFS
      .filter(def => !def.profiles || def.profiles.includes(profile))
      .map(def => ({
      key: def.key, group: def.group, group_label: def.group_label, format: def.format, custom: !!def.custom,
      label: def.custom ? (customLabels[def.key] || null) : def.label,
      actual: avg12mo[def.key]?.value ?? null,
      actual_months_used: avg12mo[def.key]?.months_used ?? 0,
      target_global: targetsGlobal[def.key] ?? null,
      target_monthly: targetsMonthly[def.key] ?? null,
    }));

    res.json({ period: targetPeriod, client_type: clientDB.client_type || null, metrics, avg_computed_at: avgComputedAt });
  } catch (e) {
    console.error(`[kpi-targets] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});

// Recalcula el promedio de 12 meses en vivo (hasta 12x llamadas a las APIs)
// y lo guarda en caché — se dispara solo con el botón "Actualizar
// promedios", nunca automático al abrir la pestaña.
app.post('/api/:slug/kpi-targets/refresh-averages', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: labelsRows } = await supabase.from('client_kpi_custom_labels').select('slot_key, label').eq('client_id', clientDB.id);
    const customLabels = {};
    (labelsRows || []).forEach(r => { customLabels[r.slot_key] = r.label; });

    const avg12mo = await computeKpiActuals12moAverage(slug, clientDB.id, customLabels);
    const computedAt = new Date().toISOString();
    const { error } = await supabase.from('clients').update({ kpi_avg_cache: { computed_at: computedAt, metrics: avg12mo } }).eq('id', clientDB.id);
    if (error) throw new Error(error.message);
    res.json({ ok: true, computed_at: computedAt });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/:slug/kpi-targets', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, metric_key, month, target_value } = req.body; // month: null (global) o 'YYYY-MM'
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!metric_key) return res.status(400).json({ error: 'Falta metric_key' });
  try {
    const { error } = await supabase.from('client_kpi_targets')
      .upsert({ client_id: clientDB.id, metric_key, month: month || null, target_value: target_value === '' ? null : target_value, updated_at: new Date().toISOString() },
        { onConflict: 'client_id,metric_key,month' });
    if (error) throw new Error(error.message);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
function flattenKpiActuals(actuals, customActuals) {
  const flat = {};
  KPI_METRIC_DEFS.forEach(def => {
    flat[def.key] = def.custom ? (customActuals[def.key] ?? null) : (def.accessor(actuals) ?? null);
  });
  return flat;
}

// Actual de un mes puntual para el histórico anual — un mes ya cerrado no
// cambia nunca, así que se cachea la primera vez que se pide (tabla
// client_kpi_monthly_actuals) y no se vuelve a calcular. El mes en curso
// nunca se cachea, porque todavía se sigue moviendo.
async function getOrComputeMonthlyActuals(slug, clientId, period, customLabels) {
  const isCurrentMonth = period === fmtDate(new Date()).slice(0, 7);
  if (!isCurrentMonth) {
    const { data: cached } = await supabase.from('client_kpi_monthly_actuals').select('metrics').eq('client_id', clientId).eq('period', period).limit(1);
    if (cached?.length) return cached[0].metrics;
  }
  const { start, end } = periodToMonthRange(period);
  const [actuals, customActuals] = await Promise.all([
    computeMonthlyKpiActuals(slug, clientId, period),
  ...
  const flat = flattenKpiActuals(actuals, customActuals);
  if (!isCurrentMonth) {
    await supabase.from('client_kpi_monthly_actuals')
      .upsert({ client_id: clientId, period, metrics: flat, computed_at: new Date().toISOString() }, { onConflict: 'client_id,period' });
  }
  return flat;
}

// ── Resumen anual de KPI — "de una sola mirada" a pedido del usuario.
// Liviano a propósito: solo lee lo ya cargado en client_kpi_targets (sin
// ninguna llamada en vivo a Meta/Google/Analytics), para que abrir esta
// caja sea instantáneo. Devuelve, por cada métrica que tenga al menos un
// mes cargado, el valor mensual de cada mes del año pedido.
app.get('/api/:slug/kpi-targets/yearly-summary', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, year } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const targetYear = year || fmtDate(new Date()).slice(0, 4);
    const [{ data: targetsRows }, { data: labelsRows }] = await Promise.all([
      supabase.from('client_kpi_targets').select('metric_key, month, target_value').eq('client_id', clientDB.id).like('month', `${targetYear}-%`),
      supabase.from('client_kpi_custom_labels').select('slot_key, label').eq('client_id', clientDB.id),
    ]);
    const customLabels = {};
    (labelsRows || []).forEach(r => { customLabels[r.slot_key] = r.label; });
    ...
    (targetsRows || []).forEach(r => {
      if (r.month === null) return; // acá solo interesan los mensuales, el Global vive en la otra caja
      (byMetric[r.metric_key] = byMetric[r.metric_key] || {})[r.month] = r.target_value;
      monthsNeeded.add(r.month);
    });

    // Actual real de cada mes que tiene al menos un target cargado —
    // cacheado por mes cerrado, en vivo solo para el mes en curso.
    const actualsPerMonth = {};
    for (const period of monthsNeeded) {
      actualsPerMonth[period] = await getOrComputeMonthlyActuals(slug, clientDB.id, period, customLabels);
    }

    const profile = clientDB.report_profile || 'ecommerce';
    const metrics = KPI_METRIC_DEFS
      .filter(def => !def.profiles || def.profiles.includes(profile))
      .filter(def => byMetric[def.key]) // solo métricas con al menos un mes cargado — "según lo cargado"
      .map(def => {
        const months = {};
        Object.entries(byMetric[def.key]).forEach(([month, target]) => {
          const actual = actualsPerMonth[month]?.[def.key] ?? null;
          let pct = null, met = null;
          if (target != null && Number(target) !== 0 && actual != null) {
            pct = Math.round((actual / target) * 100);
            met = def.lower_is_better ? actual <= target : actual >= target;
          }
          months[month] = { target, actual, pct, met };
        });
        return {
          key: def.key, format: def.format,
          label: def.custom ? (customLabels[def.key] || def.key) : def.label,
          months,
        };
      });

    res.json({ year: targetYear, metrics });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/:slug/kpi-custom-labels', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, slot_key, label } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!slot_key) return res.status(400).json({ error: 'Falta slot_key' });
  try {
    const { error } = await supabase.from('client_kpi_custom_labels')
      .upsert({ client_id: clientDB.id, slot_key, label: (label || '').trim(), updated_at: new Date().toISOString() },
        { onConflict: 'client_id,slot_key' });
    if (error) throw new Error(error.message);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// ── Resuelve qué objetivos usar para el Informe: toma el valor MENSUAL de
// client_kpi_targets del mes calendario en que cae el ÚLTIMO día del rango
// del informe, con fallback al valor GLOBAL de la misma tabla, y por último
// al KPI global viejo (client.kpi_targets) campo por campo — así ningún
// cliente pierde su objetivo cargado antes del rediseño. Traduce metric_key
// de la estructura nueva a los nombres de campo que ya usa el motor de
// hechos (roas_min_meta, cpa_max, etc.) — transversalidad: un mismo target
// cargado una vez en "KPI y Objetivos" alimenta el panel Y el reporte. ──
async function resolveMonthlyKpiTargets(clientId, legacyTargets, range) {
  ...
  try {
    ...
    const { data } = await supabase.from('client_kpi_targets').select('metric_key, month, target_value').eq('client_id', clientId);
    const global = {}, monthly = {};
    (data || []).forEach(r => { if (r.month === null) global[r.metric_key] = r.target_value; else if (r.month === period) monthly[r.metric_key] = r.target_value; });
    const val = key => monthly[key] ?? global[key] ?? null;
    return {
      ...legacy,
      roas_min_meta: val('meta_roas') ?? legacy.roas_min_meta,
      roas_min_google: val('google_roas') ?? legacy.roas_min_google,
      roas_min_combinado: val('roas_global') ?? legacy.roas_min_combinado,
      roas_min: val('roas_global') ?? legacy.roas_min,
      tasa_conversion_min: val('tasa_conversion_ecommerce') ?? legacy.tasa_conversion_min,
      cpa_max: val('meta_cpa') ?? val('google_cpa') ?? legacy.cpa_max,
      // costo_lead_max: sin equivalente todavía — se mantiene del KPI global.
    };
  } catch (e) {
...
const FRIO_ANCHOR_KEYS = [
  'ecommerce_resumen',
  'analytics_sitio', 'analytics_dispositivo_demografia', 'analytics_canales', 'analytics_eventos_clave', 'analytics_funnel_ecommerce', 'analytics_productos',
  'meta_campanas_leads', 'meta_campanas_ecommerce', 'meta_rendimiento', 'meta_mensajeria', 'meta_engagement', 'meta_video',
  'google_rendimiento', 'google_impression_share', 'google_campanas_leads', 'google_campanas_ecommerce', 'google_keywords',
  'search_console_resumen',
];
...
// ── Clasificación eCommerce/Leads — reporte en frío, se toma tal cual lo
// que la propia API reporta, sin inventar reglas de negocio:
// Meta ya trae el campo real "objective" por campaña (OUTCOME_SALES =
// eCommerce, cualquier otro valor = Leads/otro).
// Google no expone un campo "objetivo" equivalente — se usa la señal más
// fiel disponible: si la campaña trackea valor de conversión (compras)
// en el período, se cuenta como eCommerce.
function classifyMetaCampaign(objective) {
  return objective === 'OUTCOME_SALES' ? 'ecommerce' : 'leads';
}
function classifyGoogleCampaign(row) {
  return (row.conversion_value || 0) > 0 ? 'ecommerce' : 'leads';
}
...
async function computeReportFrioData(client, clientId, slug, periodType) {
  ...
  if (metaCreds?.access_token && metaCreds?.ad_account_id) {
    ...
    metaCampaigns = rawRows.map(cm => {
      ...
      return { id: cm.campaign_id, name: cm.campaign_name, status: info.status || null, objective: info.objective || null, segment: classifyMetaCampaign(info.objective), ...k };
    ...
    metaTotals = computeTotals(metaCampaigns);
    metaTotals.leads = computeTotals(metaCampaigns.filter(c => c.segment === 'leads'));
    metaTotals.ecommerce = computeTotals(metaCampaigns.filter(c => c.segment === 'ecommerce'));
    metaTotals.previous = await aggregateChannelPeriod(slug, 'meta', range.prevStart, range.prevEnd).catch(() => null);
  }
  ...
  if (customerId) {
    const [rawRows, products, kw, convActions] = await Promise.all([
      ...
      fetchTopKeywords(customerId, range.start, range.end, 50).catch(e => { console.warn('[frio] keywords omitido:', e.message); return []; }),
    ...
    googleTotals = agg;
    googleTotals.previous = await aggregateChannelPeriod(slug, 'google_ads', range.prevStart, range.prevEnd).catch(() => null);
    // Solo campañas con al menos 1 impresión en el período, como se pidió.
    console.log(`[frio][diag] agg.campaigns crudo: ${(agg.campaigns || []).length} — nombres: ${(agg.campaigns || []).map(c => c.campaign_name).join(' | ')}`);
    googleCampaigns = (agg.campaigns || []).filter(c => (c.impressions || 0) >= 1).map(c => ({ ...c, segment: classifyGoogleCampaign(c) }));
    console.log(`[frio][diag] googleCampaigns tras filtro impresiones>=1: ${googleCampaigns.length} — leads: ${googleCampaigns.filter(c=>c.segment==='leads').length}, ecommerce: ${googleCampaigns.filter(c=>c.segment==='ecommerce').length}`);
  ...
  let analytics = null;
  if (propertyId) {
    const [channels, siteMetrics, topPages, devices, demographics, funnel, topItems, keyEvents, keyEventsPrev, browsers, operatingSystems, topSite, channelsByUsers, sourceMedium, dailySeries, annualUsers, annualChannels, cities] = await Promise.all([
      ...
      fetchGA4TopItemsDetailed(propertyId, range.start, range.end, 15).catch(e => { console.warn('[frio] GA4 productos omitido:', e.message); return []; }),
      ...
      fetchGA4KeyEventsSummary(propertyId, range.prevStart, range.prevEnd).catch(() => []),
      fetchGA4BrowserBreakdown(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 navegador omitido:', e.message); return []; }),
      fetchGA4OSBreakdown(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 sistema operativo omitido:', e.message); return []; }),
      fetchGA4TopSiteSummary(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 top site omitido:', e.message); return null; }),
      fetchGA4ChannelsByUsers(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 canales por usuarios omitido:', e.message); return []; }),
      fetchGA4SourceMedium(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 fuente/medio omitido:', e.message); return []; }),
      fetchGA4DailySeries(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 serie diaria omitida:', e.message); return []; }),
      fetchGA4AnnualUsersByMonth(propertyId).catch(e => { console.warn('[frio] GA4 usuarios anual omitido:', e.message); return []; }),
      fetchGA4AnnualChannels(propertyId).catch(e => { console.warn('[frio] GA4 canales anual omitido:', e.message); return []; }),
      fetchGA4CityBreakdown(propertyId, range.start, range.end).catch(e => { console.warn('[frio] GA4 ciudades omitido:', e.message); return []; }),
    ]);
    analytics = {
      channels, site_metrics: siteMetrics, top_pages: topPages, devices, demographics, funnel, top_items: topItems, key_events: keyEvents,
      key_events_previous: keyEventsPrev,
      browsers, operating_systems: operatingSystems, top_site: topSite, channels_by_users: channelsByUsers, source_medium: sourceMedium, daily_series: dailySeries,
      annual_users_by_month: annualUsers, annual_channels: annualChannels, cities,
    };
  }

  // ── Search Console — resumen (todas las métricas que trae el conector) ──
  const scProperty = await getSearchConsoleProperty(slug).catch(() => null);
  let searchConsole = null, searchConsoleQueries = null;
  if (scProperty) {
    const [scCurrent, scPrevious, scQueries] = await Promise.all([
      fetchSearchConsoleSummary(scProperty, range.start, range.end).catch(e => { console.warn('[frio] Search Console omitido:', e.message); return null; }),
      fetchSearchConsoleSummary(scProperty, range.prevStart, range.prevEnd).catch(() => null),
      fetchTopQueries(scProperty, range.start, range.end, 15).catch(e => { console.warn('[frio] Search Console keywords omitido:', e.message); return []; }),
    ]);
    searchConsole = scCurrent ? { ...scCurrent, previous: scPrevious } : null;
    searchConsoleQueries = scQueries && scQueries.length ? scQueries : null;
  }
  ...
  return {
    range, meta: metaTotals, meta_campaigns: metaCampaigns, google: googleTotals, google_campaigns: googleCampaigns,
    pmax_products: pmaxProducts, keywords, google_objetivos: conversionActions, analytics, ecommerce, search_console: searchConsole,
    search_console_queries: searchConsoleQueries,
  };
}

// ── Plan de Optimización — IA para el Informe Frío. Única excepción a la
// regla "Frío = sin IA" (decisión explícita del usuario). Liviana y propia
// del Frío: no reusa runReportAgent (que arma headline/dek/stories sobre
// comparaciones de período), solo pide el mismo formato de "actions" a
// partir de los números YA calculados por computeReportFrioData. ──
function buildFrioOptimizationSummary(reportData) {
  const lines = [];
  const m = reportData.meta, g = reportData.google, eco = reportData.ecommerce, an = reportData.analytics;
  if (m && m.spend) {
    lines.push(`META ADS — Gasto $${Math.round(m.spend).toLocaleString('es-AR')}, ROAS ${m.roas ?? 'N/D'}, CPA $${m.cpa ?? 'N/D'}, Compras ${m.purchases||0}, Leads ${m.leads||0}, Mensajes ${m.messagingConn||0} (Depth2 ${m.msgDepth2||0} / Depth3 ${m.msgDepth3||0} / Depth5 ${m.msgDepth5||0}), CTR ${m.ctr??'N/D'}%, Frecuencia ${m.frequency??'N/D'}.`);
  }
  if (g && g.spend) {
    lines.push(`GOOGLE ADS — Gasto $${Math.round(g.spend).toLocaleString('es-AR')}, ROAS ${g.roas ?? 'N/D'}, CPA ${g.cpa ?? 'N/D'}, Conversiones ${g.conversions||0}, CTR ${g.ctr??'N/D'}%.`);
  }
  if (eco && eco.facturacion != null) {
    lines.push(`ECOMMERCE — Facturación $${Math.round(eco.facturacion).toLocaleString('es-AR')}, Pedidos ${eco.pedidos||0}, Tasa conversión ${eco.tasa_conversion_ecommerce ?? 'N/D'}%.`);
  }
  if (an && an.top_site?.current) {
    const ts = an.top_site.current;
    lines.push(`ANALYTICS — Vistas ${ts.views||0}, Usuarios ${ts.total_users||0}, Tasa de rebote ${ts.bounce_rate??'N/D'}%.`);
  }
  return lines.join('\n');
}

async function runFrioOptimizationAgent(client, reportData) {
  const summary = buildFrioOptimizationSummary(reportData);
  if (!summary) {
    return [{ titulo: 'Sin datos suficientes para recomendar acciones', motivo: 'No hay inversión ni actividad registrada en el período elegido para este cliente.', impacto: '', orientacion: 'general' }];
  }
  const md = client.mdContent || `# Cliente ${client.name}\n**Rubro**: ${client.rubro||''}`;
  const system = `${md}\n\n---\n\nSos un estratega senior de marketing digital. Con los datos reales de abajo (y SOLO esos datos, nunca inventes cifras ni canales que no aparecen), armá entre 3 y 5 acciones concretas para escalar los resultados del cliente en el próximo período. Cada acción debe ser accionable, específica al dato real que la motiva, y sin promesas de resultado garantizado. Si un canal no tiene datos, no lo menciones.\n\nDATOS DEL PERÍODO:\n${summary}\n\nRespondé SOLO con JSON válido (sin markdown, sin backticks):\n{"actions":[{"titulo":"...","motivo":"...","impacto":"","orientacion":"ecommerce|leads|general"}]}`;

  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + OPENAI_KEY },
    body: JSON.stringify({ model: 'gpt-4o', max_tokens: 1200, messages: [{ role: 'user', content: system }] })
  });
  const d = await r.json();
  if (!r.ok || d.error) throw new Error('Agente de optimización (OpenAI): ' + (d.error?.message || r.status));
  logAiCall({ slug: client.slug, model: 'gpt-4o', callType: 'report_frio_optimization', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 }).catch(() => {});
  const parsed = JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
  return Array.isArray(parsed.actions) && parsed.actions.length ? parsed.actions : [];
}


async function generateClientReportFrio(slug, periodType, options = {}) {
  const { anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url, include_optimization } = options;
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) throw new Error('Cliente no encontrado: ' + slug);
  ...
  const { data: agencyRow } = await supabase.from('agency_settings').select('logo_url').eq('id', 1).single();
  const { data: clientRow } = await supabase.from('clients').select('logo_url, color_primary').eq('id', clientDB.id).single();
  ...
  const reportData = {
    ...
    analytics: data.analytics, ecommerce: data.ecommerce, search_console: data.search_console,
    search_console_queries: data.search_console_queries,
    agency_logo_url: agencyRow?.logo_url || null, client_logo_url: clientRow?.logo_url || null,
    color_primary: clientRow?.color_primary || null,
    data_retrieved_at: new Date().toISOString(),
  };

  // ── Plan de Optimización (IA) — se genera acá mismo, al momento de crear
  // el informe, SOLO si el operador tildó "Incluir Plan de Optimización" en
  // el dashboard. El cliente nunca ve un botón de IA en el reporte público:
  // o ya viene armado, o directamente no aparece la sección. ──
  let optimizationActions = null;
  if (include_optimization) {
    try { optimizationActions = await runFrioOptimizationAgent(client, reportData); }
    catch (e) { console.warn('[frio] Plan de Optimización omitido:', e.message); }
  }
  ...
  const { data: row, error } = await supabase.from('client_reports_frio').insert({
    ...
    report_title: (report_title && report_title.trim()) || null,
    footer_text: (footer_text && footer_text.trim()) || null,
    report_data: reportData, optimization_actions: optimizationActions, expires_at: expiresAt.toISOString(),
  }).select().single();
  if (error) throw new Error(error.message);
  return row;
}
...
app.post('/api/:slug/report-frio/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type, anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url, include_optimization } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['mes_anterior', 'mes_actual', 'ultimos_90_dias'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const report = await generateClientReportFrio(slug, period_type, { anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url, include_optimization });
...
// ── Solapa "Reporte" (self-service) — el cliente elige SOLO la fecha; el
// resto (título, logo, secciones, cierre) sale de report_frio_config, la
// configuración que la agencia dejó guardada una vez desde /admin. Nunca
// pisa ni duplica "Informe" (esa pantalla completa sigue intacta y usa el
// endpoint de arriba directamente, sin pasar por acá). ──
app.post('/api/:slug/report/generate-self-service', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['mes_anterior', 'mes_actual', 'ultimos_90_dias'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const clientDB = await loadClientDB(slug);
    const config = clientDB?.report_frio_config;
    if (!config) return res.status(400).json({ error: 'Todavía no configuraron tu reporte. Pedile a la agencia que lo configure desde el panel de administración.' });
    const report = await generateClientReportFrio(slug, period_type, config);
    res.json({ token: report.token, access_code: report.access_code, url: `/rf/${report.token}`, expires_at: report.expires_at });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ══════════════════════════════════════════════════════════════════
// ── NEXUS REPORT — workspace de análisis sección por sección con IA,
// editable, con checkbox para incluir en el informe final. Reusa toda
// la lógica de datos de computeReportFrioData (misma fuente de verdad
// que el Informe Frío) — no duplica ninguna consulta a las APIs.
// ══════════════════════════════════════════════════════════════════
const NEXUS_SECTIONS = {
  business_performance: { title: 'Business Performance', extract: d => ({ ecommerce: d.ecommerce, meta_spend: d.meta?.spend, google_spend: d.google?.spend }) },
  acquisition: { title: 'Acquisition', extract: d => ({ channels: d.analytics?.channels_by_users, top_site: d.analytics?.top_site }) },
  ecommerce_funnel: { title: 'eCommerce Funnel', extract: d => ({ funnel: d.analytics?.funnel }) },
  product_intelligence: { title: 'Product Intelligence', extract: d => ({ top_items: d.analytics?.top_items }) },
  meta_ads: { title: 'Meta Ads', extract: d => ({ meta: d.meta, campaigns: (d.meta_campaigns || []).slice(0, 15) }) },
  google_ads: { title: 'Google Ads', extract: d => ({ google: d.google, campaigns: (d.google_campaigns || []).slice(0, 15), objetivos: d.google_objetivos }) },
  paid_media_business_view: { title: 'Paid Media Business View', extract: d => ({ meta: d.meta, google: d.google }) },
  seo: { title: 'SEO', extract: d => ({ search_console: d.search_console, queries: (d.search_console_queries || []).slice(0, 15) }) },
  customer_behavior: { title: 'Customer Behavior', extract: d => ({ devices: d.analytics?.devices, demographics: d.analytics?.demographics, cities: d.analytics?.cities }) },
};
...
app.post('/api/report-frio/:token/unlock', async (req, res) => {
  ...
  try {
    const { data, error } = await supabase
      .from('client_reports_frio')
      .select('*, clients(name, website_url, color_primary, logo_url)')
      .eq('token', req.params.token).limit(1);
    if (error || !data || !data.length) return res.status(404).json({ error: 'Informe no encontrado' });
    const row = data[0];
    ...
    res.json({
      ...row.report_data,
      report_title: row.report_title || row.report_data?.report_title || null,
      footer_text: row.footer_text || row.report_data?.footer_text || null,
      business_name: row.clients?.name || '', business_url: row.clients?.website_url || null,
      color_primary: row.clients?.color_primary || null, expires_at: row.expires_at,
      client_logo_url: row.clients?.logo_url || row.report_data?.client_logo_url || null,
      agency_logo_url: agencyRow?.logo_url || row.report_data?.agency_logo_url || null,
      optimization_actions: row.optimization_actions || null,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// ── Plan de Optimización (IA) — botón propio, nunca automático. Se genera
// una sola vez por informe y queda persistido en optimization_actions; si
// ya existe, se devuelve directo sin volver a llamar a la IA. ──
app.post('/api/report-frio/:token/optimize', async (req, res) => {
  const { code } = req.body;
  try {
    const { data, error } = await supabase
      .from('client_reports_frio')
    ...
    if (row.optimization_actions) return res.json({ actions: row.optimization_actions });

    const client = await loadClient(row.clients?.slug);
    if (!client) return res.status(404).json({ error: 'Cliente no encontrado' });

    const actions = await runFrioOptimizationAgent(client, row.report_data);
    await supabase.from('client_reports_frio').update({ optimization_actions: actions }).eq('token', row.token);
    res.json({ actions });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Trae las campañas reales de un canal — ESTRUCTURA, no rendimiento. A
// propósito no depende de que la campaña haya gastado algo en ningún rango
// de fechas: una campaña recién creada, sin historial todavía, tiene que
// poder matchear igual. (Antes esto se armaba con datos de performance de
// los últimos 90 días, y una campaña sin gasto en esa ventana quedaba
// invisible para el matching — bug real, corregido acá.)
// Moneda real de la cuenta por canal — Opción A del diseño de monedas
// múltiples: NUNCA se convierte/mezcla, cada canal se muestra en la suya,
// traída directo de la API (nunca cargada a mano ni asumida). Se cachea en
// memoria por proceso (dura hasta el próximo deploy) porque la moneda de
// una cuenta prácticamente no cambia nunca.
const accountCurrencyCache = new Map();
async function getAccountCurrency(slug, channel) {
  const cacheKey = `${slug}:${channel}`;
  if (accountCurrencyCache.has(cacheKey)) return accountCurrencyCache.get(cacheKey);
  let currency = null;
  try {
    if (channel === 'meta') {
      const creds = await getMetaCredentials(slug);
      if (creds?.access_token && creds?.ad_account_id) {
        const data = await metaFetch(`/${creds.ad_account_id}?fields=currency&`, creds.access_token);
        currency = data.currency || null;
      }
    } else if (channel === 'google_ads') {
      const customerId = await getGoogleAdsAccountId(slug);
      if (customerId) currency = await fetchGoogleAdsAccountCurrency(customerId);
    }
  } catch (e) { console.warn('[plan] getAccountCurrency omitido:', e.message); }
...
async function fetchLiveCampaignsForChannel(slug, channel) {
  if (channel === 'meta') {
    ...
    const data = await metaFetch(`/${creds.ad_account_id}/campaigns?fields=id,name,effective_status&limit=200&`, creds.access_token).catch(() => ({ data: [] }));
    return (data.data || []).filter(c => c.effective_status !== 'DELETED').map(c => ({ id: String(c.id), name: c.name || '(sin nombre)', raw_status: c.effective_status || null }));
  }
  if (channel === 'google_ads') {
    const customerId = await getGoogleAdsAccountId(slug);
    if (!customerId) return [];
    const rows = await fetchAllCampaignsStructure(customerId).catch(() => []);
    return rows.map(c => ({ id: c.id, name: c.name, raw_status: c.status || null }));
  }
  return [];
}

// Grupos de anuncios (ABO) — solo Meta. Se buscan DENTRO de una campaña ya
// vinculada (campaign_id real confirmado), nunca en toda la cuenta — mucho
// más preciso que el matching de campañas, que sí tiene que buscar en toda
// la cuenta porque todavía no sabe a qué campaña pertenece.
async function fetchLiveAdSetsForCampaign(slug, metaCampaignId) {
  const creds = await getMetaCredentials(slug);
  if (!creds?.access_token || !metaCampaignId) return [];
  const data = await metaFetch(`/${metaCampaignId}/adsets?fields=id,name,effective_status&limit=200&`, creds.access_token).catch(() => ({ data: [] }));
  return (data.data || []).filter(a => a.effective_status !== 'DELETED').map(a => ({ id: String(a.id), name: a.name || '(sin nombre)', raw_status: a.effective_status || null }));
}
async function matchAdSetByName(slug, metaCampaignId, adsetName) {
  const norm = normalizeCampaignName(adsetName);
  if (!norm || !metaCampaignId) return { adset_id: null, match_confidence: 'sin_match' };
  try {
    const live = await fetchLiveAdSetsForCampaign(slug, metaCampaignId);
    const exact = live.find(a => normalizeCampaignName(a.name) === norm);
    if (exact) return { adset_id: exact.id, match_confidence: 'exact_name', matched_name: exact.name, real_status: mapRealStatusToPlanStatus(exact.raw_status) };
    const fuzzy = live.find(a => { const n = normalizeCampaignName(a.name); return n.includes(norm) || norm.includes(n); });
    if (fuzzy) return { adset_id: fuzzy.id, match_confidence: 'fuzzy', matched_name: fuzzy.name, real_status: mapRealStatusToPlanStatus(fuzzy.raw_status) };
  } catch (e) { console.warn('[plan] matching de ad set omitido:', e.message); }
  return { adset_id: null, match_confidence: 'sin_match' };
}
// Igual política que buildMatchUpdate para campañas (Opción B): exacto se
// aplica solo, fuzzy queda pendiente de confirmación humana.
function buildAdSetMatchUpdate(adset, match) {
  const update = {};
  if (!match.adset_id) {
    if (adset.match_confidence !== 'sin_match') update.match_confidence = 'sin_match';
    return update;
  }
  if (match.match_confidence === 'exact_name') {
    if (match.adset_id !== adset.adset_id || adset.match_confidence !== 'exact_name') {
      update.adset_id = match.adset_id;
      update.match_confidence = 'exact_name';
    }
    if (match.real_status && match.real_status !== adset.status) update.status = match.real_status;
    if (adset.pending_adset_id) { update.pending_adset_id = null; update.pending_matched_name = null; update.pending_real_status = null; }
    return update;
  }
  if (adset.match_confidence !== 'fuzzy_pending' || adset.pending_adset_id !== match.adset_id) {
    update.match_confidence = 'fuzzy_pending';
    update.pending_adset_id = match.adset_id;
    update.pending_matched_name = match.matched_name || null;
    update.pending_real_status = match.real_status || null;
  }
  return update;
}
// Gasto real de un grupo de anuncios puntual, mismo patrón que
// fetchCampaignActuals pero a nivel adset (level=adset + filtro por id).
async function fetchAdSetActuals(slug, adsetId, since, until) {
  try {
    const creds = await getMetaCredentials(slug);
    if (!creds?.access_token || !adsetId) return null;
    const fields = 'spend,actions,action_values,adset_id';
    const timeRange = encodeURIComponent(JSON.stringify({ since, until }));
    const path = `/${adsetId}/insights?fields=${fields}&time_range=${timeRange}&level=adset&limit=100&`;
    const result = await metaFetch(path, creds.access_token).catch(() => ({ data: [] }));
    const row = (result.data || [])[0];
    if (!row) return null;
    const k = calcKpis(row);
    return { spend: k.spend || 0, conversions: k.purchases || k.leads || 0, revenue: k.revenue || 0 };
  } catch (e) {
    console.warn('[plan] fetchAdSetActuals omitido:', e.message);
    return null;
  }
}

// Traduce el estado real de Meta/Google al vocabulario simple del plan —
// sin inventar categorías nuevas: lo que no encaja en Activa/Pausada/
// Eliminada queda tal cual vino, para no esconder algo raro (en revisión,
// rechazada, etc.).
function mapRealStatusToPlanStatus(rawStatus) {
  if (!rawStatus) return null;
  const s = String(rawStatus).toUpperCase();
  if (['ACTIVE', 'ENABLED'].includes(s)) return 'Activa';
  if (['PAUSED', 'CAMPAIGN_PAUSED', 'ADSET_PAUSED'].includes(s)) return 'Pausada';
  if (['DELETED', 'REMOVED', 'ARCHIVED'].includes(s)) return 'Eliminada';
  return rawStatus;
}

// Decide qué persistir según la confianza del match — Opción B: un match
// exacto por nombre se aplica solo (como siempre); un match fuzzy (nombre
// aproximado) NUNCA se aplica solo — queda guardado en los campos pending_*
// como sugerencia, y el operador lo confirma o rechaza a mano desde la
// tabla del mes. Solo ahí se copian pending_* a los campos reales.
function buildMatchUpdate(item, match) {
  const update = {};
  if (!match.campaign_id) {
    if (item.match_confidence !== 'sin_match') update.match_confidence = 'sin_match';
    return update;
  }
  if (match.match_confidence === 'exact_name') {
    if (match.campaign_id !== item.campaign_id || item.match_confidence !== 'exact_name') {
      update.campaign_id = match.campaign_id;
      update.match_confidence = 'exact_name';
    }
    if (match.real_status && match.real_status !== item.status) update.status = match.real_status;
    if (item.pending_campaign_id) { update.pending_campaign_id = null; update.pending_matched_name = null; update.pending_real_status = null; }
    return update;
  }
  // fuzzy — nunca se aplica solo, queda pendiente de confirmación humana
  if (item.match_confidence !== 'fuzzy_pending' || item.pending_campaign_id !== match.campaign_id) {
    update.match_confidence = 'fuzzy_pending';
    update.pending_campaign_id = match.campaign_id;
    update.pending_matched_name = match.matched_name || null;
    update.pending_real_status = match.real_status || null;
  }
  return update;
...
async function matchCampaignByName(slug, channel, campaignName) {
  ...
  try {
    ...
    if (exact) return { campaign_id: exact.id, match_confidence: 'exact_name', matched_name: exact.name, real_status: mapRealStatusToPlanStatus(exact.raw_status) };
    const fuzzy = live.find(c => { const n = normalizeCampaignName(c.name); return n.includes(norm) || norm.includes(n); });
    if (fuzzy) return { campaign_id: fuzzy.id, match_confidence: 'fuzzy', matched_name: fuzzy.name, real_status: mapRealStatusToPlanStatus(fuzzy.raw_status) };
...
// Lista los meses que ya tienen al menos un plan cargado — para armar las
// solapas (Master + un tab por mes). Trae, por mes, un semáforo resumen
// (el peor de completitud/nomenclatura/ciclo de la versión más reciente)
// para el puntito de alerta en cada tab, sin tener que abrir cada uno.
app.get('/api/:slug/campaign-plans/months', async (req, res) => {
  const slug = req.params.slug;
  const { password } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    const { data: periods } = await supabase.from('campaign_plan_periods').select('id, period, version_number, status').eq('client_id', clientDB.id).order('period', { ascending: true }).order('version_number', { ascending: false });
    const latestByMonth = {};
    (periods || []).forEach(p => { if (!latestByMonth[p.period]) latestByMonth[p.period] = p; }); // ya viene ordenado por version desc

    const currentPeriod = fmtDate(new Date()).slice(0, 7);
    const months = await Promise.all(Object.values(latestByMonth).map(async p => {
      const { data: items } = await supabase.from('campaign_plan_items').select('match_confidence, budget_mensual, start_date, end_date, kpi_1, campaign_name').eq('plan_period_id', p.id);
      let semaforo = 'verde';
      const hasIssue = (items || []).some(it => it.match_confidence === 'sin_match' || !it.match_confidence || !it.budget_mensual || !it.start_date || !it.kpi_1);
      if (!items?.length || hasIssue) semaforo = 'amarillo';
      if (p.period === currentPeriod && p.status === 'DRAFT') semaforo = 'rojo';
      return { id: p.id, period: p.period, label: periodToLabel(p.period), status: p.status, semaforo, is_current: p.period === currentPeriod };
    }));
    res.json({ months });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.get('/api/:slug/campaign-plans', async (req, res) => {
  const slug = req.params.slug;
  const { password, period, version } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const targetPeriod = (period && /^\d{4}-\d{2}$/.test(period)) ? period : fmtDate(new Date()).slice(0, 7);
  try {
    const { data: allVersions } = await supabase.from('campaign_plan_periods').select('id, version_number, version_name, status, created_at').eq('client_id', clientDB.id).eq('period', targetPeriod).order('version_number', { ascending: false });
    if (!allVersions?.length) return res.json({ period: targetPeriod, exists: false, plan: null, items: [], versions: [] });

    const targetVersion = version ? Number(version) : allVersions[0].version_number; // default: la más reciente
    const { data: periodRow } = await supabase.from('campaign_plan_periods').select('*').eq('client_id', clientDB.id).eq('period', targetPeriod).eq('version_number', targetVersion).limit(1);
    const plan = periodRow?.[0];
    if (!plan) return res.json({ period: targetPeriod, exists: false, plan: null, items: [], versions: allVersions });
    ...
    let totalPlanned = 0, totalPlannedDaily = 0, totalActual = 0, anyActual = false;
    const byChannelPlanned = {};
    const enriched = await Promise.all((items || []).map(async item => {
      let effectiveDiario = item.budget_diario, effectiveMensual = item.budget_mensual;
      let adsets = null;
      // Grupos de anuncios (solo Meta). En modo ABO, el Budget Diario/Mensual
      // de la campaña deja de ser el valor guardado y pasa a ser la SUMA de
      // sus grupos — nunca se confía en un valor manual desactualizado. En
      // modo CBO, la campaña puede igual tener grupos definidos (para
      // reporting/matching) pero el presupuesto sigue siendo el cargado a
      // nivel campaña — Meta reparte solo entre esos grupos.
      const { data: rawAdsets } = await supabase.from('campaign_plan_adsets').select('*').eq('plan_item_id', item.id).order('created_at', { ascending: true });
      if (rawAdsets?.length) {
        if (item.budget_mode === 'adset') {
          effectiveDiario = 0; effectiveMensual = 0;
          adsets = await Promise.all(rawAdsets.map(async a => {
            effectiveDiario += Number(a.budget_diario) || 0;
            effectiveMensual += Number(a.budget_mensual) || 0;
            if (!a.adset_id) return a;
            const aActual = await fetchAdSetActuals(slug, a.adset_id, range.start, range.end).catch(() => null);
            return { ...a, actual_spend: aActual?.spend, actual_conversions: aActual?.conversions, actual_revenue: aActual?.revenue };
          }));
        } else {
          adsets = rawAdsets;
        }
      }
      totalPlanned += Number(effectiveMensual) || 0;
      totalPlannedDaily += Number(effectiveDiario) || 0;
      if (item.channel) byChannelPlanned[item.channel] = (byChannelPlanned[item.channel] || 0) + (Number(effectiveMensual) || 0);
      if (!item.campaign_id || !item.channel) return { ...item, budget_diario: effectiveDiario, budget_mensual: effectiveMensual, adsets };
      const actual = await fetchCampaignActuals(slug, item.channel, item.campaign_id, range.start, range.end).catch(() => null);
      if (actual) { totalActual += actual.spend; anyActual = true; }
      const result = { ...item, budget_diario: effectiveDiario, budget_mensual: effectiveMensual, adsets, actual_spend: actual?.spend, actual_conversions: actual?.conversions, actual_revenue: actual?.revenue };
      if (isCurrentPeriod && actual && effectiveMensual && Number(effectiveMensual) > 0) {
        const gastadoPct = +(actual.spend / Number(effectiveMensual) * 100).toFixed(1);
    ...
    // ── Semáforo de completitud — chequeo determinístico de campos vacíos,
    // sin nada de IA/heurística: todo lo que falta se puede señalar con certeza.
    const completeness = { issues: [] };
    enriched.forEach(it => {
      if (it.match_confidence === 'sin_match' || !it.match_confidence) completeness.issues.push({ campaign_name: it.campaign_name, issue: 'Sin vincular a una campaña real' });
      if (!it.budget_mensual) completeness.issues.push({ campaign_name: it.campaign_name, issue: it.budget_mode === 'adset' ? 'Sin presupuesto cargado en ningún grupo de anuncios' : 'Sin presupuesto cargado' });
      if (!it.start_date || !it.end_date) completeness.issues.push({ campaign_name: it.campaign_name, issue: 'Sin fechas cargadas' });
      else if (it.end_date < it.start_date) completeness.issues.push({ campaign_name: it.campaign_name, issue: 'Fecha de fin anterior a la de inicio' });
      if (!it.kpi_1) completeness.issues.push({ campaign_name: it.campaign_name, issue: 'Sin ningún KPI cargado' });
      else if (it.kpi_1_target == null) completeness.issues.push({ campaign_name: it.campaign_name, issue: `Tiene KPI (${it.kpi_1}) pero sin Target cargado` });
    });
    completeness.semaforo = !items?.length ? 'rojo' : completeness.issues.length === 0 ? 'verde' : completeness.issues.length <= 2 ? 'amarillo' : 'rojo';

    // ── Semáforo de nomenclatura — HEURÍSTICA, no una regla exacta: compara
    // la cantidad de segmentos "|" de cada nombre contra la más frecuente en
    // este mismo plan, y marca las que se desvían. Puede haber falsos
    // positivos si el cliente mezcla convenciones a propósito.
    const nomenclature = { issues: [] };
    if (items?.length >= 2) {
      const segmentCounts = items.map(it => (it.campaign_name || '').split('|').length);
      const freq = {};
      segmentCounts.forEach(c => { freq[c] = (freq[c] || 0) + 1; });
      const modeCount = Number(Object.entries(freq).sort((a, b) => b[1] - a[1])[0][0]);
      items.forEach((it, i) => {
        if (segmentCounts[i] !== modeCount) nomenclature.issues.push({ campaign_name: it.campaign_name, issue: `Tiene ${segmentCounts[i]} segmentos separados por "|", el resto del plan usa ${modeCount}` });
      });
    }
    nomenclature.semaforo = nomenclature.issues.length === 0 ? 'verde' : nomenclature.issues.length <= 1 ? 'amarillo' : 'rojo';

    // ── Semáforo de "Estado del ciclo" — texto plano avisando plazos, sin
    // ningún cambio automático de estado (eso lo sigue haciendo el operador
    // a mano). Solo aplica al período actual, no tiene sentido para meses
    // pasados o futuros.
    const cycle = { issues: [], semaforo: 'verde' };
    if (isCurrentPeriod) {
      const daysLeft = totalDaysInMonth - dayOfMonth;
      if (plan.status === 'DRAFT') {
        cycle.issues.push('Este plan sigue en Borrador y el mes ya arrancó.');
        cycle.semaforo = 'rojo';
      }
      if (daysLeft <= 5) {
        ...
        const { data: nextPlan } = await supabase.from('campaign_plan_periods').select('id').eq('client_id', clientDB.id).eq('period', nextPeriod).limit(1);
        if (!nextPlan?.length) {
          cycle.issues.push(`Faltan ${daysLeft} días para fin de mes — todavía no hay plan para ${periodToLabel(nextPeriod)}.`);
          cycle.semaforo = cycle.semaforo === 'rojo' ? 'rojo' : 'amarillo';
        }
      }
    }

    // Franja de resumen del plan (Propuesta 1 — Sección 5/6): además de lo
    // Planeado vs. Real que ya existía, ahora se suma Presupuesto Mensual
    // objetivo del cliente (plan.total_budget, opcional — lo carga el
    // operador al crear el período), Disponible = Mensual - Asignado, %
    // Asignado, y el desglose por canal. Si el operador nunca cargó un
    // total_budget, esos 3 campos quedan null y el frontend los oculta —
    // nunca se inventa un objetivo que no existe.
    const totalBudgetTarget = plan.total_budget != null ? Number(plan.total_budget) : null;
    const budgetByChannel = plan.total_budget_by_channel || {};
    const byChannel = await Promise.all(Object.entries(byChannelPlanned).map(async ([channel, total]) => {
      const target = budgetByChannel[channel] != null ? Number(budgetByChannel[channel]) : null;
      return {
        channel, total, pct: totalPlanned > 0 ? +(total / totalPlanned * 100).toFixed(1) : 0,
        currency: await getAccountCurrency(slug, channel),
        budget_target: target,
        disponible: target != null ? +(target - total).toFixed(0) : null,
        pct_target: target != null && target > 0 ? +(total / target * 100).toFixed(1) : null,
      };
    }));
    const distinctCurrencies = new Set(byChannel.map(c => c.currency).filter(Boolean));
    res.json({
      period: targetPeriod, exists: true, is_current_period: isCurrentPeriod, plan, items: enriched,
      versions: allVersions, current_version: targetVersion,
      budget_summary: {
        total_budget: totalBudgetTarget,
        total_planned: totalPlanned, total_planned_daily: totalPlannedDaily,
        total_actual: anyActual ? totalActual : null, any_actual: anyActual,
        disponible: totalBudgetTarget != null ? +(totalBudgetTarget - totalPlanned).toFixed(0) : null,
        pct_asignado: totalBudgetTarget != null && totalBudgetTarget > 0 ? +(totalPlanned / totalBudgetTarget * 100).toFixed(1) : null,
        by_channel: byChannel,
        mixed_currencies: distinctCurrencies.size > 1,
        campaign_count: items?.length || 0,
      },
      completeness, nomenclature, cycle,
    });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
...
// Duplica el plan actual (todas sus campañas) en una versión nueva del mismo
// período — "Guardar plan del mes en forma manual": un checkpoint explícito
// que no depende de haber pasado por la propuesta de IA.
app.post('/api/:slug/campaign-plans/period/:periodId/new-version', async (req, res) => {
  const slug = req.params.slug;
  const { password, version_name } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: sourcePlan } = await supabase.from('campaign_plan_periods').select('*').eq('id', req.params.periodId).single();
    if (!sourcePlan) return res.status(404).json({ error: 'Período no encontrado' });
    const { data: maxVersionRow } = await supabase.from('campaign_plan_periods').select('version_number').eq('client_id', sourcePlan.client_id).eq('period', sourcePlan.period).order('version_number', { ascending: false }).limit(1).single();
    const nextVersion = (maxVersionRow?.version_number || sourcePlan.version_number) + 1;

    const { data: newPlan, error: planError } = await supabase.from('campaign_plan_periods').insert({
      client_id: sourcePlan.client_id, period: sourcePlan.period, status: 'DRAFT',
      version_number: nextVersion, version_name: version_name || `V${nextVersion}`,
      total_budget_by_channel: sourcePlan.total_budget_by_channel, business_objective: sourcePlan.business_objective,
      primary_kpi: sourcePlan.primary_kpi, target_value: sourcePlan.target_value, restrictions: sourcePlan.restrictions,
    }).select().single();
    if (planError) throw new Error(planError.message);

    const { data: sourceItems } = await supabase.from('campaign_plan_items').select('*').eq('plan_period_id', sourcePlan.id);
    if (sourceItems?.length) {
      const cloned = sourceItems.map(({ id, plan_period_id, created_at, updated_at, ...rest }) => ({ ...rest, plan_period_id: newPlan.id }));
      const { error: itemsError } = await supabase.from('campaign_plan_items').insert(cloned);
...
// Copia un plan de un mes existente hacia un mes destino (vacío o no) —
// Punto 5 del rediseño: nombre/canal/objective/KPIs/budget diario/patrón de
// días/notas se copian igual; start_date/end_date SIEMPRE se recalculan al
// mes completo del destino (nunca se arrastran las fechas del origen), y de
// ahí Días/Budget Mensual salen solos vía computeItemBudgetFields, como
// cualquier campaña nueva. campaign_id/match_confidence/Status arrancan en
// blanco — el vínculo con Meta/Google es cosa del mes destino, nunca se
// asume que sigue siendo válido solo porque el nombre no cambió.
app.post('/api/:slug/campaign-plans/period/:sourcePeriodId/copy-to', async (req, res) => {
  const slug = req.params.slug;
  const { password, target_period } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!target_period || !/^\d{4}-\d{2}$/.test(target_period)) return res.status(400).json({ error: 'Falta target_period en formato YYYY-MM' });
  try {
    const { data: sourcePlan } = await supabase.from('campaign_plan_periods').select('*').eq('id', req.params.sourcePeriodId).single();
    if (!sourcePlan) return res.status(404).json({ error: 'Plan de origen no encontrado' });
    const { data: sourceItems } = await supabase.from('campaign_plan_items').select('*').eq('plan_period_id', sourcePlan.id);
    if (!sourceItems?.length) return res.status(400).json({ error: 'El plan de origen no tiene campañas para copiar.' });

    // Si el destino ya tiene versiones, esto se guarda como una versión
    // nueva (mismo criterio que "new-version"); si es la primera vez, V1.
    const { data: existingVersions } = await supabase.from('campaign_plan_periods').select('version_number').eq('client_id', sourcePlan.client_id).eq('period', target_period).order('version_number', { ascending: false }).limit(1);
    const nextVersion = existingVersions?.length ? existingVersions[0].version_number + 1 : 1;

    const { data: newPlan, error: planError } = await supabase.from('campaign_plan_periods').insert({
      client_id: sourcePlan.client_id, period: target_period, status: 'DRAFT',
      version_number: nextVersion, version_name: `V${nextVersion} (copiado de ${periodToLabel(sourcePlan.period)})`,
      total_budget_by_channel: sourcePlan.total_budget_by_channel, business_objective: sourcePlan.business_objective,
      primary_kpi: sourcePlan.primary_kpi, target_value: sourcePlan.target_value, restrictions: sourcePlan.restrictions,
    }).select().single();
    if (planError) throw new Error(planError.message);

    const [ty, tm] = target_period.split('-').map(Number);
    const targetStart = `${target_period}-01`;
    const targetEnd = new Date(Date.UTC(ty, tm, 0)).toISOString().slice(0, 10); // último día real del mes destino

    const cloned = sourceItems.map(({ id, plan_period_id, created_at, updated_at, campaign_id, match_confidence, status, pending_campaign_id, pending_matched_name, pending_real_status, start_date, end_date, calculated_days, dias, budget_mensual, ...rest }) => {
      const base = { ...rest, plan_period_id: newPlan.id, start_date: targetStart, end_date: targetEnd, campaign_id: null, match_confidence: 'sin_match', status: null, pending_campaign_id: null, pending_matched_name: null, pending_real_status: null };
      return { computed: computeItemBudgetFields(base), _source_item_id: id };
    });
    const cleanedCloned = cloned.map(c => c.computed);
    const { data: insertedItems, error: itemsError } = await supabase.from('campaign_plan_items').insert(cleanedCloned).select();
    if (itemsError) throw new Error(itemsError.message);

    // Campañas en modo ABO también arrastran sus grupos de anuncios —
    // mismo criterio que la campaña: fechas al mes completo destino, sin
    // match/gasto real (arranca en blanco, igual que la campaña padre).
    // Correlación por ORDEN de inserción (no por plan_code, que puede
    // repetirse vacío entre campañas sin código propio).
    for (let i = 0; i < (insertedItems || []).length; i++) {
      const item = insertedItems[i];
      const sourceItemId = cloned[i]._source_item_id;
      const { data: sourceAdsets } = await supabase.from('campaign_plan_adsets').select('*').eq('plan_item_id', sourceItemId);
      if (!sourceAdsets?.length) continue;
      const clonedAdsets = sourceAdsets.map(a => {
        const base = { plan_item_id: item.id, adset_name: a.adset_name, start_date: targetStart, end_date: targetEnd, active_days_pattern: a.active_days_pattern || 'todos', budget_diario: a.budget_diario, match_confidence: 'sin_match' };
        const computed = computeItemBudgetFields(base);
        return { ...base, calculated_days: computed.calculated_days, dias: computed.dias, budget_mensual: computed.budget_mensual };
      });
      await supabase.from('campaign_plan_adsets').insert(clonedAdsets);
    }

    res.json({ ok: true, plan: newPlan, copied: cleanedCloned.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/campaign-plans/period', async (req, res) => {
  const slug = req.params.slug;
  const { password, period, total_budget_by_channel, business_objective, primary_kpi, target_value, restrictions } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  ...
  try {
    const { data, error } = await supabase.from('campaign_plan_periods').insert({
      client_id: clientDB.id, period, status: 'DRAFT', version_number: 1, version_name: 'V1',
      total_budget_by_channel: total_budget_by_channel || null, business_objective: business_objective || null,
...
// Editar metadata de un período (presupuesto por canal, objetivo, estado, etc.)
app.put('/api/:slug/campaign-plans/period/:periodId', async (req, res) => {
  const { password, total_budget_by_channel, business_objective, primary_kpi, target_value, restrictions, status } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const update = { updated_at: new Date().toISOString() };
    if (total_budget_by_channel !== undefined) update.total_budget_by_channel = total_budget_by_channel;
...
app.post('/api/:slug/campaign-plans/item', async (req, res) => {
  ...
  if (!channel) return res.status(400).json({ error: 'Falta elegir el Channel (Meta Ads / Google Ads) — sin esto el matching nunca puede funcionar.' });
  try {
...
app.put('/api/:slug/campaign-plans/item/:itemId', async (req, res) => {
  ...
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if ('channel' in fields && !fields.channel) return res.status(400).json({ error: 'Channel no puede quedar vacío — sin esto el matching nunca puede funcionar.' });
  const allowed = ['campaign_name', 'channel', 'campaign_id', 'match_confidence', 'campaign_role', 'budget_mode', 'objective', 'campaign_objective', 'budget_diario', 'dias', 'budget_mensual', 'status', 'kpi_1', 'kpi_2', 'kpi_3', 'notas', 'restricciones', 'start_date', 'end_date', 'plan_code', 'active_days_pattern', 'kpi_1_target', 'kpi_2_target', 'kpi_3_target', 'decision', 'decision_reason'];
  const update = { updated_at: new Date().toISOString() };
...
// Re-matchea TODAS las campañas de un período de una — "Actualizar estructura",
// para cuando se creó una campaña nueva del lado de Meta/Google y todavía no
// había forma de encontrarla (o el bug de performance-vs-estructura de más
// arriba dejaba alguna afuera).
app.post('/api/:slug/campaign-plans/period/:periodId/rematch-all', async (req, res) => {
  const slug = req.params.slug;
  const { password } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: items } = await supabase.from('campaign_plan_items').select('*').eq('plan_period_id', req.params.periodId);
    if (!items?.length) return res.json({ updated: 0 });
    let updated = 0, matched = 0, statusChanged = 0, pendingConfirmation = 0;
    for (const item of items) {
      const match = await matchCampaignByName(slug, item.channel, item.campaign_name);
      const update = buildMatchUpdate(item, match);
      if (Object.keys(update).length) {
        ...
        await supabase.from('campaign_plan_items').update(update).eq('id', item.id);
        updated++;
        if (update.status) statusChanged++;
        if (update.match_confidence === 'fuzzy_pending') pendingConfirmation++;
      }
      if (match.campaign_id) matched++;
    }
    res.json({ updated, matched, total: items.length, status_changed: statusChanged, pending_confirmation: pendingConfirmation });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Confirma un match "fuzzy" pendiente — copia pending_campaign_id/pending_real_status
// a los campos reales (campaign_id/status), y marca match_confidence como
// 'confirmed' (verde, distinto de 'exact_name' para saber que fue una
// confirmación manual del operador, no un match automático).
app.post('/api/:slug/campaign-plans/item/:itemId/confirm-match', async (req, res) => {
  const { password } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: item } = await supabase.from('campaign_plan_items').select('*').eq('id', req.params.itemId).single();
    if (!item) return res.status(404).json({ error: 'Campaña no encontrada' });
    if (!item.pending_campaign_id) return res.status(400).json({ error: 'Esta campaña no tiene un match pendiente de confirmar.' });
    const update = {
      campaign_id: item.pending_campaign_id, match_confidence: 'confirmed',
      pending_campaign_id: null, pending_matched_name: null, pending_real_status: null,
      updated_at: new Date().toISOString(),
    };
    if (item.pending_real_status) update.status = item.pending_real_status;
    const { data, error } = await supabase.from('campaign_plan_items').update(update).eq('id', req.params.itemId).select().single();
    if (error) throw new Error(error.message);
...
// Rechaza un match "fuzzy" pendiente — lo descarta, vuelve a 'sin_match'.
// El próximo "Actualizar match" (o el cron diario) puede volver a
// sugerirlo si sigue siendo el candidato más parecido; para vincular una
// campaña distinta a mano, el operador usa el botón 🔗 (vincular a mano).
app.post('/api/:slug/campaign-plans/item/:itemId/reject-match', async (req, res) => {
  const { password } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data, error } = await supabase.from('campaign_plan_items').update({
      match_confidence: 'sin_match', pending_campaign_id: null, pending_matched_name: null, pending_real_status: null,
      updated_at: new Date().toISOString(),
    }).eq('id', req.params.itemId).select().single();
    if (error) throw new Error(error.message);
    res.json({ ok: true, item: data });
...
// ── GRUPOS DE ANUNCIOS (ABO, solo Meta) — viven colgados de una campaña
// del plan que eligió budget_mode='adset'. Mismos principios que las
// campañas: Días/Budget Mensual nunca se tipean a mano, se calculan de
// fechas+patrón; match exacto se aplica solo, fuzzy queda pendiente.
app.post('/api/:slug/campaign-plans/item/:itemId/adsets', async (req, res) => {
  const { password, adset_name, start_date, end_date, active_days_pattern, budget_diario } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!adset_name) return res.status(400).json({ error: 'Falta el nombre del grupo de anuncios' });
  try {
    const { data: parent } = await supabase.from('campaign_plan_items').select('campaign_id, channel, budget_mode').eq('id', req.params.itemId).single();
    if (!parent) return res.status(404).json({ error: 'Campaña no encontrada' });
    if (parent.channel !== 'meta') return res.status(400).json({ error: 'Los grupos de anuncios solo aplican a campañas de Meta Ads' });
    const computed = computeItemBudgetFields({ start_date, end_date, active_days_pattern, budget_diario });
    let match = { adset_id: null, match_confidence: 'sin_match' };
    if (parent.campaign_id) match = await matchAdSetByName(req.params.slug, parent.campaign_id, adset_name);
    const { data, error } = await supabase.from('campaign_plan_adsets').insert({
      plan_item_id: req.params.itemId, adset_name,
      adset_id: match.match_confidence === 'exact_name' ? match.adset_id : null,
      match_confidence: match.match_confidence === 'exact_name' ? 'exact_name' : match.adset_id ? 'fuzzy_pending' : 'sin_match',
      pending_adset_id: match.match_confidence === 'fuzzy' ? match.adset_id : null,
      pending_matched_name: match.match_confidence === 'fuzzy' ? match.matched_name : null,
      pending_real_status: match.match_confidence === 'fuzzy' ? match.real_status : null,
      status: match.match_confidence === 'exact_name' ? match.real_status : null,
      start_date: start_date || null, end_date: end_date || null, active_days_pattern: active_days_pattern || null,
      budget_diario: budget_diario ?? null, calculated_days: computed.calculated_days ?? null,
      dias: computed.dias ?? null, budget_mensual: computed.budget_mensual ?? null,
    }).select().single();
    if (error) throw new Error(error.message);
    res.json({ adset: data });
  } catch (e) {
...
app.put('/api/:slug/campaign-plans/adset/:adsetId', async (req, res) => {
  const { password, ...fields } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const allowed = ['adset_name', 'start_date', 'end_date', 'active_days_pattern', 'budget_diario', 'status'];
  const update = { updated_at: new Date().toISOString() };
  allowed.forEach(k => { if (fields[k] !== undefined) update[k] = fields[k] === '' ? null : fields[k]; });
  if (update.start_date !== undefined || update.end_date !== undefined || update.active_days_pattern !== undefined || update.budget_diario !== undefined) {
    try {
      const { data: current } = await supabase.from('campaign_plan_adsets').select('start_date, end_date, active_days_pattern, budget_diario').eq('id', req.params.adsetId).single();
      const merged = { ...current, ...update };
      const computed = computeItemBudgetFields(merged);
      if (computed.calculated_days != null) { update.calculated_days = computed.calculated_days; update.dias = computed.dias; update.budget_mensual = computed.budget_mensual; }
    } catch (e) { /* si falla el recálculo, se guarda lo que vino tal cual */ }
  ...
  try {
    const { data, error } = await supabase.from('campaign_plan_adsets').update(update).eq('id', req.params.adsetId).select().single();
    if (error) throw new Error(error.message);
    res.json({ adset: data });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.delete('/api/:slug/campaign-plans/adset/:adsetId', async (req, res) => {
  const { password } = req.query;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { error } = await supabase.from('campaign_plan_adsets').delete().eq('id', req.params.adsetId);
    if (error) throw new Error(error.message);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Re-matchea todos los grupos de anuncios de una campaña (misma idea que
// rematch-all de campañas, a nivel adset).
app.post('/api/:slug/campaign-plans/item/:itemId/adsets/rematch-all', async (req, res) => {
  const { password } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: parent } = await supabase.from('campaign_plan_items').select('campaign_id, channel').eq('id', req.params.itemId).single();
    if (!parent) return res.status(404).json({ error: 'Campaña no encontrada' });
    if (!parent.campaign_id) return res.status(400).json({ error: 'Primero vinculá la campaña padre a una campaña real de Meta.' });
    const { data: adsets } = await supabase.from('campaign_plan_adsets').select('*').eq('plan_item_id', req.params.itemId);
    let updated = 0, matched = 0, pendingConfirmation = 0;
    for (const a of (adsets || [])) {
      const match = await matchAdSetByName(req.params.slug, parent.campaign_id, a.adset_name);
      const update = buildAdSetMatchUpdate(a, match);
      if (Object.keys(update).length) {
        update.updated_at = new Date().toISOString();
        await supabase.from('campaign_plan_adsets').update(update).eq('id', a.id);
        updated++;
        if (update.match_confidence === 'fuzzy_pending') pendingConfirmation++;
      }
      if (match.adset_id) matched++;
    }
    res.json({ updated, matched, total: adsets?.length || 0, pending_confirmation: pendingConfirmation });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
app.post('/api/:slug/campaign-plans/adset/:adsetId/confirm-match', async (req, res) => {
  const { password } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data: a } = await supabase.from('campaign_plan_adsets').select('*').eq('id', req.params.adsetId).single();
    if (!a) return res.status(404).json({ error: 'Grupo de anuncios no encontrado' });
    if (!a.pending_adset_id) return res.status(400).json({ error: 'No hay match pendiente de confirmar.' });
    const update = {
      adset_id: a.pending_adset_id, match_confidence: 'confirmed',
      pending_adset_id: null, pending_matched_name: null, pending_real_status: null,
      updated_at: new Date().toISOString(),
    };
    if (a.pending_real_status) update.status = a.pending_real_status;
    const { data, error } = await supabase.from('campaign_plan_adsets').update(update).eq('id', req.params.adsetId).select().single();
    if (error) throw new Error(error.message);
    res.json({ ok: true, adset: data });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
app.post('/api/:slug/campaign-plans/adset/:adsetId/reject-match', async (req, res) => {
  const { password } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data, error } = await supabase.from('campaign_plan_adsets').update({
      match_confidence: 'sin_match', pending_adset_id: null, pending_matched_name: null, pending_real_status: null,
      updated_at: new Date().toISOString(),
    }).eq('id', req.params.adsetId).select().single();
    if (error) throw new Error(error.message);
    res.json({ ok: true, adset: data });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Parsea números en formato argentino ("$10.000" = diez mil, no 10 —
// el punto es separador de miles, la coma es decimal si aparece). Number()
// nativo interpreta el punto como decimal y rompe esto ("10.000" → 10).
function parseArNumber(str) {
  if (str == null || str === '') return null;
  let s = String(str).trim().replace(/[^0-9.,-]/g, ''); // saca $, espacios, etc.
  if (!s) return null;
  if (s.includes(',')) {
    // Hay coma → es el decimal; cualquier punto antes es separador de miles.
    s = s.replace(/\./g, '').replace(',', '.');
  } else {
    // Sin coma → los puntos son separadores de miles (formato AR típico de
    // montos enteros), salvo que parezca genuinamente un decimal corto
    // (ej. "4.5"), donde el único punto tiene 1-2 dígitos después.
    const parts = s.split('.');
  ...
  try {
    ...
    for (const r of values.slice(1)) {
      ...
      rowsToUpsert.push({
        ...
        budget_diario_default: parseArNumber(row.budget_diario_default),
...
app.get('/api/:slug/campaign-plans/master', async (req, res) => {
  ...
  try {
    ...
    if (error) throw new Error(error.message);
    // Se embeben los grupos de anuncios plantilla de cada campaña Meta, así
    // la tabla del Master los puede listar sin un round-trip extra por fila.
    const metaIds = (data || []).filter(r => r.channel === 'meta').map(r => r.id);
    let adsetsByMaster = {};
    if (metaIds.length) {
      const { data: allAdsets } = await supabase.from('campaign_plan_master_adsets').select('*').in('master_id', metaIds).order('created_at', { ascending: true });
      (allAdsets || []).forEach(a => { (adsetsByMaster[a.master_id] ||= []).push(a); });
    }
    const enriched = (data || []).map(r => ({ ...r, adsets: adsetsByMaster[r.id] || [] }));
    res.json({ master: enriched });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/campaign-plans/master', async (req, res) => {
  const slug = req.params.slug;
  const { password, plan_code, campaign_name, channel, campaign_type, objective, campaign_objective, budget_diario_default, budget_mode_default, active_days_pattern_default, status_default, kpi_1, kpi_2, kpi_3, notas_default, restricciones_default } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  if (!campaign_name) return res.status(400).json({ error: 'Falta el nombre de la campaña' });
  try {
    const { data, error } = await supabase.from('campaign_plan_master').insert({
      client_id: clientDB.id, plan_code: plan_code || null, campaign_name, channel: channel || null, campaign_type: campaign_type || null,
      objective: objective || null, campaign_objective: campaign_objective || null,
      budget_diario_default: budget_diario_default ?? null, budget_mode_default: budget_mode_default || null, active_days_pattern_default: active_days_pattern_default || null,
...
app.put('/api/:slug/campaign-plans/master/:masterId', async (req, res) => {
  const { password, ...fields } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const allowed = ['plan_code', 'campaign_name', 'channel', 'campaign_type', 'objective', 'campaign_objective', 'budget_diario_default', 'budget_mode_default', 'active_days_pattern_default', 'status_default', 'kpi_1', 'kpi_2', 'kpi_3', 'notas_default', 'restricciones_default'];
  const update = { updated_at: new Date().toISOString() };
  allowed.forEach(k => { if (fields[k] !== undefined) update[k] = fields[k] === '' ? null : fields[k]; });
  try {
...
// ── Grupos de anuncios PLANTILLA del Master (solo Meta) — solo nombre +
// Budget Diario default, sin fechas/matching/gasto real (eso solo existe
// una vez que se trae a un mes puntual, en campaign_plan_adsets).
app.get('/api/:slug/campaign-plans/master/:masterId/adsets', async (req, res) => {
  const { password } = req.query;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { data, error } = await supabase.from('campaign_plan_master_adsets').select('*').eq('master_id', req.params.masterId).order('created_at', { ascending: true });
    if (error) throw new Error(error.message);
...
app.post('/api/:slug/campaign-plans/master/:masterId/adsets', async (req, res) => {
  const { password, adset_name, budget_diario_default } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!adset_name) return res.status(400).json({ error: 'Falta el nombre del grupo de anuncios' });
  try {
    const { data: master } = await supabase.from('campaign_plan_master').select('channel').eq('id', req.params.masterId).single();
    if (!master) return res.status(404).json({ error: 'Campaña del Master no encontrada' });
    if (master.channel !== 'meta') return res.status(400).json({ error: 'Los grupos de anuncios solo aplican a campañas de Meta Ads' });
    const { data, error } = await supabase.from('campaign_plan_master_adsets').insert({ master_id: req.params.masterId, adset_name, budget_diario_default: budget_diario_default ?? null }).select().single();
    if (error) throw new Error(error.message);
    res.json({ adset: data });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.put('/api/:slug/campaign-plans/master-adset/:adsetId', async (req, res) => {
  const { password, adset_name, budget_diario_default } = req.body;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const update = { updated_at: new Date().toISOString() };
    if (adset_name !== undefined) update.adset_name = adset_name;
    if (budget_diario_default !== undefined) update.budget_diario_default = budget_diario_default === '' ? null : budget_diario_default;
    const { data, error } = await supabase.from('campaign_plan_master_adsets').update(update).eq('id', req.params.adsetId).select().single();
    if (error) throw new Error(error.message);
    res.json({ adset: data });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.delete('/api/:slug/campaign-plans/master-adset/:adsetId', async (req, res) => {
  const { password } = req.query;
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  try {
    const { error } = await supabase.from('campaign_plan_master_adsets').delete().eq('id', req.params.adsetId);
    if (error) throw new Error(error.message);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Arma los items a insertar en un mes a partir de las filas del Master,
// compartida entre import-from-master y master/save-to-month para no
// duplicar la lógica de matching/cálculo. Corrige de paso una inconsistencia
// real: antes, un match fuzzy se aplicaba directo (campaign_id) igual que
// uno exacto — acá ya sigue la misma política que el resto de la
// plataforma (Opción B): exacto se aplica solo, fuzzy queda pendiente.
async function buildMasterImportItems(slug, masterRows, monthStart, monthEnd, existingCodes, existingNames) {
  const { data: adsetLinks } = await supabase.from('campaign_plan_master_adsets').select('master_id').in('master_id', masterRows.map(r => r.id));
  const masterIdsWithAdsets = new Set((adsetLinks || []).map(a => a.master_id));
  const toInsert = [];
  for (const mRow of masterRows) {
    // Dedup por Plan Code cuando existe; si la campaña no tiene código
    // (ahora es opcional), cae a dedup por nombre — un plan_code vacío no
    // puede usarse como clave porque puede repetirse entre varias campañas.
    const alreadyImported = mRow.plan_code ? existingCodes.has(mRow.plan_code) : existingNames.has(mRow.campaign_name);
    if (alreadyImported) continue;
    const hasAdsets = masterIdsWithAdsets.has(mRow.id);
    ...
    const isExact = match.campaign_id && match.match_confidence === 'exact_name';
    const isFuzzy = match.campaign_id && match.match_confidence === 'fuzzy';
    toInsert.push({
      plan_code: mRow.plan_code, campaign_name: mRow.campaign_name, channel: mRow.channel,
      budget_mode: mRow.budget_mode_default || (hasAdsets ? 'adset' : 'campaign'),
      campaign_id: isExact ? match.campaign_id : null,
      match_confidence: isExact ? 'exact_name' : isFuzzy ? 'fuzzy_pending' : 'sin_match',
      pending_campaign_id: isFuzzy ? match.campaign_id : null,
      pending_matched_name: isFuzzy ? match.matched_name : null,
      pending_real_status: isFuzzy ? match.real_status : null,
      ...
      status: isExact ? match.real_status : (mRow.status_default || 'Proposed'),
      kpi_1: mRow.kpi_1, kpi_2: mRow.kpi_2, kpi_3: mRow.kpi_3,
      notas: mRow.notas_default, restricciones: mRow.restricciones_default, start_date: monthStart, end_date: monthEnd,
      _master_id: mRow.id, // se usa para copiar los grupos de anuncios plantilla, no se inserta tal cual
    });
  }
  return toInsert;
}
// Después de insertar los items del mes, copia los grupos de anuncios
// plantilla de cada campaña del Master (si tenía) a campaign_plan_adsets
// del item recién creado — heredan el Budget Diario default, arrancan sin
// fechas propias (se completan con el mes del período) ni match (recién se
// puede matchear una vez que el item padre tenga campaign_id real).
async function cloneMasterAdsetsToItems(insertedItems, monthStart, monthEnd) {
  for (const item of insertedItems) {
    if (!item._master_id) continue;
    const { data: masterAdsets } = await supabase.from('campaign_plan_master_adsets').select('*').eq('master_id', item._master_id);
    if (!masterAdsets?.length) continue;
    const rows = masterAdsets.map(a => {
      const computed = computeItemBudgetFields({ start_date: monthStart, end_date: monthEnd, active_days_pattern: 'todos', budget_diario: a.budget_diario_default });
      return {
        plan_item_id: item.id, adset_name: a.adset_name, match_confidence: 'sin_match',
        start_date: monthStart, end_date: monthEnd, active_days_pattern: 'todos',
        budget_diario: a.budget_diario_default ?? null, calculated_days: computed.calculated_days ?? null,
        dias: computed.dias ?? null, budget_mensual: computed.budget_mensual ?? null,
      };
    });
    await supabase.from('campaign_plan_adsets').insert(rows);
  }
}
...
app.post('/api/:slug/campaign-plans/period/:periodId/import-from-master', async (req, res) => {
  ...
  try {
    ...
    const { data: existingItems } = await supabase.from('campaign_plan_items').select('plan_code, campaign_name').eq('plan_period_id', plan.id);
    const existingCodes = new Set((existingItems || []).map(i => i.plan_code).filter(Boolean));
    const existingNames = new Set((existingItems || []).map(i => i.campaign_name).filter(Boolean));

    const toInsert = await buildMasterImportItems(slug, masterRows || [], monthStart, monthEnd, existingCodes, existingNames);
    ...
    const cleaned = toInsert.map(({ _master_id, ...rest }) => ({ ...rest, plan_period_id: plan.id }));
    const { data: inserted, error } = await supabase.from('campaign_plan_items').insert(cleaned).select();
    if (error) throw new Error(error.message);
    // Correlación por ORDEN de inserción, no por plan_code (puede repetirse
    // vacío entre campañas sin código propio, ahora que es opcional).
    await cloneMasterAdsetsToItems(inserted.map((it, i) => ({ ...it, _master_id: toInsert[i]._master_id })), monthStart, monthEnd);
...
// "Guardar este Plan en [mes]" — disparado desde la pantalla del Master, no
// desde adentro de un mes. Si el mes todavía no tiene plan (V1), lo crea;
// si ya existe, usa la versión más reciente — y en cualquier caso importa
// las campañas del Master que todavía no estén ahí (misma lógica de
// arriba, sin duplicar código de matching/cálculo).
app.post('/api/:slug/campaign-plans/master/save-to-month', async (req, res) => {
  const slug = req.params.slug;
  const { password, period } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!period || !/^\d{4}-\d{2}$/.test(period)) return res.status(400).json({ error: 'Falta el mes (formato YYYY-MM).' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  try {
    let { data: existing } = await supabase.from('campaign_plan_periods').select('*').eq('client_id', clientDB.id).eq('period', period).order('version_number', { ascending: false }).limit(1);
    let plan = existing?.[0];
    if (!plan) {
      const { data: created, error: createError } = await supabase.from('campaign_plan_periods').insert({
        client_id: clientDB.id, period, status: 'DRAFT', version_number: 1, version_name: 'V1',
      }).select().single();
    ...
    if (toInsert.length) {
      const cleaned = toInsert.map(({ _master_id, ...rest }) => ({ ...rest, plan_period_id: plan.id }));
      const { data: inserted, error: insertError } = await supabase.from('campaign_plan_items').insert(cleaned).select();
      if (insertError) throw new Error(insertError.message);
      // Correlación por ORDEN de inserción, no por plan_code (puede repetirse
      // vacío entre campañas sin código propio, ahora que es opcional).
      await cloneMasterAdsetsToItems(inserted.map((it, i) => ({ ...it, _master_id: toInsert[i]._master_id })), monthStart, monthEnd);
    }
    res.json({ ok: true, period: plan.period, imported: toInsert.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// "¿Me das sugerencias para las campañas?" — análisis de SOLO LECTURA, no
// crea ni modifica nada. Mira el Master + cómo vino rindiendo cada campaña
// ya vinculada en el mes en curso, y devuelve comentarios en texto — el
// operador decide qué hacer con eso a mano.
app.post('/api/:slug/campaign-plans/master/suggestions', async (req, res) => {
  const slug = req.params.slug;
  const { password } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const client = await loadClient(slug);
  ...
  try {
    const { data: masterRows } = await supabase.from('campaign_plan_master').select('*').eq('client_id', clientDB.id);
    if (!masterRows?.length) return res.status(400).json({ error: 'El Plan Master está vacío — cargá al menos una campaña antes de pedir sugerencias.' });

    const currentPeriod = fmtDate(new Date()).slice(0, 7);
    const range = computeReportPeriodRange('mes_actual');
    const { data: periodRow } = await supabase.from('campaign_plan_periods').select('id').eq('client_id', clientDB.id).eq('period', currentPeriod).order('version_number', { ascending: false }).limit(1);
    let currentItemsByCode = {};
    if (periodRow?.length) {
      const { data: items } = await supabase.from('campaign_plan_items').select('*').eq('plan_period_id', periodRow[0].id);
      (items || []).forEach(it => { if (it.plan_code) currentItemsByCode[it.plan_code] = it; });
    }

    const rowsText = await Promise.all(masterRows.map(async mRow => {
      const item = currentItemsByCode[mRow.plan_code];
      let actualText = 'sin datos reales este mes (no está activa en el plan actual, o sin campaña vinculada)';
      if (item?.campaign_id && item?.channel) {
        const actual = await fetchCampaignActuals(slug, item.channel, item.campaign_id, range.start, range.end).catch(() => null);
        if (actual) actualText = `este mes: gasto $${Math.round(actual.spend)}, ${Math.round(actual.conversions)} conversiones, ingresos $${Math.round(actual.revenue)}`;
      }
      return `- "${mRow.campaign_name}" (${mRow.plan_code}, ${mRow.channel || 'canal N/D'}, tipo ${mRow.campaign_type || 'N/D'}, objetivo ${mRow.objective || 'N/D'}, budget diario de referencia $${mRow.budget_diario_default || 'N/D'}) — ${actualText}`;
    }));

    const prompt = `Sos un estratega de Paid Media de la agencia Docta Nexus, revisando el Plan Master de "${client.name}" (el molde de campañas de este cliente, no un mes puntual). Dale una mirada crítica y práctica — NO propongas cambios de presupuesto con números (eso lo hace otra función del sistema), solo comentarios cualitativos de estrategia: campañas que vienen rindiendo mal y valdría la pena revisar, huecos evidentes en la cobertura (ej. "no hay ninguna campaña de X tipo"), campañas duplicadas o redundantes, o cualquier cosa que te llame la atención. Sé concreto y breve, en viñetas, máximo 6 puntos. Si algo no tiene datos reales suficientes para opinar, decilo en vez de inventar.

Campañas del Plan Master:
${rowsText.join('\n')}

Respondé en texto plano, en español, con viñetas "- ", sin JSON ni markdown.`;

    const r = await fetch('https://api.openai.com/v1/chat/completions', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + OPENAI_KEY },
      body: JSON.stringify({ model: 'gpt-4o', max_tokens: 900, messages: [{ role: 'user', content: prompt }] }),
    });
    const d = await r.json();
    if (!r.ok || d.error) throw new Error('Agente de sugerencias (OpenAI): ' + (d.error?.message || r.status));
    logAiCall({ slug, model: 'gpt-4o', callType: 'campaign_master_suggestions', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 }).catch(() => {});
    res.json({ suggestions: d.choices?.[0]?.message?.content || 'Sin sugerencias.' });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    ...
    const planText = rowsWithActuals.map((r, i) => {
      ...
      if (r.notas) parts.push(`notas del operador: "${r.notas}"`);
      if (r.restricciones) parts.push(`restricciones: "${r.restricciones}"`);
    ...
    // objetivo, no solo la tendencia campaña por campaña. Lee de la
    // estructura nueva de "Conversión Principal" (KPI_METRIC_DEFS), con
    // fallback global→mensual — misma fuente de verdad que el panel y el
    // Informe editorial (transversalidad).
    let kpiText = '';
    try {
      const { data: targetRows } = await supabase.from('client_kpi_targets').select('metric_key, month, target_value').eq('client_id', clientDB.id);
      const global = {}, monthly = {};
      (targetRows || []).forEach(r => { if (r.month === null) global[r.metric_key] = r.target_value; else if (r.month === currentPeriod) monthly[r.metric_key] = r.target_value; });
      const hasAny = Object.keys(global).length || Object.keys(monthly).length;
      if (hasAny) {
        const kpiActuals = await computeMonthlyKpiActuals(slug, clientDB.id, currentPeriod);
        const lines = [];
        KPI_METRIC_DEFS.filter(def => !def.custom).forEach(def => {
          const target = monthly[def.key] ?? global[def.key];
          if (target == null) return;
          const actual = def.accessor(kpiActuals);
          lines.push(`${def.label} (${def.group_label}) — objetivo: ${target}, real: ${actual ?? 'N/D'}`);
        });
...
[{"campaign_name":"...","plan_code":"...","channel":"meta|google_ads","objective":"...","budget_diario_change_pct":0,"budget_diario_new":null,"active_days_pattern":"todos|lunes_viernes|lunes_sabado|sabado_domingo","kpi_1":"...","kpi_2":"","kpi_3":"","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
    ...
    const proposedItems = proposed.map(p => {
      ...
      const budgetChangePct = baseBudgetDiario > 0 ? +(((budget_diario - baseBudgetDiario) / baseBudgetDiario) * 100).toFixed(1) : null;
      return {
        ...
        // Transparencia: qué usó la IA de base para esta decisión — se muestra
        // en la vista previa, no queda "atrás de cámara".
        data_used: baseline?.actual
          ? { spend: baseline.actual.spend, conversions: baseline.actual.conversions, revenue: baseline.actual.revenue, matched_by: baseline.actual.matched_by || null, budget_change_pct: budgetChangePct }
          : { spend: null, conversions: null, revenue: null, matched_by: null, budget_change_pct: budgetChangePct },
      };
    });
...
app.post('/api/:slug/campaign-plans/confirm', async (req, res) => {
  ...
  try {
    const { data: existingVersions } = await supabase.from('campaign_plan_periods').select('version_number').eq('client_id', clientDB.id).eq('period', period).order('version_number', { ascending: false }).limit(1);
    const nextVersion = existingVersions?.length ? existingVersions[0].version_number + 1 : 1;
    const { data: plan, error: planError } = await supabase.from('campaign_plan_periods')
      .insert({ client_id: clientDB.id, period, status: 'PROPOSED', version_number: nextVersion, version_name: `V${nextVersion}` })
      .select().single();
    if (planError) throw new Error(planError.message);
...
app.post('/api/:slug/campaign-plans/period/:periodId/story/generate', async (req, res) => {
  ...
  try {
    ...
    // Moneda real por canal — mismo criterio ya usado en el resto de la
    // plataforma (Opción A): nunca se convierte/mezcla, cada canal se
    // muestra en la suya. Si dos canales presentes en el plan tienen
    // monedas distintas (ej. Meta en USD, Google en ARS), NO se arma un
    // total combinado — sería sumar unidades distintas.
    const channelCurrencies = {};
    for (const ch of channelsPresent) channelCurrencies[ch] = (await getAccountCurrency(slug, ch)) || client.currency || 'ARS';
    const distinctCurrencies = new Set(Object.values(channelCurrencies));
    const mixedCurrency = distinctCurrencies.size > 1;
    ...
    const totalBudget = mixedCurrency ? null : items.reduce((s, it) => s + (Number(it.budget_mensual) || 0), 0);
    const totalCurrency = mixedCurrency ? null : (distinctCurrencies.values().next().value || client.currency || 'ARS');

    const itemsText = items.map((it, i) => `${i + 1}. "${it.campaign_name}" — ${it.channel || 'canal N/D'}, objetivo: ${it.objective || 'N/D'}, presupuesto mensual: $${it.budget_mensual || 0} ${channelCurrencies[it.channel] || ''}${it.notas ? `, notas del operador: "${it.notas}"` : ''}${it.restricciones ? `, restricciones: "${it.restricciones}"` : ''}${[it.kpi_1, it.kpi_2, it.kpi_3].filter(Boolean).length ? `, KPIs: ${[it.kpi_1, it.kpi_2, it.kpi_3].filter(Boolean).join(', ')}` : ''}`).join('\n');
...
${mixedCurrency ? `\nIMPORTANTE: los canales de este plan están en MONEDAS DISTINTAS (${channelsPresent.map(ch => `${ch}: ${channelCurrencies[ch]}`).join(', ')}). NUNCA sumes ni compares montos entre canales como si fueran la misma moneda — si mencionás un presupuesto, aclarále siempre la moneda correspondiente a ese canal.\n` : ''}
...
{"headline":"1 oración editorial resumiendo el plan del período","dek":"2-3 oraciones ampliando el headline","composition_narrative":"3-5 oraciones contando la MEZCLA de campañas de este plan y por qué — ej. 'planificamos X campañas de conversión (Y en Meta, Z en Google) enfocadas en escalar lo que mejor funciona, y sumamos una de alcance para sostener marca' — basate en los objetivos/notas reales de arriba, no inventes motivos que no estén ahí","campaign_blurbs":[{"campaign_name":"...","blurb":"1-2 oraciones sobre esta campaña puntual, basadas en su objetivo/notas/KPI reales"}]}`;
    ...
    const reportData = {
      period_label: periodToLabel(plan.period), total_budget: totalBudget, total_currency: totalCurrency, mixed_currency: mixedCurrency,
      business_objective: plan.business_objective, primary_kpi: plan.primary_kpi, target_value: plan.target_value,
      channel_split: {
        meta: { amount: metaSpend, currency: channelCurrencies.meta || null },
        google_ads: { amount: googleSpend, currency: channelCurrencies.google_ads || null },
      },
      objective_split: byObjective,
      items: items.map(it => ({ campaign_name: it.campaign_name, channel: it.channel, objective: it.objective, budget_mensual: it.budget_mensual, currency: channelCurrencies[it.channel] || null, kpi_1: it.kpi_1, kpi_2: it.kpi_2, kpi_3: it.kpi_3 })),
...
// Google Ads — Claude y GPT como dos motores 100% independientes (mismo
// patrón que Meta Ads). Ya no hay Síntesis automática combinada — cada uno
// corre solo, con su propio botón, su propio cupo y su propio toggle on/off.
app.post('/api/:slug/intelligence/analyze-google-claude', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if ((clientDB.enabled_features || DEFAULT_ENABLED_FEATURES).ai_google_claude === false) return res.status(403).json({ error: 'Esta función está desactivada para tu cuenta.' });
  if (!ANTHROPIC_KEY) return res.status(500).json({ error: 'Falta la API key de Claude configurada' });
  const quota = await checkAiQuota(slug, 'google_claude_analysis');
  if (quota.blocked) return res.status(429).json({ error: quota.message, ai_usage: quota.usage });
  ...
  try {
    ...
    const result = await runExpertClaude('google_ads', contextText, slug);
    await incrementAiUsage(clientDB.id, 'google_claude_analysis');
    const ai_usage = await getAiUsage(slug, 'google_claude_analysis');
    res.json({ context: contextText, claude: result, ai_usage });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

app.post('/api/:slug/intelligence/analyze-google-gpt', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if ((clientDB.enabled_features || DEFAULT_ENABLED_FEATURES).ai_google_gpt === false) return res.status(403).json({ error: 'Esta función está desactivada para tu cuenta.' });
  if (!OPENAI_KEY) return res.status(500).json({ error: 'Falta la API key de OpenAI configurada' });
  const quota = await checkAiQuota(slug, 'google_gpt_analysis');
  if (quota.blocked) return res.status(429).json({ error: quota.message, ai_usage: quota.usage });
  ...
  try {
    ...
    const result = await runExpertGPT('google_ads', contextText, slug);
    await incrementAiUsage(clientDB.id, 'google_gpt_analysis');
    const ai_usage = await getAiUsage(slug, 'google_gpt_analysis');
    res.json({ context: contextText, gpt: result, ai_usage });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/intelligence/analyze-ecommerce', async (req, res) => {
  ...
  const quotaEcom = await checkAiQuota(slug, 'ecommerce_analysis');
  if (quotaEcom.blocked) return res.status(429).json({ error: quotaEcom.message, ai_usage: quotaEcom.usage });
  ...
  try {
    ...
    await incrementAiUsage(clientDB.id, 'ecommerce_analysis');
    const ai_usage = await getAiUsage(slug, 'ecommerce_analysis');
    res.json({ context: contextText, ...result, ai_usage });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/intelligence/analyze-ga4-metrics', async (req, res) => {
  ...
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!ANTHROPIC_KEY || !OPENAI_KEY) return res.status(500).json({ error: 'Faltan API keys de IA configuradas' });
  const quotaAn1 = await checkAiQuota(slug, 'analytics_analysis');
  if (quotaAn1.blocked) return res.status(429).json({ error: quotaAn1.message, ai_usage: quotaAn1.usage });
  ...
  try {
    ...
    await incrementAiUsage(clientDB.id, 'analytics_analysis');
    const ai_usage = await getAiUsage(slug, 'analytics_analysis');
    res.json({ context: contextText, ...result, ai_usage });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
app.post('/api/:slug/intelligence/analyze-ga4-funnel', async (req, res) => {
  ...
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!ANTHROPIC_KEY || !OPENAI_KEY) return res.status(500).json({ error: 'Faltan API keys de IA configuradas' });
  const quotaAn2 = await checkAiQuota(slug, 'analytics_analysis');
  if (quotaAn2.blocked) return res.status(429).json({ error: quotaAn2.message, ai_usage: quotaAn2.usage });
  ...
  try {
    ...
    await incrementAiUsage(clientDB.id, 'analytics_analysis');
    const ai_usage = await getAiUsage(slug, 'analytics_analysis');
    res.json({ context: contextText, steps, ...result, ai_usage });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});
...
// ── Backfill de resiliencia — Reporte Inteligente ───────────────────
// Si el cron nocturno se pierde un día (deploy, restart, lo que sea), la
// próxima vez que alguien abra el dashboard y la carga EN VIVO funcione
// bien, dejamos un registro de hoy en ad_metrics_daily. Así getTrackingHealth()
// nunca vuelve a decir "no sincronizado" mientras la conexión real esté sana
// — el cron sigue siendo la vía principal, esto es solo la red de seguridad.
async function backfillTodayMarker(clientId, source, totals) {
  try {
    await supabase.from('ad_metrics_daily').upsert({
      client_id: clientId, source, date: today(), campaign_id: '__ACCOUNT__',
      spend: totals.spend || 0, impressions: totals.impressions || 0, clicks: totals.clicks || 0,
      ctr: totals.ctr ?? null, conversions: totals.conversions || 0, conversion_value: totals.conversion_value || 0,
      roas: totals.roas ?? null,
    }, { onConflict: 'client_id,source,date,campaign_id' });
  } catch (e) { console.warn(`[backfill] ${source} omitido:`, e.message); }
}
...
app.get('/api/:slug/google-ads/metrics', async (req, res) => {
  ...
  try {
    ...
    const liveCurrency = await getAccountCurrency(slug, 'google_ads');
    backfillTodayMarker(clientDB.id, 'google_ads', data).catch(() => {});
    res.json({ period: period || '7d', since, until, data, currency: liveCurrency || clientDB.currency || 'ARS' });
  } catch(e) {
...
app.get('/api/:slug/google-ads/objetivos', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  ...
  if (!customerId) return res.status(400).json({ error: 'Este cliente no tiene una cuenta de Google Ads asociada' });
  try {
    const { since, until } = computeDateRangeForPeriod(period || '30d', iso_year, iso_week);
    const objetivos = await fetchConversionActionsSummary(customerId, since, until);
    res.json({ period: period || '30d', since, until, objetivos });
  } catch (e) {
    console.error(`[google-ads/objetivos] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});

app.get('/api/:slug/analytics/eventos', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  ...
  try {
    const propertyId = await getGA4PropertyId(slug);
    if (!propertyId) return res.status(400).json({ error: 'Este cliente no tiene una propiedad de GA4 asociada' });
    const { since, until } = computeDateRangeForPeriod(period || '30d', iso_year, iso_week);
    const eventos = await fetchGA4KeyEventsSummary(propertyId, since, until);
    res.json({ period: period || '30d', since, until, eventos });
  } catch (e) {
    console.error(`[analytics/eventos] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});

app.get('/api/:slug/meta/action-types', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  ...
  try {
    const creds = await getMetaCredentials(slug);
    if (!creds || !creds.access_token || !creds.ad_account_id) return res.status(400).json({ error: 'Este cliente no tiene una cuenta de Meta Ads asociada' });
    const { since, until } = computeDateRangeForPeriod(period || '30d', iso_year, iso_week);
    const action_types = await fetchMetaRawActionTypesSummary(creds.ad_account_id, creds.access_token, since, until);
    res.json({ period: period || '30d', since, until, action_types });
  } catch (e) {
    console.error(`[meta/action-types] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});

// ── Debug: Conversiones Personalizadas configuradas en la cuenta ──
// Devuelve nombre real + ID de cada Conversión Personalizada de Meta,
// para identificar (por nombre, ej. "Hacer Donación") a qué ID
// corresponde un action_type tipo "offsite_conversion.custom.<ID>"
// que aparece en /meta/action-types sin nombre legible.
app.get('/api/:slug/meta/custom-conversions', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password } = req.query;
  ...
  try {
    const creds = await getMetaCredentials(slug);
    if (!creds || !creds.access_token || !creds.ad_account_id) return res.status(400).json({ error: 'Este cliente no tiene una cuenta de Meta Ads asociada' });
    const result = await metaFetch(`/${creds.ad_account_id}/customconversions?fields=id,name,custom_event_type,description,is_archived&limit=200&`, creds.access_token);
    res.json({ custom_conversions: result.data || [] });
  } catch (e) {
    console.error(`[meta/custom-conversions] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});
...
app.get('/api/:slug/google-ads/products', async (req, res) => {
  ...
  try {
    ...
    const liveCurrency = await getAccountCurrency(slug, 'google_ads');
    res.json({ period: period || '7d', since, until, products, currency: liveCurrency || clientDB.currency || 'ARS' });
  } catch(e) {
...
// Palabras clave en vivo (Search) — mismo motor que ya usa el Informe Frío
// (fetchTopKeywords), ahora también disponible directo en el panel del
// dashboard sin tener que generar un reporte para verlas.
app.get('/api/:slug/google-ads/keywords', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  ...
  try {
    const { since, until } = computeDateRangeForPeriod(period || '7d', iso_year, iso_week);
    const keywords = await fetchTopKeywords(customerId, since, until, 50);
    const liveCurrency = await getAccountCurrency(slug, 'google_ads');
    res.json({ period: period || '7d', since, until, keywords, currency: liveCurrency || clientDB.currency || 'ARS' });
  } catch(e) {
    console.error(`[google-ads/keywords] Error en ${slug}:`, e.message);
    res.status(500).json({ error: e.message });
  }
});

// ── Conversiones por día de la semana — Google Ads ──────────────────
app.get('/api/:slug/google-ads/conversions-by-weekday', async (req, res) => {
  const slug = req.params.slug;
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  ...
  try {
    const { since, until } = computeDateRangeForPeriod(period || '30d', iso_year, iso_week);
    const days = await fetchConversionsByWeekday(customerId, since, until);
    res.json({ profile: clientDB.report_profile, days });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
});

// ── Conversiones por día de la semana — Meta Ads ────────────────────
// Reagrupa datos que ya se piden hoy (insights con time_increment=1), sin
// integrar nada nuevo a la cuenta de Meta. Mensajes si el cliente es leads,
// Compras si es ecommerce, ambos si es híbrido.
const WEEKDAY_LABELS = ['Domingo','Lunes','Martes','Miércoles','Jueves','Viernes','Sábado'];
app.get('/api/:slug/meta/conversions-by-weekday', async (req, res) => {
  const slug = req.params.slug;
  const client = await loadClientWithCreds(slug);
  if (!client) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, iso_year, iso_week } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!client.access_token || !client.ad_account_id) return res.status(400).json({ error: 'Este cliente no tiene Meta Ads conectado' });

  try {
    const { since, until } = computeDateRangeForPeriod(period || '30d', iso_year, iso_week);
    const r = await metaFetch(
      '/' + client.ad_account_id + '/insights?fields=actions&time_increment=1&time_range=' +
      encodeURIComponent(JSON.stringify({ since, until })) + '&level=account&', client.access_token
    );
    const rows = r.data || [];
    const totals = { lead: [0,0,0,0,0,0,0], purchase: [0,0,0,0,0,0,0] };
    rows.forEach(row => {
      const d = new Date(row.date_start + 'T12:00:00Z'); // mediodía UTC evita corrimiento de día por huso horario
      const wd = d.getUTCDay(); // 0=Domingo
      const actions = row.actions || [];
      const messages = getAction(actions, 'onsite_conversion.messaging_conversation_started_7d');
      const purchases = getAction(actions, 'offsite_conversion.fb_pixel_purchase') || getAction(actions, 'purchase') || getAction(actions, 'omni_purchase');
      totals.lead[wd] += messages;
      totals.purchase[wd] += purchases;
    });
    res.json({
      profile: client.report_profile,
      days: WEEKDAY_LABELS.map((label, i) => ({ label, mensajes: totals.lead[i], compras: totals.purchase[i] })),
    });
  } catch(e) {
    res.status(500).json({ error: e.message });
  }
...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    const liveCurrency = await getAccountCurrency(slug, 'meta');
    res.json({ ...data, currency: liveCurrency || client.currency || 'ARS' });
  } catch(e) {
...
app.post('/api/:slug/analyze-only', async (req, res) => {
  ...
  const { password, period } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDBForToggle = await loadClientDB(slug);
  if (clientDBForToggle && (clientDBForToggle.enabled_features || DEFAULT_ENABLED_FEATURES).ai_meta_claude === false) return res.status(403).json({ error: 'Esta función está desactivada para tu cuenta.' });
  const quota = await checkAiQuota(slug, 'meta_claude_analysis');
  if (quota.blocked) return res.status(429).json({ error: quota.message, ai_usage: quota.usage });
  try {
    ...
    // Recién acá, con la llamada a la IA ya resuelta con éxito, se descuenta la consulta
    const clientDBForUsage = await loadClientDB(slug);
    if (clientDBForUsage) await incrementAiUsage(clientDBForUsage.id, 'meta_claude_analysis').catch(() => {});
    const ai_usage = await getAiUsage(slug, 'meta_claude_analysis');
    res.json({ analysis, ai_usage });
...
// ── Creative Intelligence Fase 1 — capa adicional sobre Creatividades,
// NO reemplaza nada de lo que ya existía. Son señales inferidas a partir
// del delivery observable de Meta, no métricas oficiales de Andromeda
// (Meta no expone eso) — se etiquetan siempre así en el frontend.
// Es un análisis CROSS-SECCIONAL (una sola foto del período elegido, no
// una serie de varios días por anuncio) porque hoy no se guarda historial
// diario por creatividad — la fatiga y el lifecycle son heurísticas sobre
// esa foto, no una tendencia real en el tiempo. Documentado a propósito
// para no sobre-prometer precisión que los datos disponibles no sostienen.
function computeCreativeIntelligence(creatives){
  const withSpend = creatives.filter(c => c.spend > 0);
  const totalSpend = withSpend.reduce((s,c)=>s+c.spend,0);
  const sorted = [...withSpend].sort((a,b)=>b.spend-a.spend);

  const pctOfSpend = n => totalSpend > 0 ? +(sorted.slice(0,n).reduce((s,c)=>s+c.spend,0) / totalSpend * 100).toFixed(1) : null;
  const top1Pct = pctOfSpend(1), top3Pct = pctOfSpend(3), top5Pct = pctOfSpend(5);
  const explorationRatio = top5Pct != null ? +(100 - top5Pct).toFixed(1) : null;

  // Promedios de cuenta, para comparar cada creatividad contra "lo normal"
  // de esta cuenta en este período — nunca contra un benchmark externo.
  const avgCtr = withSpend.length ? withSpend.reduce((s,c)=>s+c.ctr,0)/withSpend.length : null;
  const cpaList = withSpend.filter(c=>c.mainResult?.cpa != null).map(c=>c.mainResult.cpa);
  const avgCpa = cpaList.length ? cpaList.reduce((s,v)=>s+v,0)/cpaList.length : null;
  const avgSpend = withSpend.length ? totalSpend / withSpend.length : 0;

  const lifecycleOf = c => {
    const hasEnoughVolume = c.spend >= avgSpend * 0.5 || c.impressions >= 1000;
    if (!hasEnoughVolume) {
      const promising = avgCtr != null && c.ctr >= avgCtr;
      return promising ? 'explorer' : 'insufficient_data';
    }
    const ctrGood = avgCtr == null || c.ctr >= avgCtr * 1.1;
    const ctrBad  = avgCtr != null && c.ctr < avgCtr * 0.7;
    const cpaGood = avgCpa == null || c.mainResult?.cpa == null || c.mainResult.cpa <= avgCpa * 0.9;
    const cpaBad  = avgCpa != null && c.mainResult?.cpa != null && c.mainResult.cpa > avgCpa * 1.3;
    if (ctrBad || cpaBad) return 'loser';
    if (ctrGood && cpaGood) return 'winner';
    return 'insufficient_data';
  };

  // Fatiga combinada — Frecuencia alta + CTR por debajo del promedio de
  // cuenta + costo por resultado por encima del promedio, todo a la vez
  // (no solo Frecuencia sola, como el criterio viejo).
  const isFatigued = c => {
    const highFreq = c.frequency > 3.5;
    const lowCtr = avgCtr != null && c.ctr < avgCtr * 0.85;
    const highCpa = avgCpa != null && c.mainResult?.cpa != null && c.mainResult.cpa > avgCpa * 1.15;
    return highFreq && (lowCtr || highCpa);
  };

  creatives.forEach(c => {
    c.lifecycle = withSpend.includes(c) ? lifecycleOf(c) : 'insufficient_data';
    c.fatigued = isFatigued(c);
  });

  return {
    concentration: { top1Pct, top3Pct, top5Pct },
    explorationRatio,
    winnersCount: creatives.filter(c=>c.lifecycle==='winner').length,
    losersCount: creatives.filter(c=>c.lifecycle==='loser').length,
    explorersCount: creatives.filter(c=>c.lifecycle==='explorer').length,
    fatiguedCount: creatives.filter(c=>c.fatigued).length,
  };
}
...
app.get('/api/:slug/creatives', async (req, res) => {
  ...
  const client = await loadClientWithCreds(slug);
  if (!client) return res.status(404).json({ error: 'Cliente no encontrado' });
  const clientDBFull = await loadClientDB(slug); // loadClient() no trae id/enabled_features — los necesitamos acá
  const { password, period, withClaude, withGpt } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  ...
  try {
    ...
    const [adsData, batchRes] = await Promise.all([
      metaFetch('/' + accountId + '/insights?fields=ad_id,ad_name,adset_name,campaign_name,spend,impressions,clicks,ctr,cpm,reach,frequency,actions&date_preset=' + preset + '&level=ad&sort=spend_descending&limit=50&', token),
      metaFetch('/' + accountId + '/ads?fields=id,creative{thumbnail_url,image_url,video_id,title,body,object_story_spec{link_data{picture},video_data{image_url}}}&thumbnail_width=600&thumbnail_height=450&limit=50&', token).catch(() => ({ data: [] }))
    ]);
    ...
    const creatives = ads.map(ad => {
      ...
      return {
        ...
        imageUrl: c.image_url || c.thumbnail_url ||
          ...
          c.object_story_spec?.video_data?.image_url || null,
    ...
    const creativeIntelligence = computeCreativeIntelligence(creatives);

    // Claude y GPT como dos motores 100% independientes (mismo patrón que
    // Meta Ads) — ya no hay Nexus AI ni veredicto Andromeda combinado.
    // Cada uno analiza el Top 6 por spend y devuelve su propio resultado.
    let claudeAnalysis = null, gptAnalysis = null, claudeBlocked = null, gptBlocked = null;
    const enoughData = creatives.length >= 15;

    if (withClaude === 'true' && creatives.length > 0) {
      if (!enoughData) {
        claudeBlocked = `Necesitás al menos 15 creatividades para un análisis confiable — hoy tenés ${creatives.length}.`;
      } else if ((clientDBFull.enabled_features || DEFAULT_ENABLED_FEATURES).ai_creatives_claude === false) {
        claudeBlocked = 'Esta función está desactivada para tu cuenta.';
      } else {
        const quota = await checkAiQuota(slug, 'creatives_claude_analysis');
        if (quota.blocked) { claudeBlocked = quota.message; }
        else {
          const top = creatives.slice(0, 6);
          const claudeAnalyses = await Promise.all(top.map(c => analyzeCreativeWithClaude(c, slug).catch(() => ({}))));
          top.forEach((c, i) => { c.claudeAnalysis = claudeAnalyses[i] || null; });
          claudeAnalysis = { perAd: claudeAnalyses, adNames: top.map(c => c.adName) };
          await incrementAiUsage(clientDBFull.id, 'creatives_claude_analysis').catch(() => {});
        }
      }
    }

    if (withGpt === 'true' && creatives.length > 0 && OPENAI_KEY) {
      if (!enoughData) {
        gptBlocked = `Necesitás al menos 15 creatividades para un análisis confiable — hoy tenés ${creatives.length}.`;
      } else if ((clientDBFull.enabled_features || DEFAULT_ENABLED_FEATURES).ai_creatives_gpt === false) {
        gptBlocked = 'Esta función está desactivada para tu cuenta.';
      } else {
        const quota = await checkAiQuota(slug, 'creatives_gpt_analysis');
        if (quota.blocked) { gptBlocked = quota.message; }
        else {
          const top = creatives.slice(0, 6);
          const gptAnalyses = await Promise.all(top.map(c => analyzeCreativeWithGPT(c, slug).catch(() => ({}))));
          top.forEach((c, i) => { c.gptAnalysis = gptAnalyses[i] || null; });
          gptAnalysis = { perAd: gptAnalyses, adNames: top.map(c => c.adName) };
          await incrementAiUsage(clientDBFull.id, 'creatives_gpt_analysis').catch(() => {});
        }
      }
    }

    const ai_usage_claude = await getAiUsage(slug, 'creatives_claude_analysis');
    const ai_usage_gpt = await getAiUsage(slug, 'creatives_gpt_analysis');
    res.json({
      creatives, claudeAnalysis, gptAnalysis, claudeBlocked, gptBlocked,
      period: preset, total: creatives.length,
      ai_usage_claude, ai_usage_gpt, min_creatives_for_ai: 15, creativeIntelligence,
    });
  } catch(e) {
...
app.post('/api/:slug/analyze-gpt', async (req, res) => {
  ...
  const clientDBForToggle = await loadClientDB(slug);
  if (clientDBForToggle && (clientDBForToggle.enabled_features || DEFAULT_ENABLED_FEATURES).ai_meta_gpt === false) return res.status(403).json({ error: 'Esta función está desactivada para tu cuenta.' });
  const quota = await checkAiQuota(slug, 'meta_gpt_analysis');
  if (quota.blocked) return res.status(429).json({ error: quota.message, ai_usage: quota.usage });
...
Respondé SOLO en JSON con este formato exacto:
{
  ...
  "conclusion_items": [{"icon":"emoji","titulo":"título","detalle":"detalle"}]
    ...
    const clientDBForUsage = await loadClientDB(slug);
    if (clientDBForUsage) await incrementAiUsage(clientDBForUsage.id, 'meta_gpt_analysis').catch(() => {});
    const ai_usage = await getAiUsage(slug, 'meta_gpt_analysis');
    res.json({ analysis, model: 'gpt-4o-mini', ai_usage });
  } catch(e) {
...
// Consultar el cupo de IA sin disparar ningún análisis — usado por el
// frontend para refrescar "Te quedan X de Y consultas" (ej. al volver a
// entrar a la solapa Meta Ads sin recargar toda la página).
app.get('/api/:slug/ai-usage', async (req, res) => {
  const slug = req.params.slug;
  const { password, feature } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const ai_usage = await getAiUsage(slug, feature || 'meta_analysis');
  res.json({ ai_usage });
});

// Trae el cupo de TODAS las funciones de IA de una sola vez — usado al
// loguear para pintar todos los hints sin hacer 4-5 requests sueltos.
app.get('/api/:slug/ai-usage-all', async (req, res) => {
  const slug = req.params.slug;
  const { password } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const features = ['meta_claude_analysis', 'meta_gpt_analysis', 'google_claude_analysis', 'google_gpt_analysis'];
  const entries = await Promise.all(features.map(async f => [f, await getAiUsage(slug, f)]));
  res.json({ ai_usage: Object.fromEntries(entries) });
});
...
// Fase 4 — URL limpia por cliente (/{slug} en vez de quedarse siempre en
// /login). Con los tokens de la Fase 1 ya en producción, dashboard.html
// hace history.pushState('/'+slug) al loguear. Esta ruta es lo que hace que
// esa URL sobreviva un F5: sin esto, recargar en /{slug} caería en el
// catch-all de abajo y mostraría la home comercial (index.html) en vez del
// dashboard. Va DESPUÉS de /admin, /login, /rf/:token, /r/:token, /ps/:token
// (todas ya registradas arriba) y ANTES del catch-all final — Express
// matchea por orden de registro, no por especificidad, así que el orden acá
// importa. express.static ya se ejecutó antes que todo esto, así que un
// archivo real (css/js/imagen) nunca llega hasta acá.
app.get('/:slug', (req, res, next) => {
  const reserved = ['admin', 'login', 'health', 'rf', 'r', 'ps', 'api', 'favicon.ico'];
