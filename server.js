...
// Bug real corregido: createTransporter() se llamaba pero nunca estaba
// definida en ningún lado — el mail de aviso (éxito o error de renovación)
// tiraba ReferenceError y se perdía en el catch de más abajo. La
// renovación del token en sí NUNCA dependía de esto (ya se guarda antes,
// línea de arriba), pero el aviso por mail jamás llegaba.
// Requiere EMAIL_USER + EMAIL_PASS (contraseña de aplicación de Gmail,
// no la contraseña normal de la cuenta) como variables de entorno en
// Render. Si no están seteadas, no rompe nada — solo no manda el mail.
...
async function renewTokenIfNeeded(slug) {
  ...
  try {
    ...
    // Notificar por email
    if (clientDB.email_alerts?.length) {
      const transporter = createTransporter();
      if (!transporter) {
        console.log(`[TOKEN] ${slug}: token renovado OK, pero EMAIL_USER/EMAIL_PASS no están configurados — no se mandó el aviso por mail.`);
      } else {
    ...
    if (clientDB.email_alerts?.length) {
      const transporter = createTransporter();
      if (!transporter) {
        console.log(`[TOKEN] ${slug}: falló la renovación y tampoco se pudo avisar por mail (EMAIL_USER/EMAIL_PASS no configurados). Error real: ${e.message}`);
      } else {
      await transporter.sendMail({
        from: `"Meta Intelligence | Docta Nexus" <${process.env.EMAIL_USER}>`,
        to: clientDB.email_alerts.join(','),
