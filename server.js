...
async function exportPlanToSheet(sheetId, tabName, items) {
  ...
  const values = [header, ...items.map(r => [
    ...
    r.status || '', r.kpi_1 || '', r.kpi_2 || '', r.kpi_3 || '', r.notas || '',
  ])];
...
app.post('/api/:slug/campaign-plans/item', async (req, res) => {
  const slug = req.params.slug;
  const { password, plan_period_id, campaign_name, channel, objective, budget_diario, dias, budget_mensual, kpi_1, kpi_2, kpi_3, notas, start_date, end_date, status } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  ...
  try {
    ...
    const { data, error } = await supabase.from('campaign_plan_items').insert({
      ...
      objective: objective || null, budget_diario: budget_diario ?? null, dias: dias ?? null, budget_mensual: budget_mensual ?? null,
      kpi_1: kpi_1 || null, kpi_2: kpi_2 || null, kpi_3: kpi_3 || null, notas: notas || null,
...
app.put('/api/:slug/campaign-plans/item/:itemId', async (req, res) => {
  ...
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const allowed = ['campaign_name', 'channel', 'campaign_id', 'match_confidence', 'campaign_role', 'objective', 'budget_diario', 'dias', 'budget_mensual', 'status', 'kpi_1', 'kpi_2', 'kpi_3', 'notas', 'start_date', 'end_date'];
  const update = { updated_at: new Date().toISOString() };
...
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    ...
    const { data: currentItems } = periodRow?.length
      ...
      : { data: [] };
    if (!currentItems?.length) return res.status(400).json({ error: `No hay un plan cargado para ${currentPeriod} todavía — cargá al menos una campaña antes de generar una propuesta.` });
...
${!anyRealData ? '\nIMPORTANTE: ninguna campaña del plan tiene datos reales disponibles este mes — en ese caso, no tenés base numérica para ajustar presupuestos, así que mantené el plan estructuralmente similar pero dejalo explícito en las notas de cada campaña ("sin datos reales para ajustar este mes, se mantiene el plan"). No inventes un ajuste que no podés justificar.\n' : ''}
Plan actual y su desempeño real de este mes:
${planText}
${kpiText}
Respondé SOLO en JSON, un array de campañas propuestas para ${nextMonthLabel}:
[{"campaign_name":"...","channel":"meta|google_ads","start_date":"YYYY-MM-DD","end_date":"YYYY-MM-DD","objective":"...","budget_diario":0,"dias":0,"budget_mensual":0,"kpi_1":"...","kpi_2":"","kpi_3":"","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
  ...
  try {
    ...
    const itemsWithMatch = await Promise.all(items.map(async it => {
      ...
      return {
        ...
        campaign_id: match.campaign_id, match_confidence: match.campaign_id ? match.match_confidence : 'sin_match',
        objective: it.objective || null, budget_diario: it.budget_diario || null, dias: it.dias || null, budget_mensual: it.budget_mensual || null,
        kpi_1: it.kpi_1 || null, kpi_2: it.kpi_2 || null, kpi_3: it.kpi_3 || null, notas: it.notas || null,
        start_date: it.start_date || null, end_date: it.end_date || null, status: it.status || 'Proposed',
      };
    }));
