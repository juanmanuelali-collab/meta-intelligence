...
async function runAnalysis(client, metricsData) {
  ...
  const PROFILES = {
    ecommerce: {
      ...
      foco: 'ROAS, CPA, valor de conversiones y embudo pixel completo (vista de contenido → carrito → checkout → compra)',
      metricas_principales: 'ROAS, CPA, Compras, Valor de conversiones, Checkout iniciado, Al carrito',
      metricas_ignorar: 'No mencionar mensajería ni profundidad de conversación a menos que sea relevante. No evaluar como "bajo" el CTR de campañas de remarketing.',
      benchmarks: 'ROAS >2x mínimo viable, >4x saludable. CPA debe compararse contra ticket promedio del negocio. CTR feed 1-3%. Frecuencia máxima 3.5 para prospecting, hasta 5 para remarketing.',
      acciones_prioritarias: 'Optimizar embudo pixel, ajustar audiencias TOFU/MOFU/BOFU, rotar creatividades por fase del funnel, escalar conjuntos con mejor ROAS.',
    },
    mensajeria: {
      nombre: 'Mensajería / Leads / Servicios',
      foco: 'Costo por mensaje, profundidad de conversación (depth 2/3/5), CPL y calidad de los leads generados',
      metricas_principales: 'Conexiones de mensajería, Costo por mensaje, Depth 2/3/5, CPL, Leads, Primeras respuestas',
      metricas_ignorar: 'NO mencionar ROAS ni valor de conversiones — este cliente no tiene ecommerce. No evaluar pixel de compras.',
      benchmarks: 'Depth 5 / Conexiones >30% indica conversaciones de calidad. Depth 2 / Conexiones >70% mínimo. Costo por mensaje comparar contra ticket del servicio. Frecuencia máxima 3 para cold, hasta 4 para remarketing.',
      acciones_prioritarias: 'Mejorar hooks creativos para aumentar tasa de apertura de chat, optimizar horarios de entrega, segmentar audiencias por intención de compra, evaluar respuesta del equipo comercial.',
    },
    trafico: {
      nombre: 'Tráfico / Contenido / Medios',
      foco: 'CPC, CTR, vistas de landing page y volumen de clics de calidad',
      metricas_principales: 'CPC, CTR, Vistas de landing, Clics en enlace, Clics de salida',
      metricas_ignorar: 'NO mencionar ROAS ni compras. No evaluar mensajería como KPI principal.',
      benchmarks: 'CPC benchmark varía por industria. CTR >1.5% saludable para tráfico frío. Landing page views / Link clicks >70% indica landing rápida. Frecuencia máxima 2.5 para tráfico frío.',
      acciones_prioritarias: 'Mejorar copy de anuncios para aumentar CTR, optimizar landing page para reducir rebote, ajustar segmentación por intereses, testear formatos (carrusel vs imagen única vs video).',
    },
    ong: {
      nombre: 'ONG / Donaciones / Causa social',
      foco: 'Donaciones generadas, costo por donación, alcance de la causa y engagement con el contenido de impacto social',
      metricas_principales: 'Donaciones (onsite + web), Costo por donación, Alcance, Engagement, Compartidos, Guardados',
      metricas_ignorar: 'NO mencionar ROAS — no aplica para ONGs. No evaluar conversiones comerciales como compras. No penalizar CPM alto si el alcance de causa lo justifica.',
      benchmarks: 'El engagement y compartidos son especialmente valiosos — amplifican el mensaje sin costo adicional. Frecuencia puede ser mayor (hasta 4) si el mensaje es de impacto social. Costo por donación debe compararse contra donación promedio de la organización.',
      acciones_prioritarias: 'Amplificar contenido de alto impacto emocional, usar testimonios reales, targeting a audiencias similares a donantes actuales, optimizar landing de donación, testear diferentes llamados a la acción.',
    },
    awareness: {
      nombre: 'Marca / Awareness / Institucional',
      foco: 'CPM eficiente, frecuencia óptima de exposición, alcance único máximo y share of voice en el mercado',
      metricas_principales: 'Alcance, CPM, Frecuencia, Impresiones, Engagement de marca',
      metricas_ignorar: 'NO mencionar ROAS ni CPA como métricas de éxito — awareness no se evalúa en conversiones directas. No penalizar CTR bajo en campañas de alcance puro.',
      benchmarks: 'CPM eficiente varía por mercado. Frecuencia ideal 2-4 para recordación de marca. Alcance único >15% del target ideal. CTR en awareness puede ser 0.3-0.8% y es normal.',
      acciones_prioritarias: 'Maximizar alcance único sin sacrificar frecuencia mínima de recordación, diversificar placements para menor CPM, usar video para mayor impacto de marca, crear audiencias de remarketing para futura conversión.',
    },
    video: {
      nombre: 'Video / Contenido / Creadores',
      foco: 'ThruPlays, curva de retención (3s→10s→25%→75%→100%), costo por reproducción completa y engagement de video',
      metricas_principales: 'ThruPlays, Video views 3s, Retención 75%, Tiempo promedio de vista, Costo por ThruPlay, Engagement',
      metricas_ignorar: 'NO evaluar ROAS ni CPA como métricas principales. No penalizar CTR bajo si las visualizaciones son altas.',
      benchmarks: 'Retención al 75% >20% es excelente. Drop entre 3s y 10s >50% indica hook débil. ThruPlay rate >15% es saludable. Costo por ThruPlay comparar contra objetivo de awareness del creador.',
      acciones_prioritarias: 'Optimizar los primeros 3 segundos del video (hook), testear diferentes miniaturas, usar subtítulos para retención en silencio, segmentar por intereses afines al contenido, crear lookalike de viewers de alto engagement.',
    },
  };
...
## PERFIL DEL CLIENTE: ${profile.nombre.toUpperCase()}
Este es el contexto más importante para tu análisis. RESPETARLO ES OBLIGATORIO.

FOCO DEL ANÁLISIS: ${profile.foco}
MÉTRICAS PRINCIPALES (las que más importan): ${profile.metricas_principales}
RESTRICCIONES: ${profile.metricas_ignorar}
BENCHMARKS PARA ESTE PERFIL: ${profile.benchmarks}
ACCIONES PRIORITARIAS PARA ESTE PERFIL: ${profile.acciones_prioritarias}

Cualquier análisis, diagnóstico o recomendación DEBE estar alineado con este perfil. No apliques lógica de ecommerce a un cliente de mensajería, ni lógica de awareness a uno de conversiones.
` : `
## PERFIL DEL CLIENTE: NO DEFINIDO
El perfil del cliente no fue configurado. Analizá con criterio general pero indicá en el resumen que se recomienda configurar el perfil para análisis más precisos.
`;
...
// Métricas por período (7d / 14d / este mes)
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    let timeRange;
    if (period === 'yesterday') {
      const daysAgo1 = new Date(today); daysAgo1.setUTCDate(today.getUTCDate()-1);
      timeRange = encodeURIComponent(JSON.stringify({ since: fmt(daysAgo1), until: fmt(daysAgo1) }));
    } else if (period === '14d') {
      timeRange = encodeURIComponent(JSON.stringify({ since: fmt(daysAgo(14)), until: fmt(daysAgo(1)) }));
    } else if (period === 'month') {
      const firstDay = new Date(Date.UTC(today.getUTCFullYear(), today.getUTCMonth(), 1));
      timeRange = encodeURIComponent(JSON.stringify({ since: fmt(firstDay), until: fmt(daysAgo(1)) }));
    } else {
      timeRange = encodeURIComponent(JSON.stringify({ since: fmt(daysAgo(7)), until: fmt(daysAgo(1)) }));
    }

    const base = '/' + accountId + '/insights?fields=' + fields;
    const [data, campaigns] = await Promise.all([
      metaFetch(base + '&time_range=' + timeRange + '&level=account&', token),
      metaFetch(base + ',campaign_id,campaign_name&time_range=' + timeRange + '&level=campaign&limit=50&', token),
    ]);

    console.log(`[metrics] ${period} | rows: ${data.data?.length} | spend: ${data.data?.[0]?.spend}`);
