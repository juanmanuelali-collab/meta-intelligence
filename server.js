...
app.get('/api/:slug/creatives', async (req, res) => {
  ...
  try {
    ...
    // UNA SOLA llamada: insights con campo ad_id para buscar creatives después
    // Limitamos a 15 anuncios para máxima velocidad
    const [adsData, adsInfo] = await Promise.all([
      // Métricas de rendimiento
      metaFetch(
        '/' + accountId + '/insights?fields=ad_id,ad_name,adset_name,campaign_name,spend,impressions,clicks,ctr,cpm,reach,frequency,actions&date_preset=' + preset + '&level=ad&sort=spend_descending&limit=15&',
        token
      ),
      // Creatives en batch — una sola llamada trayendo thumbnail_url de todos los ads
      metaFetch(
        '/' + accountId + '/ads?fields=id,name,creative{id,thumbnail_url,image_url,title,body,video_id}&effective_status=["ACTIVE","PAUSED"]&limit=50&',
        token
      ).catch(() => ({ data: [] }))
    ]);
    ...
    // Mapear creatives por ad_id para lookup O(1)
    const creativeMap = {};
    (adsInfo.data || []).forEach(ad => {
      if (ad.creative) creativeMap[ad.id] = ad.creative;
    });
    ...
    const creatives = ads.map(ad => {
      ...
      // Creative desde el mapa (sin llamada extra)
      const c         = creativeMap[ad.ad_id] || {};
      const imageUrl  = c.thumbnail_url || c.image_url || null;
      ...
      for (const k of resultChecks) {
        ...
        if (a && parseInt(a.value) > 0) {
          mainResult = {
            type:  k,
            label: resultLabels[k] || k,
            value: parseInt(a.value),
            cpa:   spend > 0 ? spend / parseInt(a.value) : null,
          };
          break;
        }
      }
