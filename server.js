...
app.put('/api/admin/clients/:slug', async (req, res) => {
  const slug = req.params.slug;
  const { password, name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType } = req.body;
  if (!checkAdminPassword(password)) return res.status(401).json({ error: 'No autorizado' });
  const ok = await updateClientRecord(slug, { name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType });
