...
const { fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts, fetchCampaignMetricsRange, aggregateCampaignRows, fetchTopKeywords, fetchConversionActionsSummary, fetchAllCampaignsStructure } = require('./google_ads'); // Fase 1 — conector Google Ads
...
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  try {
    ...
    const byChannel = Object.entries(byChannelPlanned).map(([channel, total]) => ({
...
app.post('/api/:slug/campaign-plans/period/:sourcePeriodId/copy-to', async (req, res) => {
  ...
  try {
    ...
    const cloned = sourceItems.map(({ id, plan_period_id, created_at, updated_at, campaign_id, match_confidence, status, pending_campaign_id, pending_matched_name, pending_real_status, start_date, end_date, calculated_days, dias, budget_mensual, ...rest }) => {
      ...
      return computeItemBudgetFields(base);
    });
    const { error: itemsError } = await supabase.from('campaign_plan_items').insert(cloned);
    if (itemsError) throw new Error(itemsError.message);

    res.json({ ok: true, plan: newPlan, copied: cloned.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
...
app.post('/api/:slug/campaign-plans/master/save-to-month', async (req, res) => {
  ...
  try {
    ...
    if (toInsert.length) {
      const { error: insertError } = await supabase.from('campaign_plan_items').insert(toInsert);
