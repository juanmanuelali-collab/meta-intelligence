...
async function fetchLiveCampaignsForChannel(slug, channel) {
  if (channel === 'meta') {
    ...
    return (data.data || []).filter(c => c.effective_status !== 'DELETED').map(c => ({ id: String(c.id), name: c.name || '(sin nombre)' }));
  }
  ...
  try {
    ...
    for (const item of items) {
      ...
      if (newConfidence !== item.match_confidence || match.campaign_id !== item.campaign_id) {
        await supabase.from('campaign_plan_items').update({
          campaign_id: match.campaign_id, match_confidence: newConfidence, updated_at: new Date().toISOString(),
        }).eq('id', item.id);
        updated++;
      }
      if (match.campaign_id) matched++;
    }
    res.json({ updated, matched, total: items.length });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
