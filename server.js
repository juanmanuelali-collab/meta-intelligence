...
const { loadClientDB, verifyPassword, checkPassword, getMetaCredentials, updateMetaToken, getGoogleAdsAccountId, getGA4PropertyId, getSheetsId, setSheetsId, createClientRecord, setMetaCredentials, setGoogleAdsAccountId, setGA4PropertyId, updateClientRecord, getClientCredentialsSummary, supabase } = require('./db'); // Sub-pasos 3.1/3.2/3.3 — auth + credenciales vía Supabase
const { fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts, fetchCampaignMetricsRange, aggregateCampaignRows } = require('./google_ads'); // Fase 1 — conector Google Ads
const { fetchGA4WeeklyMetrics, fetchGA4AcquisitionForDate, fetchGA4SiteMetricsForDate, fetchGA4AcquisitionForRange, fetchGA4SiteMetricsForRange, fetchGA4TopPages, fetchGA4TopItems, fetchGA4FunnelCounts, GA4_FUNNEL_EVENTS, fetchGA4TopItemsDetailed, fetchGA4DeviceBreakdown, fetchGA4Demographics } = require('./ga4'); // Integración GA4 — tráfico y eventos por semana (post-Fase 5)
...
// cliente (ej. columnas extra que el cliente ya tenía, como "Link a
// Creativos", que nuestro sistema no usa pero no debe romper la lectura).
const PLAN_COLUMNS = {
  'nombre de campaña': 'nombre_campana', 'canal': 'canal', 'medio/tipo': 'medio_tipo',
  'fecha inicio': 'fecha_inicio', 'fecha fin': 'fecha_fin', 'presupuesto diario': 'presupuesto_diario',
  'objetivo': 'objetivo', 'target audiencia': 'target_audiencia', 'status': 'status',
  'kpi a medir': 'kpi_a_medir', 'notas': 'notas',
};

// Lee la primera hoja de la planilla del cliente y devuelve las filas como
// objetos {nombre_campana, canal, ...}, mapeando por el nombre de columna
// del encabezado (fila 1), sin asumir un orden fijo.
async function readCampaignPlanFromSheet(sheetId) {
  const sheets = await getSheetsClient();
  if (!sheets) throw new Error('No se pudo autenticar con Google Sheets — revisar GOOGLE_SERVICE_ACCOUNT.');
  const meta = await sheets.spreadsheets.get({ spreadsheetId: sheetId });
  const firstTabName = meta.data.sheets?.[0]?.properties?.title;
  if (!firstTabName) throw new Error('La planilla no tiene ninguna pestaña.');
  const res = await sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range: `'${firstTabName}'!A1:Z500` });
  const values = res.data.values || [];
  if (!values.length) return { tabName: firstTabName, rows: [] };
  const headerRow = values[0].map(h => (h || '').trim().toLowerCase());
  const colMap = headerRow.map(h => PLAN_COLUMNS[h] || null);
  const rows = values.slice(1)
    .filter(r => r.some(c => (c || '').trim() !== ''))
    .map(r => {
      const row = {};
      colMap.forEach((key, i) => { if (key) row[key] = (r[i] || '').trim(); });
      return row;
    });
  return { tabName: firstTabName, rows };
}

// Crea una pestaña nueva en la misma planilla (ej. "Agosto 2026") con el
// plan propuesto — nunca pisa la pestaña existente, siempre agrega una nueva.
async function writeCampaignPlanDraftToSheet(sheetId, tabName, rows) {
  const sheets = await getSheetsClient();
  if (!sheets) throw new Error('No se pudo autenticar con Google Sheets — revisar GOOGLE_SERVICE_ACCOUNT.');
  ...
  const header = ['Nombre de Campaña', 'Canal', 'Medio/Tipo', 'Fecha Inicio', 'Fecha Fin', 'Presupuesto Diario', 'Objetivo', 'Target Audiencia', 'Status', 'KPI a Medir', 'Notas'];
  const values = [header, ...rows.map(r => [
    r.nombre_campana || '', r.canal || '', r.medio_tipo || '', r.fecha_inicio || '', r.fecha_fin || '',
    r.presupuesto_diario ?? '', r.objetivo || '', r.target_audiencia || '', r.status || 'Propuesta IA',
    r.kpi_a_medir || '', r.notas || '',
  ])];
...
async function aggregateChannelPeriod(slug, source, start, end) {
  const EMPTY = { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null, reach: 0, impressions: 0, clicks: 0, frequency: null, engagement: 0, messages: 0, cpmsg: null };
  try {
    if (source === 'meta') {
      const creds = await getMetaCredentials(slug);
    ...
    if (source === 'google_ads') {
      ...
      return {
        ...
        reach: 0, impressions: Math.round(agg.impressions || 0), clicks: Math.round(agg.clicks || 0),
...
async function computeReportFacts(client, clientId, slug, periodType) {
  ...
  const objectiveMap = await getCampaignObjectiveMap(clientId);
  const hasAnyClassified = { leads: Object.values(objectiveMap).includes('leads'), ecommerce: Object.values(objectiveMap).includes('ecommerce') };

  const [
    metaCur, metaPrev, googleCur, googlePrev,
    leadsMetaCur, leadsMetaPrev, leadsGoogleCur, leadsGooglePrev,
    ecomAdsMetaCur, ecomAdsMetaPrev, ecomAdsGoogleCur, ecomAdsGooglePrev,
    ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail,
  ] = await Promise.all([
    ...
    aggregateChannelByObjective(slug, 'meta', 'leads', objectiveMap, range.start, range.end),
    aggregateChannelByObjective(slug, 'meta', 'leads', objectiveMap, range.prevStart, range.prevEnd),
    aggregateChannelByObjective(slug, 'google_ads', 'leads', objectiveMap, range.start, range.end),
    aggregateChannelByObjective(slug, 'google_ads', 'leads', objectiveMap, range.prevStart, range.prevEnd),
    aggregateChannelByObjective(slug, 'meta', 'ecommerce', objectiveMap, range.start, range.end),
    aggregateChannelByObjective(slug, 'meta', 'ecommerce', objectiveMap, range.prevStart, range.prevEnd),
    aggregateChannelByObjective(slug, 'google_ads', 'ecommerce', objectiveMap, range.start, range.end),
    aggregateChannelByObjective(slug, 'google_ads', 'ecommerce', objectiveMap, range.prevStart, range.prevEnd),
    fetchEcommercePeriodMetrics(clientId, periodType, range),
    fetchEcommercePeriodMetrics(clientId, periodType, { start: range.prevStart, end: range.prevEnd }),
    fetchReportFunnel(slug, range.start, range.end),
    fetchEcommerce12MonthEvolution(clientId),
    fetchDailyChannelTotals(clientId, range.start, range.end),
    fetchAnalyticsDetail(slug, hasAnyClassified.ecommerce, range.start, range.end),
  ]);

  const hasEcommerceCampaigns = hasAnyClassified.ecommerce && ((ecomAdsMetaCur.spend || 0) + (ecomAdsGoogleCur.spend || 0) > 0);
  const hasLeadsCampaigns = hasAnyClassified.leads && ((leadsMetaCur.spend || 0) + (leadsGoogleCur.spend || 0) > 0);
  ...
  // ROAS/CPA por canal — SOLO sobre las campañas clasificadas como eCommerce
  // (ROAS no tiene sentido mezclado con campañas de leads, que suelen no tener revenue real).
  const ecomChannels = [
    { key: 'meta', label: 'Meta Ads', cur: ecomAdsMetaCur, prev: ecomAdsMetaPrev },
    { key: 'google_ads', label: 'Google Ads', cur: ecomAdsGoogleCur, prev: ecomAdsGooglePrev },
  ];
  for (const ch of ecomChannels) {
    ...
    if (ch.cur.roas != null && ch.cur.roas < chRoasMin) {
      facts.push({ tipo: 'critico', canal: ch.label, hecho: `ROAS del período (campañas eCommerce): ${ch.cur.roas} (objetivo: ${chRoasMin})${roasDelta != null ? `, ${roasDelta >= 0 ? 'subió' : 'bajó'} ${Math.abs(roasDelta)}% vs. el período anterior` : ''}`, confianza: 'alta', impacto_estimado: null });
    } else if (ch.cur.roas != null && ch.cur.roas >= chRoasMin * 1.3) {
      facts.push({ tipo: 'escalar', canal: ch.label, hecho: `ROAS del período (campañas eCommerce): ${ch.cur.roas}, muy por encima del objetivo (${chRoasMin})`, confianza: 'alta', impacto_estimado: null });
    }
    if (roasDelta != null && Math.abs(roasDelta) >= 15 && ch.cur.roas >= chRoasMin) {
      facts.push({ tipo: roasDelta > 0 ? 'oportunidad' : 'critico', canal: ch.label, hecho: `El ROAS ${roasDelta > 0 ? 'mejoró' : 'empeoró'} ${Math.abs(roasDelta)}% vs. el período anterior (${ch.prev.roas ?? 'N/D'} → ${ch.cur.roas})`, confianza: 'media', impacto_estimado: null });
    }
    if (cpaMax != null && ch.cur.cpa != null && ch.cur.cpa > cpaMax) {
      facts.push({ tipo: 'critico', canal: ch.label, hecho: `CPA del período: $${Math.round(ch.cur.cpa).toLocaleString('es-AR')} (objetivo: $${cpaMax.toLocaleString('es-AR')} máximo)`, confianza: 'alta', impacto_estimado: null });
    }
  }

  // ROAS combinado (Meta+Google, solo campañas eCommerce) vs. su propio objetivo
  if (targets.roas_min_combinado != null && hasEcommerceCampaigns) {
    const combSpend = (ecomAdsMetaCur.spend || 0) + (ecomAdsGoogleCur.spend || 0);
    const combRevenue = (ecomAdsMetaCur.revenue || 0) + (ecomAdsGoogleCur.revenue || 0);
    ...
    if (combRoas != null && combRoas < roasMinCombinado) {
      facts.push({ tipo: 'critico', canal: 'General', hecho: `ROAS combinado del período (Meta+Google, eCommerce): ${combRoas} (objetivo: ${roasMinCombinado})`, confianza: 'alta', impacto_estimado: null });
    } else if (combRoas != null && combRoas >= roasMinCombinado * 1.2) {
      facts.push({ tipo: 'oportunidad', canal: 'General', hecho: `ROAS combinado del período (Meta+Google, eCommerce): ${combRoas}, por encima del objetivo (${roasMinCombinado})`, confianza: 'media', impacto_estimado: null });
    }
  }

  // Alcance y notoriedad — cuenta completa (todas las campañas, sin importar objetivo clasificado).
  const reachData = {
    reach: (metaCur.reach || 0), impressions: (metaCur.impressions || 0) + (googleCur.impressions || 0),
    clicks: (metaCur.clicks || 0) + (googleCur.clicks || 0), frequency: metaCur.frequency,
    engagement: (metaCur.engagement || 0),
  };
  // Mensajes recibidos (Meta) — solo si hay campañas clasificadas como Leads.
  const messagesData = hasLeadsCampaigns ? { messages: metaCur.messages || 0, cpmsg: metaCur.cpmsg } : null;

  // Comparación entre canales eCommerce (para las burbujas + oportunidad de reasignar presupuesto)
  if (ecomAdsMetaCur.roas != null && ecomAdsGoogleCur.roas != null && ecomAdsMetaCur.spend && ecomAdsGoogleCur.spend) {
    const best = ecomAdsMetaCur.roas >= ecomAdsGoogleCur.roas ? { label: 'Meta Ads', roas: ecomAdsMetaCur.roas } : { label: 'Google Ads', roas: ecomAdsGoogleCur.roas };
    const worst = ecomAdsMetaCur.roas >= ecomAdsGoogleCur.roas ? { label: 'Google Ads', roas: ecomAdsGoogleCur.roas } : { label: 'Meta Ads', roas: ecomAdsMetaCur.roas };
    if (best.roas >= worst.roas * 1.4) {
      facts.push({ tipo: 'oportunidad', canal: 'Meta Ads vs. Google Ads', hecho: `${best.label} rinde ${best.roas}x contra ${worst.roas}x de ${worst.label} en el mismo período (campañas eCommerce) — hay margen para reasignar presupuesto hacia ${best.label}`, confianza: 'media', impacto_estimado: null });
    }
  }

  // eCommerce del negocio (facturación/pedidos manuales, siempre se intenta — no depende de campañas clasificadas)
  if (ecomCur) {
    ...
    if (facturacionDelta != null && Math.abs(facturacionDelta) >= 10) {
      facts.push({ tipo: facturacionDelta > 0 ? 'oportunidad' : 'critico', canal: 'eCommerce', hecho: `Facturación del período: $${Math.round(ecomCur.facturacion).toLocaleString('es-AR')}, ${facturacionDelta > 0 ? 'subió' : 'bajó'} ${Math.abs(facturacionDelta)}% vs. el período anterior`, confianza: 'alta', impacto_estimado: null });
    }
    if (ticketDelta != null && Math.abs(ticketDelta) >= 8) {
      facts.push({ tipo: ticketDelta > 0 ? 'oportunidad' : 'critico', canal: 'eCommerce', hecho: `Ticket promedio ${ticketDelta > 0 ? 'subió' : 'bajó'} ${Math.abs(ticketDelta)}% (de $${Math.round(ticketPrev).toLocaleString('es-AR')} a $${Math.round(ticketCur).toLocaleString('es-AR')})`, confianza: 'media', impacto_estimado: null });
    }
    if (convDelta != null && Math.abs(convDelta) >= 10) {
      facts.push({ tipo: convDelta > 0 ? 'oportunidad' : 'critico', canal: 'eCommerce', hecho: `Tasa de conversión del sitio ${convDelta > 0 ? 'mejoró' : 'empeoró'} ${Math.abs(convDelta)}% vs. el período anterior`, confianza: 'media', impacto_estimado: null });
    }
    if (targets.tasa_conversion_min != null && ecomCur.tasa_conversion_ecommerce != null) {
      ...
      if (ecomCur.tasa_conversion_ecommerce < convMin) {
        facts.push({ tipo: 'critico', canal: 'eCommerce', hecho: `Tasa de conversión del sitio: ${ecomCur.tasa_conversion_ecommerce}% (objetivo: ${convMin}% mínimo)`, confianza: 'alta', impacto_estimado: null });
      }
    }
  }

  // Leads Performance: costo por lead combinado (solo campañas clasificadas como Leads) vs. su propio objetivo
  if (hasLeadsCampaigns && targets.costo_lead_max != null) {
    const combSpend = (leadsMetaCur.spend || 0) + (leadsGoogleCur.spend || 0);
    const combConv = (leadsMetaCur.conversions || 0) + (leadsGoogleCur.conversions || 0);
    const cpl = combConv > 0 ? +(combSpend / combConv).toFixed(0) : null;
    const cplMax = Number(targets.costo_lead_max);
    if (cpl != null && cpl > cplMax) {
      facts.push({ tipo: 'critico', canal: 'General', hecho: `Costo por lead combinado del período: $${cpl.toLocaleString('es-AR')} (objetivo: $${cplMax.toLocaleString('es-AR')} máximo)`, confianza: 'alta', impacto_estimado: null });
    } else if (cpl != null && cpl <= cplMax * 0.7) {
      facts.push({ tipo: 'oportunidad', canal: 'General', hecho: `Costo por lead combinado del período: $${cpl.toLocaleString('es-AR')}, muy por debajo del objetivo ($${cplMax.toLocaleString('es-AR')})`, confianza: 'media', impacto_estimado: null });
    }
  }
  ...
  if (funnel) {
    ...
    if (worst && worst.users_lost_pct >= 35) {
      ...
      facts.push({ tipo: 'critico', canal: 'Analytics', hecho: `El mayor quiebre del funnel está entre el paso ${idx} y "${worst.event}": se pierde ${worst.users_lost_pct}% de los usuarios (${worst.users_lost} personas) en el período`, confianza: 'media', impacto_estimado: null });
    }
  }

  return {
    range, facts, metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail, reachData, messagesData,
    leadsMetaCur, leadsMetaPrev, leadsGoogleCur, leadsGooglePrev, ecomAdsMetaCur, ecomAdsMetaPrev, ecomAdsGoogleCur, ecomAdsGooglePrev,
    hasEcommerceCampaigns, hasLeadsCampaigns,
  };
}
...
async function runReportAgent(client, factsData, contextNote, panoramaTheme) {
  const {
    facts, range, funnel, ecomCur, ecomPrev, reachData, messagesData,
    leadsMetaCur, leadsMetaPrev, leadsGoogleCur, leadsGooglePrev,
    ecomAdsMetaCur, ecomAdsMetaPrev, ecomAdsGoogleCur, ecomAdsGooglePrev,
    hasEcommerceCampaigns, hasLeadsCampaigns,
  } = factsData;
  const negocioParts = [];
  if (hasEcommerceCampaigns) negocioParts.push('un eCommerce');
  if (hasLeadsCampaigns) negocioParts.push('una empresa de generación de leads (servicios)');
  const negocio = negocioParts.length ? negocioParts.join(' y también ') : 'un negocio con campañas de marketing digital';
  ...
  if (!facts.length) {
    return {
      ...
      stories: [],
      leads_narrative: '', ecommerce_narrative: '',
      funnel_narrative: '', conversion_insight: '', reach_narrative: '', messages_narrative: '',
      actions: [
        { titulo: 'Testear nuevas variantes de creatividad', motivo: 'Renovar el creative set ayuda a sostener el rendimiento antes de que aparezca fatiga.', impacto: '' },
        { titulo: 'Revisar la segmentación activa', motivo: 'Confirmar que los públicos siguen alineados al objetivo del negocio.', impacto: '' },
        { titulo: 'Auditar la velocidad de carga del sitio', motivo: 'Una mejora de performance ayuda a sostener la conversión.', impacto: '' },
        { titulo: 'Revisar el copy de las landing pages', motivo: 'Pequeños ajustes de mensaje pueden mejorar la conversión sin más inversión.', impacto: '' },
        { titulo: 'Mantener el monitoreo semanal', motivo: 'Sostener la cadencia de revisión para detectar cambios apenas aparezcan.', impacto: '' },
      ],
    };
  }
  const factsText = facts.map((f, i) => `${i + 1}. [${f.tipo.toUpperCase()}] (${f.canal}) ${f.hecho} — Confianza: ${f.confianza}`).join('\n');
  ...
  // Dos bloques de métricas SEPARADOS por objetivo — nunca mezclan campañas de leads con campañas de venta.
  const ecomChannelsText = hasEcommerceCampaigns ? [chanSummary('Meta Ads (eCommerce)', ecomAdsMetaCur, ecomAdsMetaPrev), chanSummary('Google Ads (eCommerce)', ecomAdsGoogleCur, ecomAdsGooglePrev)].join('\n') : null;
  const leadsChannelsText = hasLeadsCampaigns ? [chanSummary('Meta Ads (Leads)', leadsMetaCur, leadsMetaPrev), chanSummary('Google Ads (Leads)', leadsGoogleCur, leadsGooglePrev)].join('\n') : null;
  ...
  const reachText = (reachData && (reachData.reach || reachData.impressions))
    ? `Alcance (Meta): ${reachData.reach.toLocaleString('es-AR')} personas. Impresiones totales: ${reachData.impressions.toLocaleString('es-AR')}. Clics totales: ${reachData.clicks.toLocaleString('es-AR')}.${reachData.frequency != null ? ` Frecuencia promedio: ${reachData.frequency}.` : ''}${reachData.engagement ? ` Interacciones con las publicaciones: ${reachData.engagement.toLocaleString('es-AR')}.` : ''}`
    : null;
  ...
  const prompt = `Sos un agente experto en leer reportes de campañas de marketing digital (Meta Ads, Google Ads, Analytics) para la agencia Docta Nexus. Vas a redactar, en español rioplatense (voseo), el informe de ${range.label} para "${client.name}", ${negocio}.
...
${factsText}
${ecomChannelsText ? `\nMétricas crudas — campañas de eCommerce (Meta + Google, para el anclaje "Meta Ads + Google Ads — eCommerce"):\n${ecomChannelsText}\n` : ''}${leadsChannelsText ? `\nMétricas crudas — campañas de Leads (Meta + Google, para el anclaje "Meta Ads + Google Ads — Leads"):\n${leadsChannelsText}\n` : ''}${funnelText ? `\nFunnel de eCommerce del período (para el análisis de fricción):\n${funnelText}\n` : ''}${convText ? `\nConversión del sitio (para el análisis de conversión):\n${convText}\n` : ''}${reachText ? `\nAlcance y notoriedad del período (para la sección de marca):\n${reachText}\n` : ''}${messagesText ? `\nMensajes recibidos del período (para la sección de mensajería):\n${messagesText}\n` : ''}${panoramaTheme ? `\nTEMA A DESTACAR EN EL PANORAMA (headline/dek) — el operador de la agencia pidió específicamente orientar la apertura del informe hacia esto, sin inventar ni forzar un hecho que no esté arriba:\n"${panoramaTheme}"\n` : ''}${contextNote ? `\nCONTEXTO ADICIONAL QUE TE DEJÓ EL OPERADOR DE LA AGENCIA (podés usarlo para decidir qué priorizar, enfatizar o mencionar — pero NUNCA para ocultar o inventar un hecho real de los de arriba; si pide algo que contradice un hecho, priorizá siempre la verdad de los datos):\n"${contextNote}"\n` : ''}
Con esto generá:
1. "headline": 1 sola oración editorial, la más importante del período${panoramaTheme ? ' — priorizá el tema que pidió el operador si hay un hecho real que lo respalde' : ''}.
2. "dek": 2-3 oraciones ampliando el headline, conectando los hechos entre sí si corresponde.
3. "stories": elegí las 3-5 más relevantes de la lista de hechos (no hace falta usarlas todas si hay muchas, ni inventar si hay pocas), en el orden que vos decidas que tiene más sentido para contar la historia del período. Cada una con: "titulo" (corto), "motivo" (por qué pasó, 1-2 oraciones), "analisis" (2-3 oraciones de contexto/profundidad), "accion" (la acción concreta más importante), "consejos" (array de 2 tips accionables adicionales), "prioridad" ("alta" si el hecho es CRITICO, "media" si es OPORTUNIDAD, "baja" si es ESCALAR), "confianza" (copiá EXACTO el valor de Confianza del hecho).
4. "actions": el "Plan de Optimización" — SIEMPRE 5 o más acciones, en orden de impacto. Si hay menos de 5 hechos que ameriten una acción propia, completá con buenas prácticas de optimización relevantes para el negocio (testeo de creatividades, segmentación, landing pages, velocidad del sitio, etc.) — está bien que sean recomendaciones generales de buenas prácticas siempre que no inventes un número o dato que no esté en los hechos. Cada una con "titulo", "motivo" (1 oración) e "impacto" (frase corta cualitativa, o con el número si viene de un hecho real — nunca inventada; dejar vacío "" si es una recomendación general sin impacto medible).
${ecomChannelsText ? `"ecommerce_narrative": 3-5 oraciones contando cómo le fue a Meta Ads Y Google Ads JUNTOS en sus campañas de eCommerce este período (comparalos entre sí si tiene sentido).\n` : ''}${leadsChannelsText ? `"leads_narrative": 3-5 oraciones contando cómo le fue a Meta Ads Y Google Ads JUNTOS en sus campañas de Leads este período (comparalos entre sí si tiene sentido).\n` : ''}${funnelText ? `"funnel_narrative": 3-4 oraciones en tono amigable (no técnico, pensado para que lo entienda el dueño del negocio, no un analista) explicando en qué paso del funnel hay más fricción/pérdida de gente y por qué podría estar pasando ahí puntualmente.\n` : ''}${convText ? `"conversion_insight": 3-4 oraciones explicando qué significa la tasa de conversión del sitio, por qué esa variación importa para el negocio, y 1-2 recomendaciones concretas para mejorarla.\n` : ''}${reachText ? `"reach_narrative": 3-4 oraciones sobre notoriedad de marca — alcance, impresiones, frecuencia y engagement — SIN mezclar con conversión/ROAS, es una historia de "cuánta gente te vio" no de "cuánta gente compró".\n` : ''}${messagesText ? `"messages_narrative": 3-4 oraciones sobre los mensajes/conversaciones recibidas y su costo, en tono orientado a leads/consultas, no a ventas directas.\n` : ''}
Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]${ecomChannelsText ? ',"ecommerce_narrative":"..."' : ''}${leadsChannelsText ? ',"leads_narrative":"..."' : ''}${funnelText ? ',"funnel_narrative":"..."' : ''}${convText ? ',"conversion_insight":"..."' : ''}${reachText ? ',"reach_narrative":"..."' : ''}${messagesText ? ',"messages_narrative":"..."' : ''}}`;
  ...
  return {
    ...
    leads_narrative: parsed.leads_narrative || '', ecommerce_narrative: parsed.ecommerce_narrative || '',
    ...
    stories: parsed.stories || [], actions,
  };
}
...
async function generateClientReport(slug, periodType, options = {}) {
  const { context_note, panorama_theme, hero_image_url, closing_image_url, recipient_name, business_name_override } = options;
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) throw new Error('Cliente no encontrado: ' + slug);
  client.slug = slug;
  ...
  // El "profile" para efectos de la plantilla (secciones a mostrar) se infiere de qué
  // campañas hay clasificadas — ya no es una elección manual del operador.
  const profile = factsData.hasEcommerceCampaigns && factsData.hasLeadsCampaigns ? 'hibrido'
    : factsData.hasEcommerceCampaigns ? 'ecommerce'
    : factsData.hasLeadsCampaigns ? 'leads' : 'ecommerce';

  const reportData = {
    profile, period_type: periodType, period_label: factsData.range.label,
    headline: agent.headline, dek: agent.dek, stories: agent.stories, actions: agent.actions,
    leads_narrative: agent.leads_narrative, ecommerce_narrative: agent.ecommerce_narrative,
    ...
    has_ecommerce_campaigns: factsData.hasEcommerceCampaigns, has_leads_campaigns: factsData.hasLeadsCampaigns,
    leads_meta: factsData.leadsMetaCur, leads_meta_prev: factsData.leadsMetaPrev,
    leads_google: factsData.leadsGoogleCur, leads_google_prev: factsData.leadsGooglePrev,
    ecomads_meta: factsData.ecomAdsMetaCur, ecomads_meta_prev: factsData.ecomAdsMetaPrev,
    ecomads_google: factsData.ecomAdsGoogleCur, ecomads_google_prev: factsData.ecomAdsGooglePrev,
...
app.post('/api/:slug/report/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type, context_note, panorama_theme, hero_image_url, closing_image_url, recipient_name, business_name_override } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['semana', 'mes_anterior', 'mes_actual'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const report = await generateClientReport(slug, period_type, { context_note, panorama_theme, hero_image_url, closing_image_url, recipient_name, business_name_override });
    res.json({
...
// ═══════════════════════════════════════════════════════════════
// ── PLAN DE CAMPAÑAS ── la planilla de Google Sheets de CADA CLIENTE
// es la única fuente real (no hay copia en Supabase). Solo se guarda
// en Supabase el VÍNCULO entre una fila del Sheet (por nombre de
// campaña) y la campaña real de Meta/Google, para no tener que
// re-vincular cada vez que se abre la solapa.
// ═══════════════════════════════════════════════════════════════

// Trae los vínculos guardados de un cliente, como mapa nombre_campana -> vínculo
async function getPlanLinks(clientId) {
  const { data } = await supabase.from('campaign_plans').select('nombre_campana, linked_source, linked_campaign_id, linked_campaign_name').eq('client_id', clientId);
  const map = {};
  (data || []).forEach(r => { map[r.nombre_campana] = r; });
  return map;
}

// Leer el plan: en vivo desde el Sheet del cliente, + vínculos guardados + gasto real del mes en curso
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  if (!sheetId) return res.status(400).json({ error: 'Este cliente todavía no tiene una planilla de Plan de Campañas configurada (cargarla en /admin).' });
  try {
    const { tabName, rows } = await readCampaignPlanFromSheet(sheetId);
    const links = await getPlanLinks(clientDB.id);
    const range = computeReportPeriodRange('mes_actual');
    const enriched = await Promise.all(rows.map(async row => {
      const link = links[row.nombre_campana];
      if (!link || !link.linked_source || !link.linked_campaign_id) return { ...row, ...link };
      const actual = await fetchCampaignActuals(slug, link.linked_source, link.linked_campaign_id, range.start, range.end).catch(() => null);
      return { ...row, ...link, actual_spend: actual?.spend, actual_conversions: actual?.conversions, actual_revenue: actual?.revenue };
    }));
    res.json({ tab_name: tabName, rows: enriched });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Guardar/actualizar el vínculo manual de una fila (identificada por nombre de campaña) a una campaña real
app.post('/api/:slug/campaign-plans/link', async (req, res) => {
  const slug = req.params.slug;
  const { password, nombre_campana, linked_source, linked_campaign_id, linked_campaign_name } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  if (!nombre_campana) return res.status(400).json({ error: 'Falta nombre_campana' });
  try {
    const { error } = await supabase.from('campaign_plans').upsert(
      { client_id: clientDB.id, nombre_campana, linked_source: linked_source || null, linked_campaign_id: linked_campaign_id || null, linked_campaign_name: linked_campaign_name || null, updated_at: new Date().toISOString() },
      { onConflict: 'client_id,nombre_campana' }
    );
    if (error) throw new Error(error.message);
    res.json({ ok: true });
  } catch (e) {
...
// Listar campañas reales recientes de Meta/Google, para el desplegable de vínculo manual
// (últimos 90 días — suficiente para ver qué está activo/reciente sin traer todo el historial).
app.get('/api/:slug/campaign-plans/live-campaigns', async (req, res) => {
  ...
  } catch (e) { console.warn('[campaign-plans] Meta live-campaigns omitido:', e.message); }
  try {
    const customerId = await getGoogleAdsAccountId(slug);
    if (customerId) {
  ...
  } catch (e) { console.warn('[campaign-plans] Google live-campaigns omitido:', e.message); }
  res.json(out);
});
...
async function fetchCampaignActuals(slug, linkedSource, linkedCampaignId, since, until) {
  try {
    if (linkedSource === 'meta') {
      const creds = await getMetaCredentials(slug);
      if (!creds?.access_token || !creds?.ad_account_id) return null;
      const raw = await fetchMetaCampaignMetricsForRange(creds.ad_account_id, creds.access_token, since, until);
      const row = raw.find(r => String(r.campaign_id) === String(linkedCampaignId));
      if (!row) return null;
      const k = calcKpis(row);
      return { spend: k.spend || 0, conversions: k.purchases || k.leads || 0, revenue: k.revenue || 0 };
    }
    if (linkedSource === 'google_ads') {
      const customerId = await getGoogleAdsAccountId(slug);
      if (!customerId) return null;
      const raw = await fetchCampaignMetricsRange(customerId, since, until);
      const row = raw.find(r => String(r.campaign?.id) === String(linkedCampaignId));
      if (!row) return null;
      const m = row.metrics || {};
    ...
    console.warn('[campaign-plans] fetchCampaignActuals omitido:', e.message);
  }
  return null;
}

// Genera una PROPUESTA de plan para el mes siguiente (solo preview, no
// escribe nada todavía): lee el plan actual del Sheet + su desempeño real
// (por fila vinculada) y le pide a la IA que proponga ajustes. El resultado
// se devuelve al dashboard para previsualizar — recién se escribe al Sheet
// cuando el operador confirma explícitamente (endpoint /write-draft-to-sheet).
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  if (!sheetId) return res.status(400).json({ error: 'Este cliente todavía no tiene una planilla de Plan de Campañas configurada (cargarla en /admin).' });
  try {
    const { rows: currentRows } = await readCampaignPlanFromSheet(sheetId);
    if (!currentRows.length) return res.status(400).json({ error: 'La planilla no tiene campañas cargadas todavía — cargá al menos una fila antes de generar una propuesta.' });
    const links = await getPlanLinks(clientDB.id);

    const range = computeReportPeriodRange('mes_actual');
    const rowsWithActuals = await Promise.all(currentRows.map(async r => {
      const link = links[r.nombre_campana];
      let actual = null;
      if (link?.linked_source && link?.linked_campaign_id) {
        actual = await fetchCampaignActuals(slug, link.linked_source, link.linked_campaign_id, range.start, range.end);
      }
      return { ...r, actual };
    }));

    const planText = rowsWithActuals.map((r, i) => {
      const parts = [`${i + 1}. "${r.nombre_campana}" (${r.canal || 'canal N/D'}, status ${r.status || 'N/D'})`];
      if (r.presupuesto_diario) parts.push(`presupuesto diario planeado: $${r.presupuesto_diario}`);
      if (r.objetivo) parts.push(`objetivo: ${r.objetivo}`);
      if (r.kpi_a_medir) parts.push(`KPI a medir: ${r.kpi_a_medir}`);
      if (r.actual) parts.push(`REAL este mes: gasto $${Math.round(r.actual.spend)}, ${Math.round(r.actual.conversions)} conversiones, ingresos $${Math.round(r.actual.revenue)}`);
      else parts.push('sin vincular a una campaña real todavía');
      return parts.join(' — ');
    }).join('\n');

    const nextMonthDate = new Date(); nextMonthDate.setUTCMonth(nextMonthDate.getUTCMonth() + 1);
    const nextMonthLabel = `${MESES_ES[nextMonthDate.getUTCMonth()]} ${nextMonthDate.getUTCFullYear()}`;

    const prompt = `Sos un estratega de Paid Media de la agencia Docta Nexus. Tenés el plan de campañas actual de "${client.name}" y cómo viene rindiendo cada una este mes (datos reales, no inventes números nuevos). Tu trabajo es proponer el plan de ${nextMonthLabel}: qué campañas continuar tal cual, cuáles ajustar de presupuesto (subir si rinde bien, bajar o pausar si rinde mal), y si falta algo evidente, sugerir una campaña nueva de tipo similar a las que ya existen (sin inventar plataformas o audiencias que no tengan sentido con el resto del plan).

Plan actual y su desempeño real de este mes:
${planText}

Respondé SOLO en JSON, un array de campañas propuestas para ${nextMonthLabel}, con este formato exacto:
[{"nombre_campana":"...","canal":"meta|google_ads|otro","medio_tipo":"...","presupuesto_diario":0,"objetivo":"...","kpi_a_medir":"...","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
    ...
    const proposedRows = proposed.map(p => ({
      nombre_campana: p.nombre_campana || '(sin nombre)', canal: p.canal || '', medio_tipo: p.medio_tipo || '',
      presupuesto_diario: p.presupuesto_diario ?? '', objetivo: p.objetivo || '', kpi_a_medir: p.kpi_a_medir || '',
      notas: p.notas || '', status: 'Propuesta IA',
    }));
    res.json({ next_month_label: nextMonthLabel, suggested_tab_name: nextMonthLabel, rows: proposedRows });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});

// Confirmado por el operador: recién ACÁ se escribe la pestaña nueva en el
// Sheet real, con las filas que se previsualizaron (pueden venir editadas
// respecto a lo que propuso la IA — el operador tiene la última palabra).
app.post('/api/:slug/campaign-plans/write-draft-to-sheet', async (req, res) => {
  const slug = req.params.slug;
  const { password, tab_name, rows } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const sheetId = await getSheetsId(slug);
  if (!sheetId) return res.status(400).json({ error: 'Este cliente todavía no tiene una planilla de Plan de Campañas configurada.' });
  if (!tab_name || !rows?.length) return res.status(400).json({ error: 'Falta tab_name o rows' });
  try {
    await writeCampaignPlanDraftToSheet(sheetId, tab_name, rows);
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
