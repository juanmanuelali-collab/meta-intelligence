...
function periodLabelForPrompt(period, isoYear, isoWeek) {
  ...
  return ({ yesterday: 'ayer', '7d': 'los últimos 7 días', '14d': 'los últimos 14 días', month: 'este mes' }[period] || period);
}
...
async function computeGrowthScoreAndFacts(client, clientId, slug) {
  ...
  const missing = Object.entries(tracking).filter(([, v]) => !v.configured || !v.fresh).map(([k]) => k);
  if (missing.length) {
    const labels = { meta: 'Meta Ads', google_ads: 'Google Ads', ga4: 'Google Analytics' };
    facts.push({ tipo: 'critico', canal: 'Tracking', hecho: `Sin datos recientes o sin conectar: ${missing.map(m => labels[m] || m).join(', ')}`, confianza: 'alta', impacto_estimado: null });
  }
...
// Agrega spend/ingresos/conversiones de un canal (meta o google_ads) para un rango de fechas.
async function aggregateChannelPeriod(clientId, source, start, end) {
  const { data, error } = await supabase
    .from('ad_metrics_daily')
    .select('spend, conversions, conversion_value')
    .eq('client_id', clientId).eq('source', source)
    .gte('date', start).lte('date', end);
  if (error || !data || !data.length) return { spend: 0, conversions: 0, revenue: 0, roas: null, cpa: null };
  const spend = data.reduce((s, r) => s + Number(r.spend || 0), 0);
  const conversions = data.reduce((s, r) => s + Number(r.conversions || 0), 0);
  const revenue = data.reduce((s, r) => s + Number(r.conversion_value || 0), 0);
  return {
...
async function computeReportFacts(client, clientId, slug, profile, periodType) {
  ...
  const [metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail] = await Promise.all([
    aggregateChannelPeriod(clientId, 'meta', range.start, range.end),
    aggregateChannelPeriod(clientId, 'meta', range.prevStart, range.prevEnd),
    aggregateChannelPeriod(clientId, 'google_ads', range.start, range.end),
    aggregateChannelPeriod(clientId, 'google_ads', range.prevStart, range.prevEnd),
  ...
  for (const ch of channels) {
    ...
    if (ch.cur.roas != null && ch.cur.roas < roasMin) {
      facts.push({ tipo: 'critico', canal: ch.label, hecho: `ROAS del período: ${ch.cur.roas} (objetivo: ${roasMin})${roasDelta != null ? `, ${roasDelta >= 0 ? 'subió' : 'bajó'} ${Math.abs(roasDelta)}% vs. el período anterior` : ''}`, confianza: 'alta', impacto_estimado: null });
    } else if (ch.cur.roas != null && ch.cur.roas >= roasMin * 1.3) {
      facts.push({ tipo: 'escalar', canal: ch.label, hecho: `ROAS del período: ${ch.cur.roas}, muy por encima del objetivo (${roasMin})`, confianza: 'alta', impacto_estimado: null });
    }
    if (roasDelta != null && Math.abs(roasDelta) >= 15 && ch.cur.roas >= roasMin) {
...
async function fetchAnalyticsDetail(slug, profile, start, end) {
  ...
  const [channels, topProducts, devices, demographics] = await Promise.all([
    ...
    profile === 'ecommerce' ? fetchGA4TopItemsDetailed(propertyId, start, end, 6).catch(e => { console.warn('[reporte] productos GA4 omitidos:', e.message); return []; }) : Promise.resolve([]),
...
async function runReportAgent(client, profile, factsData) {
  const { facts, range, metaCur, metaPrev, googleCur, googlePrev } = factsData;
...
Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","channel_narratives":{"meta":"...","google":"..."},"stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]}`;

  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + OPENAI_KEY },
    body: JSON.stringify({ model: 'gpt-4o', max_tokens: 3200, messages: [{ role: 'user', content: prompt }] })
  });
  const d = await r.json();
...
async function generateClientReport(slug, profile, periodType) {
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) throw new Error('Cliente no encontrado: ' + slug);
  client.slug = slug;
  ...
  const agent = await runReportAgent(client, profile, factsData);
...
app.post('/api/:slug/report/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, profile, period_type } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  ...
  try {
    const report = await generateClientReport(slug, profile, period_type);
    res.json({
...
app.get('/api/report/:token/meta', async (req, res) => {
  try {
    const { data, error } = await supabase
      .from('client_reports')
      .select('token, expires_at, period_label, client_id, clients(name, website_url, client_rubro)')
