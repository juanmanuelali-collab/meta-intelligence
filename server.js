...
const DEFAULT_ENABLED_FEATURES = {
  ...
  informe: true, plan_campanias: true, nexus_reporte: true,
};
...
app.put('/api/admin/clients/:slug', async (req, res) => {
  const slug = req.params.slug;
  const { password, name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit, aiFeatureLimits } = req.body;
  if (!checkAdminPassword(password)) return res.status(401).json({ error: 'No autorizado' });
  ...
  const ok = await updateClientRecord(slug, { name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit: parsedLimit, aiFeatureLimits });
