...
async function fetchReportFunnel(slug, profile, start, end) {
  if (profile !== 'ecommerce' && profile !== 'hibrido') return null;
  try {
    const propertyId = await getGA4PropertyId(slug);
...
async function computeReportFacts(client, clientId, slug, profile, periodType) {
  const range = computeReportPeriodRange(periodType);

  const [metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily, analyticsDetail] = await Promise.all([
    ...
    fetchReportFunnel(slug, profile, range.start, range.end),
    (profile === 'ecommerce' || profile === 'hibrido') ? fetchEcommerce12MonthEvolution(clientId) : Promise.resolve([]),
    fetchDailyChannelTotals(clientId, range.start, range.end),
    fetchAnalyticsDetail(slug, profile, range.start, range.end),
  ]);
  ...
  // Alcance y notoriedad — datos crudos para que el agente redacte, no son "hechos" de alerta.
  const reachData = {
  ...
  // Mensajes recibidos (Meta) — solo relevante para perfiles Leads/Híbrido.
  const messagesData = (profile === 'leads' || profile === 'hibrido')
    ? { messages: metaCur.messages || 0, cpmsg: metaCur.cpmsg }
    : null;

  // Comparación entre canales (para las burbujas + para detectar oportunidad de reasignar presupuesto)
  if (metaCur.roas != null && googleCur.roas != null && metaCur.spend && googleCur.spend) {
  ...
  // eCommerce (perfil eCommerce)
  if ((profile === 'ecommerce' || profile === 'hibrido') && ecomCur) {
  ...
  // Leads Performance: costo por lead combinado vs. su propio objetivo
  if ((profile === 'leads' || profile === 'hibrido') && targets.costo_lead_max != null) {
    const combSpend = (metaCur.spend || 0) + (googleCur.spend || 0);
...
async function fetchAnalyticsDetail(slug, profile, start, end) {
  ...
  const [channels, topProducts, devices, demographics] = await Promise.all([
    ...
    profile === 'ecommerce' ? fetchGA4TopItemsDetailed(propertyId, start, end, 10).catch(e => { console.warn('[reporte] productos GA4 omitidos:', e.message); return []; }) : Promise.resolve([]),
...
async function runReportAgent(client, profile, factsData, contextNote) {
  const { facts, range, metaCur, metaPrev, googleCur, googlePrev, funnel, ecomCur, ecomPrev, reachData, messagesData } = factsData;
  const negocio = profile === 'ecommerce' ? 'un eCommerce' : profile === 'hibrido' ? 'un negocio híbrido (eCommerce + generación de leads)' : 'una empresa de generación de leads (servicios)';
  ...
  const isEcomLike = profile === 'ecommerce' || profile === 'hibrido';
  const funnelText = (isEcomLike && funnel && funnel.length)
  ...
  const convText = (isEcomLike && ecomCur && ecomCur.tasa_conversion_ecommerce != null)
  ...
  const prompt = `Sos un agente experto en leer reportes de campañas de marketing digital (Meta Ads, Google Ads, Analytics${profile === 'ecommerce' ? ', eCommerce' : ''}) para la agencia Docta Nexus. Vas a redactar, en español rioplatense (voseo), el informe de ${range.label} para "${client.name}", ${negocio}.
...
Con esto generá:
1. "headline": 1 sola oración editorial, la más importante del período — puede ser de cualquiera de los canales, elegí la que más importa, no sigas un orden fijo.
...
async function generateClientReport(slug, profile, periodType, options = {}) {
  const { context_note, hero_image_url, closing_image_url, recipient_name, business_name_override } = options;
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) throw new Error('Cliente no encontrado: ' + slug);
  client.slug = slug;

  const factsData = await computeReportFacts(client, clientDB.id, slug, profile, periodType);
  const agent = await runReportAgent(client, profile, factsData, context_note);
...
app.post('/api/:slug/report/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, profile, period_type, context_note, hero_image_url, closing_image_url, recipient_name, business_name_override } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['ecommerce', 'leads', 'hibrido'].includes(profile)) return res.status(400).json({ error: 'Perfil inválido' });
  if (!['semana', 'mes_anterior', 'mes_actual'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const report = await generateClientReport(slug, profile, period_type, { context_note, hero_image_url, closing_image_url, recipient_name, business_name_override });
    res.json({
...
// ═══════════════════════════════════════════════════════════════
// ── PLAN DE CAMPAÑAS ── lista editable por cliente (no por mes —
// las campañas tienen su propio rango, muchas "always on"; el plan
// que se le manda al cliente cada mes es un filtro de esta lista).
// ═══════════════════════════════════════════════════════════════

// Listar todas las filas del plan de un cliente
app.get('/api/:slug/campaign-plans', async (req, res) => {
  ...
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { data, error } = await supabase.from('campaign_plans').select('*').eq('client_id', clientDB.id).order('created_at', { ascending: false });
  if (error) return res.status(500).json({ error: error.message });
  const rows = data || [];
  // Para las filas vinculadas a una campaña real, sumar el gasto real del mes en curso (en vivo).
  const range = computeReportPeriodRange('mes_actual');
  const enriched = await Promise.all(rows.map(async row => {
    if (!row.linked_source || !row.linked_campaign_id) return row;
    const actual = await fetchCampaignActuals(slug, row.linked_source, row.linked_campaign_id, range.start, range.end).catch(() => null);
    return actual ? { ...row, actual_spend: actual.spend, actual_conversions: actual.conversions, actual_revenue: actual.revenue } : row;
  }));
  res.json({ rows: enriched });
});

// Crear una fila nueva del plan
app.post('/api/:slug/campaign-plans', async (req, res) => {
  const slug = req.params.slug;
  const { password, row } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  if (!row?.nombre_campana) return res.status(400).json({ error: 'Falta el nombre de la campaña' });
  const insertRow = {
    client_id: clientDB.id,
    nombre_campana: row.nombre_campana, canal: row.canal || null, medio_tipo: row.medio_tipo || null,
    fecha_inicio: row.fecha_inicio || null, fecha_fin: row.fecha_fin || null,
    presupuesto_diario: row.presupuesto_diario ?? null, objetivo: row.objetivo || null,
    target_audiencia: row.target_audiencia || null, status: row.status || 'Planeada',
    kpi_a_medir: row.kpi_a_medir || null, notas: row.notas || null,
    linked_source: row.linked_source || null, linked_campaign_id: row.linked_campaign_id || null,
    linked_campaign_name: row.linked_campaign_name || null,
  };
  const { data, error } = await supabase.from('campaign_plans').insert(insertRow).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json({ row: data });
});

// Actualizar una fila existente del plan
app.put('/api/:slug/campaign-plans/:id', async (req, res) => {
  const slug = req.params.slug;
  const { password, row } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  ...
  const { data, error } = await supabase.from('campaign_plans').update(update).eq('id', req.params.id).eq('client_id', clientDB.id).select().single();
  if (error) return res.status(500).json({ error: error.message });
  res.json({ row: data });
});

// Borrar una fila del plan
app.delete('/api/:slug/campaign-plans/:id', async (req, res) => {
  const slug = req.params.slug;
  const { password } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { error } = await supabase.from('campaign_plans').delete().eq('id', req.params.id).eq('client_id', clientDB.id);
  if (error) return res.status(500).json({ error: error.message });
  res.json({ ok: true });
});

// Confirmar (o descartar) un lote de borrador propuesto por la IA
app.post('/api/:slug/campaign-plans/confirm-draft', async (req, res) => {
  const slug = req.params.slug;
  const { password, draft_batch_id, action } = req.body; // action: 'confirm' | 'discard'
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return res.status(404).json({ error: 'Cliente no encontrado' });
  if (!draft_batch_id) return res.status(400).json({ error: 'Falta draft_batch_id' });
  try {
    if (action === 'discard') {
      await supabase.from('campaign_plans').delete().eq('client_id', clientDB.id).eq('draft_batch_id', draft_batch_id);
    } else {
      await supabase.from('campaign_plans').update({ is_draft: false, status: 'Planeada', updated_at: new Date().toISOString() })
        .eq('client_id', clientDB.id).eq('draft_batch_id', draft_batch_id);
    }
    res.json({ ok: true });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
// Genera un borrador de plan para el mes siguiente: lee las filas actuales
// (no-borrador) + su desempeño real del mes en curso, y le pide a la IA que
// proponga ajustes — SIEMPRE queda como borrador (is_draft:true), nunca pisa
// el plan activo. El operador lo revisa y confirma o descarta después.
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    const { data: currentRows, error } = await supabase.from('campaign_plans').select('*').eq('client_id', clientDB.id).eq('is_draft', false);
    if (error) throw new Error(error.message);
    if (!currentRows || !currentRows.length) return res.status(400).json({ error: 'No hay campañas cargadas en el plan todavía — cargá al menos una antes de generar una propuesta.' });

    const range = computeReportPeriodRange('mes_actual');
    const rowsWithActuals = await Promise.all(currentRows.map(async r => {
      let actual = null;
      if (r.linked_source && r.linked_campaign_id) {
        actual = await fetchCampaignActuals(slug, r.linked_source, r.linked_campaign_id, range.start, range.end);
      }
      return { ...r, actual };
    }));

    const planText = rowsWithActuals.map((r, i) => {
      const parts = [`${i + 1}. "${r.nombre_campana}" (${r.canal || 'canal N/D'}, status ${r.status})`];
      if (r.presupuesto_diario != null) parts.push(`presupuesto diario planeado: $${r.presupuesto_diario}`);
      ...
      else if (r.linked_campaign_id) parts.push('vinculada pero sin datos reales disponibles este mes');
...
[{"nombre_campana":"...","canal":"meta|google_ads|otro","medio_tipo":"...","presupuesto_diario":0,"objetivo":"...","kpi_a_medir":"...","notas":"motivo del ajuste, 1-2 oraciones, basado en el dato real de arriba"}]`;
    ...
    const draftBatchId = crypto.randomUUID();
    const draftRows = proposed.map(p => ({
      client_id: clientDB.id, nombre_campana: p.nombre_campana || '(sin nombre)', canal: p.canal || null,
      medio_tipo: p.medio_tipo || null, presupuesto_diario: p.presupuesto_diario ?? null,
      objetivo: p.objetivo || null, kpi_a_medir: p.kpi_a_medir || null, notas: p.notas || null,
      status: 'Propuesta IA', is_draft: true, draft_batch_id: draftBatchId,
    }));
    const { data: inserted, error: insertError } = await supabase.from('campaign_plans').insert(draftRows).select();
    if (insertError) throw new Error(insertError.message);
    res.json({ draft_batch_id: draftBatchId, rows: inserted });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
