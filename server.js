...
async function fetchAccountMetrics(client) {
  ...
  // Video fields directos — solo funcionan a nivel cuenta y campaña
  ...
  const [accountData, campaignData, campaignStatus, adsetData, adData] = await Promise.all([
    metaFetch('/' + accountId + '/insights?fields=' + fieldsWithVideo + '&date_preset=last_7d&level=account&', token).catch(()=>({data:[]})),
    metaFetch('/' + accountId + '/insights?fields=' + fieldsWithVideo + ',campaign_id,campaign_name&date_preset=last_7d&level=campaign&limit=50&', token).catch(()=>({data:[]})),
