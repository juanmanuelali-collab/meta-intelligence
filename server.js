  ...
  for (const slug of await listClients()) {
    try {
      ...
      for (const item of (items || [])) {
        ...
        if (match.campaign_id && match.real_status && match.real_status !== item.status) {
          await supabase.from('campaign_plan_items').update({ status: match.real_status, updated_at: new Date().toISOString() }).eq('id', item.id);
        }
      }
...
app.get('/api/:slug/campaign-plans/months', async (req, res) => {
  ...
  try {
    ...
    const months = await Promise.all(Object.values(latestByMonth).map(async p => {
      ...
      return { period: p.period, label: periodToLabel(p.period), status: p.status, semaforo, is_current: p.period === currentPeriod };
    }));
    res.json({ months });
  } catch (e) {
    res.status(500).json({ error: e.message });
...
app.post('/api/:slug/campaign-plans/period/:periodId/rematch-all', async (req, res) => {
  ...
  try {
    ...
    let updated = 0, matched = 0, statusChanged = 0;
    for (const item of items) {
      const match = await matchCampaignByName(slug, item.channel, item.campaign_name);
      const newConfidence = match.campaign_id ? match.match_confidence : 'sin_match';
      const update = {};
      if (newConfidence !== item.match_confidence || match.campaign_id !== item.campaign_id) {
        update.campaign_id = match.campaign_id;
        update.match_confidence = newConfidence;
      }
      // Sincroniza el Status del plan con el estado real de la campaña
      // (Activa/Pausada/Eliminada) — solo si hay match confirmado y el
      // estado real difiere del que tiene cargado el plan.
      if (match.campaign_id && match.real_status && match.real_status !== item.status) {
        update.status = match.real_status;
        statusChanged++;
      }
    ...
    res.json({ updated, matched, total: items.length, status_changed: statusChanged });
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
});
