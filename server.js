...
app.get('/api/:slug/creatives', async (req, res) => {
  ...
  try {
    ...
    // UNA sola llamada con todo junto — métricas + thumbnail en insights nivel ad
    const adsData = await metaFetch(
      '/' + accountId + '/insights?fields=ad_id,ad_name,adset_name,campaign_name,spend,impressions,clicks,ctr,cpm,reach,frequency,actions&date_preset=' + preset + '&level=ad&sort=spend_descending&limit=12&',
      token
    );
    ...
    // Traer thumbnails en UNA llamada batch usando IDs separados por coma
    const adIds = ads.map(a => a.ad_id).join(',');
    let thumbnailMap = {};
    try {
      const batchBody = ads.map(a => ({
        method: 'GET',
        relative_url: a.ad_id + '?fields=id,creative{thumbnail_url,image_url,video_id,title,body}'
      }));
      const batchRes = await fetch('https://graph.facebook.com/' + META_API_VER + '/?batch=' + encodeURIComponent(JSON.stringify(batchBody)) + '&access_token=' + token, {
        method: 'POST'
      });
      ...
      (batchData || []).forEach(item => {
        if (item && item.code === 200) {
          try {
            ...
            if (d.id && d.creative) thumbnailMap[d.id] = d.creative;
          } catch(e) {}
        }
      });
      console.log('[creatives] Batch OK, thumbnails:', Object.keys(thumbnailMap).length);
    } catch(e) {
      console.warn('[creatives] Batch falló, continuando sin imágenes:', e.message);
    }
    ...
    const resultChecks = Object.keys(resultLabels);

    const creatives = ads.map(ad => {
      const spend   = parseFloat(ad.spend || 0);
      const actions = ad.actions || [];
      const c       = thumbnailMap[ad.ad_id] || {};
    ...
    if (ANTHROPIC_KEY && creatives.length > 0) {
      try {
        ...
        const prompt = 'Analizá estas creatividades de Meta Ads.\n\n' + summary + '\n\nRespondé SOLO en JSON:\n{"winner":"anuncio ganador y por qué en 1 oración","loser":"peor anuncio y qué cambiarías","pattern":"patrón detectado","recommendations":["acción 1","acción 2","acción 3"],"budget_suggestion":"sugerencia de presupuesto"}';
