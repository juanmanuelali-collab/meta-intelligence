...
async function checkAiQuota(slug) {
  const usage = await getAiUsage(slug);
  if (!usage) return { blocked: false, usage: null }; // no debería pasar (checkPassword ya validó cliente activo), no bloqueamos por las dudas
  if (usage.limit !== null && usage.used >= usage.limit) {
    return {
      blocked: true,
      usage,
      message: `Alcanzaste el límite de ${usage.limit} consultas mensuales del agente IA. Se renueva el 1° del próximo mes.`,
    };
  }
...
app.get('/api/admin/clients/:slug', async (req, res) => {
  ...
  const aiUsage = await getAiUsage(slug); // null si el cliente está inactivo (getAiUsage internamente sí filtra active, es consistente con el resto de accesos "en vivo")
  res.json({
    client: {
...
app.put('/api/admin/clients/:slug', async (req, res) => {
  const slug = req.params.slug;
  const { password, name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit } = req.body;
  if (!checkAdminPassword(password)) return res.status(401).json({ error: 'No autorizado' });
  ...
  const ok = await updateClientRecord(slug, { name, clientPassword, currency, colorPrimary, colorAccent, colorBtnText, active, reportProfile, clientType, enabledFeatures, aiMonthlyLimit: parsedLimit });
...
const KPI_METRIC_DEFS = [
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'roas_global', label: 'ROAS Global', format: 'ratio', accessor: a => a.ecommerce?.roas },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'pedidos', label: 'Pedidos', format: 'number', accessor: a => a.ecommerce?.pedidos },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'facturacion', label: 'Facturación', format: 'money', accessor: a => a.ecommerce?.facturacion },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'tasa_conversion_ecommerce', label: 'Tasa de Conversión (eCommerce)', format: 'percent', accessor: a => a.ecommerce?.tasa_conversion_ecommerce },
  { group: 'global', group_label: 'Globales Sitio Web / eCommerce', key: 'ticket_promedio', label: 'Ticket Promedio', format: 'money', accessor: a => a.ecommerce?.ticket_promedio },
...
app.get('/api/:slug/kpi-targets', async (req, res) => {
  ...
  try {
    ...
    const metrics = KPI_METRIC_DEFS.map(def => ({
