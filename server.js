...
app.get('/api/:slug/creatives', async (req, res) => {
  ...
  try {
    ...
    // 1. Traer ads con métricas e id del creative
    const adsData = await metaFetch(
      '/' + accountId + '/insights?fields=ad_id,ad_name,adset_name,campaign_name,spend,impressions,clicks,ctr,cpm,reach,frequency,actions&date_preset=' + preset + '&level=ad&sort=spend_descending&limit=30&',
      token
    );

    const ads = adsData.data || [];
    if (!ads.length) return res.json({ creatives: [], period: preset });

    // 2. Para cada ad, traer su creative (thumbnail + texto)
    const creativePromises = ads.map(async ad => {
      let creative = {};
      try {
        // Primero obtener el creative_id del ad
        const adInfo = await metaFetch(
          '/' + ad.ad_id + '?fields=creative{id,thumbnail_url,image_url,title,body,object_story_spec,video_id}&',
          token
        );
        const c = adInfo.creative || {};

        // Determinar tipo y URL de imagen
        ...
        if (c.video_id) {
          ...
          if (!imageUrl) {
            try {
              const vid = await metaFetch('/' + c.video_id + '?fields=thumbnails{uri}&', token);
              imageUrl = vid.thumbnails?.data?.[0]?.uri || null;
            } catch(e) { /* fallback */ }
          }
        }
        ...
        const title   = c.title || linkData.name || linkData.message || '';
        const body    = c.body  || linkData.description || linkData.message || '';

        creative = { imageUrl, mediaType, title, body, creativeId: c.id };
      } catch(e) {
        console.warn('[creatives] Error en ad', ad.ad_id, ':', e.message);
      }
      ...
      const resultChecks = [
        ...
        'offsite_conversion.fb_pixel_purchase',
        'offsite_conversion.fb_pixel_lead',
        'lead', 'post_engagement', 'link_click',
      ];
      let mainResult = null;
      for (const k of resultChecks) {
        const a = actions.find(a => a.action_type === k);
        if (a && parseInt(a.value) > 0) {
          mainResult = {
            ...
            cpa:   spend > 0 && parseInt(a.value) > 0 ? spend / parseInt(a.value) : null,
          };
          break;
        }
      }
    ...
    // 3. Análisis IA de creatividades
    let aiAnalysis = null;
    if (ANTHROPIC_KEY && creatives.length > 0) {
      try {
        const top3    = [...creatives].sort((a,b) => b.spend - a.spend).slice(0,3);
        const bottom3 = [...creatives].sort((a,b) => a.ctr - b.ctr).slice(0,3);

        const creativeSummary = creatives.slice(0,10).map((c,i) =>
          `${i+1}. "${c.adName}" | $${Math.round(c.spend)} | CTR ${c.ctr.toFixed(2)}% | CPM $${Math.round(c.cpm)} | Frec ${c.frequency.toFixed(1)}${c.mainResult ? ' | ' + c.mainResult.label + ': ' + c.mainResult.value : ''}`
        ).join('\n');

        const prompt = `Analizá estas creatividades de Meta Ads y dá recomendaciones concretas.
...
{
  "winner": "nombre del anuncio ganador y por qué en 1 oración",
  "loser": "anuncio con peor performance y qué cambiarías",
  "pattern": "patrón detectado entre los mejores anuncios",
  "recommendations": ["acción concreta 1", "acción concreta 2", "acción concreta 3"],
  "budget_suggestion": "sugerencia de redistribución de presupuesto entre creatividades"
}`;
        ...
        console.warn('[creatives AI]', e.message);
      }
    }

    res.json({ creatives, aiAnalysis, period: preset, total: creatives.length });
  } catch(e) {
