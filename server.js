...
app.post('/api/:slug/campaign-plans/generate-next-month', async (req, res) => {
  ...
  try {
    ...
    let liveText = '';
    try {
      const [metaCreds, googleCustomerId] = await Promise.all([getMetaCredentials(slug), getGoogleAdsAccountId(slug)]);
      const liveNames = new Set();
      if (metaCreds?.access_token && metaCreds?.ad_account_id) {
        const raw = await fetchMetaCampaignMetricsForRange(metaCreds.ad_account_id, metaCreds.access_token, since30, until1);
        raw.forEach(r => r.campaign_name && liveNames.add(`Meta: ${r.campaign_name}`));
      }
      if (googleCustomerId) {
        const raw = await fetchCampaignMetricsRange(googleCustomerId, since30, until1);
        raw.forEach(r => r.campaign?.name && liveNames.add(`Google: ${r.campaign.name}`));
      }
