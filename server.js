...
async function computeReportFrioData(client, clientId, slug, periodType) {
  ...
  let analytics = null;
  if (propertyId) {
    const [channels, siteMetrics, topPages, devices, demographics, funnel, topItems, keyEvents, keyEventsPrev, browsers, operatingSystems, topSite, channelsByUsers, dailySeries, annualUsers, annualChannels, cities] = await Promise.all([
    ...
    analytics = {
      ...
      browsers, operating_systems: operatingSystems, top_site: topSite, channels_by_users: channelsByUsers, daily_series: dailySeries,
...
async function generateClientReportFrio(slug, periodType, options = {}) {
  const { anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url } = options;
  const client = await loadClient(slug);
  const clientDB = await loadClientDB(slug);
  if (!client || !clientDB) throw new Error('Cliente no encontrado: ' + slug);
...
app.post('/api/:slug/report-frio/generate', async (req, res) => {
  const slug = req.params.slug;
  const { password, period_type, anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  if (!['mes_anterior', 'mes_actual', 'ultimos_90_dias'].includes(period_type)) return res.status(400).json({ error: 'Período inválido' });
  try {
    const report = await generateClientReportFrio(slug, period_type, { anchors, intro_title, intro_note, closing_title, closing_note, report_title, footer_text, hero_image_url });
