...
async function computeReportFrioData(client, clientId, slug, periodType) {
  ...
  let analytics = null;
  if (propertyId) {
    const [channels, siteMetrics, topPages, devices, demographics, funnel, topItems, keyEvents, browsers, operatingSystems, topSite, channelsByUsers, dailySeries, annualUsers, annualChannels] = await Promise.all([
