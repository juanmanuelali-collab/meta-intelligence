...
async function computeGrowthScoreAndFacts(client, clientId) {
  const targets = client.kpi_targets || {};
...
async function runBriefingAgent(client, growthInfo) {
  ...
  if (!facts.length) {
    return { headline: `Todo en orden: no encontramos problemas críticos ni cambios bruscos en las cuentas de ${client.name}.`, stories: [], timeline: [] };
  }
  ...
  const prompt = `Sos un agente experto en leer reportes de campañas de marketing digital (Meta Ads, Google Ads, Analytics, eCommerce) para la agencia Docta Nexus. Redactá, en español rioplatense (voseo), un briefing diario corto para el cliente "${client.name}".

Growth Score de hoy: ${growth_score}/100 (ya calculado, no lo recalcules).

Hechos YA DETECTADOS Y CALCULADOS (no inventes números ni agregues datos que no estén acá):
${factsText}
${seriesText ? `\nSerie histórica de referencia (para narrar la evolución, sin inventar valores):\n${seriesText}\n` : ''}
Con esto, generá:
1. "headline": 1 sola oración, directa, sin relleno, resumiendo lo más importante del día.
2. "stories": un objeto por cada hecho de la lista de arriba (mismo orden), con "titulo" (corto), "motivo" (por qué está pasando, 1-2 oraciones), "accion" (la acción concreta recomendada), "prioridad" ("alta" si el hecho es CRITICO, "media" si es OPORTUNIDAD, "baja" si es ESCALAR), "confianza" (copiá EXACTO el valor de Confianza del hecho, no lo cambies), "impacto_texto" (frase corta; si el hecho no trae impacto estimado, describilo cualitativamente sin inventar un número).
3. "timeline": hasta 4 entradas narrando en 1 oración cada una cómo veníamos los últimos días (usá la serie histórica si está disponible), con "dia" (fecha corta ej "28/07") y "texto".

Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","stories":[{"titulo":"...","motivo":"...","accion":"...","prioridad":"alta|media|baja","confianza":"alta|media|baja","impacto_texto":"..."}],"timeline":[{"dia":"...","texto":"..."}]}`;
  ...
  logAiCall({ slug: client.slug, model: 'gpt-4o-mini', callType: 'briefing_openai', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0 }).catch(() => {});
  const parsed = JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
  return { headline: parsed.headline || '', stories: parsed.stories || [], timeline: parsed.timeline || [] };
}
...
async function generateDailyBriefing(slug) {
  ...
  const growthInfo = await computeGrowthScoreAndFacts(client, clientDB.id);
