...
async function computeReportFrioData(client, clientId, slug, periodType) {
  ...
  let analytics = null;
  if (propertyId) {
    const [channels, siteMetrics, topPages, devices, demographics, funnel, topItems, keyEvents, browsers, operatingSystems, topSite, channelsByUsers, dailySeries] = await Promise.all([
  ...
  if (scProperty) {
    searchConsole = await fetchSearchConsoleSummary(scProperty, range.start, range.end).catch(e => { console.warn('[frio] Search Console omitido:', e.message); return null; });
  }
