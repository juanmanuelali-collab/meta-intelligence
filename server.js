...
const { loadClientDB, verifyPassword, checkPassword, getMetaCredentials, updateMetaToken, getGoogleAdsAccountId, supabase } = require('./db'); // Sub-pasos 3.1/3.2/3.3 — auth + credenciales vía Supabase
...
// siguen viniendo del JSON por ahora — eso se resuelve en pasos futuros),
// pero access_token / ad_account_id se pisan con lo que hay en Supabase,
// que es la fuente de verdad desde la migración.
...
{"resumen": "1-2 oraciones sobre el estado general y lo más importante del día", "acciones": "acciones concretas recomendadas o aplicadas", "alertas": "problemas críticos detectados o vacío si no hay"}`;
  ...
  try {
    ...
    const d = await r.json();
    logAiCall({ slug, model: 'claude-haiku-4-5-20251001', callType: 'memory_summary', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
...
async function authMiddleware(req, res, next) {
  ...
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'Contraseña incorrecta' });
  req.client = client;
  req.clientSlug = slug;
  next();
}
...
app.post('/api/:slug/login', async (req, res) => {
  ...
  // Mientras dure la transición, seguimos devolviendo también los
  // datos de config que hoy vive en el JSON, para no romper el frontend.
  const clientFile = loadClient(slug);
...
async function aggregateAdMetricsForWeek(clientId, isoYear, isoWeek) {
  const { data, error } = await supabase
    .from('ad_metrics_daily')
    .select('source, spend, impressions, clicks, conversions, conversion_value')
    .eq('client_id', clientId)
    .eq('iso_year', isoYear)
    .eq('iso_week', isoWeek);
  ...
  for (const row of data) {
    if (!bySource[row.source]) bySource[row.source] = { spend: 0, impressions: 0, clicks: 0, conversions: 0, conversion_value: 0 };
...
{"resumen_ejecutivo":"2-3 oraciones integrando ambos ángulos","prioridad":"alta|media|baja","accion_inmediata":"la 1 acción más importante a tomar ahora","impacto_estimado":"estimación cualitativa del impacto si se actúa (ej: alto/medio/bajo, con breve razón)"}`;
  ...
  const d = await r.json();
  logAiCall({ slug, model: 'claude-sonnet-4-20250514', callType: 'expert_synthesis_' + scope, inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
  return JSON.parse((d.content?.[0]?.text || '{}').replace(/```json|```/g, '').trim());
}
...
function summarizeEcommerceForPrompt(row) {
  ...
  const inversionTotal = Number(row.inversion_meta_manual ?? row.inversion_meta_auto ?? 0) + Number(row.inversion_google_manual ?? row.inversion_google_auto ?? 0);
  ...
  return `eCommerce ${row.month}/${row.year}: facturación $${row.facturacion ?? 'N/D'}, pedidos ${row.pedidos ?? 'N/D'}, ticket promedio $${ticketPromedio ?? 'N/D'}, margen bruto ${row.margen_bruto_pct ?? 'N/D'}%, margen neto ${row.margen_neto_pct ?? 'N/D'}%. Inversión total (Meta+Google) $${inversionTotal}, CAC real $${cacReal ?? 'N/D'}. Clientes nuevos ${row.clientes_nuevos ?? 'N/D'}, recurrentes ${row.clientes_recurrentes ?? 'N/D'}. Stock total ${row.stock_total ?? 'N/D'}, sin stock ${row.productos_sin_stock ?? 'N/D'}. NPS ${row.nps ?? 'N/D'}. Objetivo mensual $${row.objetivo_mensual ?? 'N/D'}, ROAS esperado ${row.roas_esperado ?? 'N/D'}, CAC máximo $${row.cac_maximo ?? 'N/D'}.`;
}
...
async function generateAndromedaVerdict(creatives, claudeAnalyses, gptAnalyses, slug = 'creatives') {
  ...
  const d = await r.json();
  logAiCall({ slug, model: 'claude-sonnet-4-20250514', callType: 'andromeda_verdict', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
  return JSON.parse((d.content?.[0]?.text || '{}').replace(/```json|```/g, '').trim());
}
