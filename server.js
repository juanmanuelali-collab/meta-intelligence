...
const { fetchGA4WeeklyMetrics, fetchGA4AcquisitionForDate, fetchGA4SiteMetricsForDate, fetchGA4AcquisitionForRange, fetchGA4SiteMetricsForRange, fetchGA4TopPages, fetchGA4TopItems, fetchGA4FunnelCounts, GA4_FUNNEL_EVENTS } = require('./ga4'); // Integración GA4 — tráfico y eventos por semana (post-Fase 5)
...
async function computeReportFacts(client, clientId, slug, profile, periodType) {
  ...
  const [metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily] = await Promise.all([
  ...
  return { range, facts, metaCur, metaPrev, googleCur, googlePrev, ecomCur, ecomPrev, funnel, year12, daily };
}
...
async function runReportAgent(client, profile, factsData) {
  ...
  if (!facts.length) {
    return {
      ...
      stories: [],
      actions: [{ titulo: 'Seguir el plan actual', motivo: 'No hay señales que ameriten un cambio de estrategia esta vez.', impacto: '' }],
    };
  }
...
3. "stories": elegí las 3-5 más relevantes de la lista (no hace falta usarlas todas si hay muchas, ni inventar si hay pocas), en el orden que vos decidas que tiene más sentido para contar la historia del período. Cada una con: "titulo" (corto), "motivo" (por qué pasó, 1-2 oraciones), "analisis" (2-3 oraciones de contexto/profundidad), "accion" (la acción concreta más importante), "consejos" (array de 2 tips accionables adicionales), "prioridad" ("alta" si el hecho es CRITICO, "media" si es OPORTUNIDAD, "baja" si es ESCALAR), "confianza" (copiá EXACTO el valor de Confianza del hecho).
4. "actions": 3-4 acciones de cierre, en orden de impacto, con "titulo", "motivo" (1 oración) e "impacto" (frase corta cualitativa o con el número si viene de un hecho — nunca inventada).

Respondé SOLO en JSON, sin texto antes ni después, con este formato exacto:
{"headline":"...","dek":"...","stories":[{"titulo":"...","motivo":"...","analisis":"...","accion":"...","consejos":["...","..."],"prioridad":"alta|media|baja","confianza":"alta|media|baja"}],"actions":[{"titulo":"...","motivo":"...","impacto":"..."}]}`;

  const r = await fetch('https://api.openai.com/v1/chat/completions', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + OPENAI_KEY },
    body: JSON.stringify({ model: 'gpt-4o', max_tokens: 2800, messages: [{ role: 'user', content: prompt }] })
  });
  const d = await r.json();
  ...
  const parsed = JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
  return { headline: parsed.headline || '', dek: parsed.dek || '', stories: parsed.stories || [], actions: parsed.actions || [] };
}
