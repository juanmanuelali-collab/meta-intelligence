...
app.post('/api/:slug/campaign-plans/item', async (req, res) => {
  const slug = req.params.slug;
  const { password, plan_period_id, campaign_name, channel, objective, budget_diario, dias, budget_mensual, kpi_1, kpi_2, kpi_3, notas, start_date, end_date, status, plan_code, active_days_pattern, kpi_1_target, kpi_2_target, kpi_3_target } = req.body;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  ...
  try {
    ...
    const { data, error } = await supabase.from('campaign_plan_items').insert({
      ...
      objective: objective || null, budget_diario: budget_diario ?? null, dias: computed.dias ?? dias ?? null,
      ...
      notas: notas || null, start_date: start_date || null, end_date: end_date || null, status: status || 'Proposed',
    }).select().single();
    if (error) throw new Error(error.message);
...
app.put('/api/:slug/campaign-plans/item/:itemId', async (req, res) => {
  ...
  if (!(await checkPassword(req.params.slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const allowed = ['campaign_name', 'channel', 'campaign_id', 'match_confidence', 'campaign_role', 'objective', 'budget_diario', 'dias', 'budget_mensual', 'status', 'kpi_1', 'kpi_2', 'kpi_3', 'notas', 'start_date', 'end_date', 'plan_code', 'active_days_pattern', 'kpi_1_target', 'kpi_2_target', 'kpi_3_target'];
  const update = { updated_at: new Date().toISOString() };
  ...
  try {
    ...
    for (const mRow of (masterRows || [])) {
      ...
      toInsert.push({
        ...
        notas: mRow.notas_default, start_date: monthStart, end_date: monthEnd,
      });
    }
  ...
  try {
    ...
    const itemsWithMatch = await Promise.all(items.map(async it => {
      ...
      return {
        ...
        objective: it.objective || null, budget_diario: it.budget_diario || null,
        ...
        notas: it.notas || null, start_date: it.start_date || null, end_date: it.end_date || null, status: it.status || 'Proposed',
      };
    }));
