...
const { fetchCampaignMetrics, normalizeToAdMetricsRows, fetchShoppingProducts, fetchCampaignMetricsRange, aggregateCampaignRows, fetchTopKeywords, fetchConversionActionsSummary, fetchAllCampaignsStructure, fetchAccountCurrency: fetchGoogleAdsAccountCurrency } = require('./google_ads'); // Fase 1 — conector Google Ads
...
const DEFAULT_ENABLED_FEATURES = {
  ...
  ai_box_meta_ads: true, ai_box_google_ads: true, ai_box_ecommerce: true,
  ai_box_analytics: true, ai_box_creatives: true,
};
...
app.get('/api/admin/clients/:slug', async (req, res) => {
  ...
  const featureUsage = {};
  for (const f of ['meta_claude_analysis', 'meta_gpt_analysis', 'google_analysis', 'ecommerce_analysis', 'analytics_analysis', 'creatives_dual']) {
...
app.post('/api/:slug/intelligence/analyze-google', async (req, res) => {
  ...
  const quotaGoogle = await checkAiQuota(slug, 'google_analysis');
  if (quotaGoogle.blocked) return res.status(429).json({ error: quotaGoogle.message, ai_usage: quotaGoogle.usage });
  ...
  try {
    ...
    await incrementAiUsage(clientDB.id, 'google_analysis');
    const ai_usage = await getAiUsage(slug, 'google_analysis');
...
app.get('/api/:slug/creatives', async (req, res) => {
  ...
  const client = await loadClientWithCreds(slug);
  if (!client) return res.status(404).json({ error: 'Cliente no encontrado' });
  const { password, period, withAnalysis, withNexusAi } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  ...
  try {
    ...
    const [adsData, batchRes] = await Promise.all([
      ...
      metaFetch('/' + accountId + '/ads?fields=id,creative{thumbnail_url,image_url,video_id,title,body}&thumbnail_width=600&thumbnail_height=450&limit=50&', token).catch(() => ({ data: [] }))
    ]);
    ...
    // Análisis IA general (Nexus AI) — YA NO es automático: antes corría
    // siempre que hubiera creatividades, ahora requiere que el operador lo
    // pida explícitamente (withNexusAi=true) Y que haya al menos 15
    // creatividades cargadas (con menos, el patrón/ganador no es confiable
    // estadísticamente). Comparte cupo con el Análisis Dual por anuncio.
    let aiAnalysis = null;
    let nexusAiBlocked = null;
    if (withNexusAi === 'true' && ANTHROPIC_KEY && creatives.length >= 15) {
      const quotaNexus = await checkAiQuota(slug, 'creatives_dual');
      if (quotaNexus.blocked) {
        nexusAiBlocked = quotaNexus.message;
      } else {
      try {
        const summary = creatives.slice(0, 8).map((c, i) =>
        ...
        const clientDBForUsage = await loadClientDB(slug);
        if (clientDBForUsage) await incrementAiUsage(clientDBForUsage.id, 'creatives_dual').catch(() => {});
    ...
    let dualBlocked = null;
    if (withAnalysis === 'true' && creatives.length > 0) {
      const quotaDual = await checkAiQuota(slug, 'creatives_dual');
      if (quotaDual.blocked) {
        dualBlocked = quotaDual.message;
      } else {
        ...
        const clientDBForUsage2 = await loadClientDB(slug);
        if (clientDBForUsage2) await incrementAiUsage(clientDBForUsage2.id, 'creatives_dual').catch(() => {});
      }
    }

    const ai_usage = await getAiUsage(slug, 'creatives_dual');
    res.json({
      creatives, aiAnalysis, andromedaVerdict, period: preset, total: creatives.length,
      ai_usage, error_nexus_ai: nexusAiBlocked, error_dual_analysis: dualBlocked,
      min_creatives_for_ai: 15, creativeIntelligence,
    });
  } catch(e) {
...
app.get('/api/:slug/ai-usage-all', async (req, res) => {
  ...
  const { password } = req.query;
  if (!(await checkPassword(slug, password))) return res.status(401).json({ error: 'No autorizado' });
  const features = ['meta_claude_analysis', 'meta_gpt_analysis', 'creatives_dual', 'google_analysis', 'ecommerce_analysis', 'analytics_analysis'];
