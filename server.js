...
app.get('/api/:slug/metrics', async (req, res) => {
  ...
  try {
    ...
    // Normalizar valores numéricos para evitar errores .toFixed() en el cliente
    ...
    res.json({
      account:   normalizeRow(data.data?.[0]  || {}),
      campaigns: (campaigns.data || []).map(normalizeRow),
      adsets:    (adsets.data    || []).map(normalizeRow),
      ads:       (ads.data       || []).map(normalizeRow),
      period,
      currency: client.currency || 'ARS',
    });
  } catch(e) {
