...
async function getSheetsClient() {
  try {
    ...
    console.warn('[sheets] Error auth:', e.message);
    return null;
  }
}
...
async function logAiCall({ slug, model, callType, inputTokens, outputTokens, notes = '' }) {
  ...
  try {
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: 'Sheet1!A:J',
      valueInputOption: 'RAW',
      requestBody: { values: [[fecha, hora, slug || 'sistema', model, callType, inputTokens, outputTokens, costo.toFixed(6), proveedor, notes]] },
    });
  } catch(e) {
    console.warn('[sheets] Error logging:', e.message);
  }
}
...
Respondé ÚNICAMENTE con este JSON (sin markdown):
{
  ...
  logAiCall({ slug: 'dashboard', model: 'claude-opus-4-5', callType: 'dashboard_analysis', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
...
app.post('/api/:slug/save-email', (req, res) => {
  const slug = req.params.slug;
  const client = loadClient(slug);
  if (!client) return res.status(404).json({ error: 'Cliente no encontrado' });
...
async function analyzeCreativeWithClaude(ad) {
...
Respondé SOLO en JSON:
{
  ...
  const d = await r.json();
  logAiCall({ slug: 'creatives', model: 'claude-haiku-4-5-20251001', callType: 'creative_analysis_claude', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0, notes: ad.adName?.slice(0,50) }).catch(()=>{});
  return JSON.parse((d.content?.[0]?.text || '{}').replace(/```json|```/g, '').trim());
}

async function analyzeCreativeWithGPT(ad) {
...
Respondé SOLO en JSON:
{
  ...
  const d = await r.json();
  logAiCall({ slug: 'creatives', model: 'gpt-4o-mini', callType: 'creative_analysis_gpt', inputTokens: d.usage?.prompt_tokens || 0, outputTokens: d.usage?.completion_tokens || 0, notes: ad.adName?.slice(0,50) }).catch(()=>{});
  return JSON.parse((d.choices?.[0]?.message?.content || '{}').replace(/```json|```/g, '').trim());
}

async function generateAndromedaVerdict(creatives, claudeAnalyses, gptAnalyses) {
  ...
  const d = await r.json();
  logAiCall({ slug: 'creatives', model: 'claude-sonnet-4-20250514', callType: 'andromeda_verdict', inputTokens: d.usage?.input_tokens || 0, outputTokens: d.usage?.output_tokens || 0 }).catch(()=>{});
  return JSON.parse((d.content?.[0]?.text || '{}').replace(/```json|```/g, '').trim());
}
  ...
  try {
    ...
    if (withAnalysis === 'true' && creatives.length > 0) {
      ...
      [claudeAnalyses, gptAnalyses] = await Promise.all([
        Promise.all(top.map(c => analyzeCreativeWithClaude(c).catch(() => ({})))),
        OPENAI_KEY ? Promise.all(top.map(c => analyzeCreativeWithGPT(c).catch(() => ({})))) : Promise.resolve(top.map(() => ({}))),
      ]);
      andromedaVerdict = await generateAndromedaVerdict(top, claudeAnalyses, gptAnalyses).catch(() => null);
...
app.get('/api/:slug/followers', async (req, res) => {
  ...
  try {
    ...
    // Guardar snapshot de followers en el data de hoy
    const todayFile = path.join(__dirname, 'data', slug, today() + '.json');
    if (fs.existsSync(todayFile)) {
      try {
        const snap = JSON.parse(fs.readFileSync(todayFile, 'utf8'));
        snap.followers = results;
        fs.writeFileSync(todayFile, JSON.stringify(snap, null, 2));
      } catch(e) {}
    }
