...
async function fetchAccountMetrics(client) {
  ...
  // Cuenta, campañas, adsets y anuncios en paralelo
  const [accountData, campaignData, adsetData, adData] = await Promise.all([
    metaFetch('/' + accountId + '/insights?fields=' + fields + '&date_preset=last_7d&level=account&', token),
    metaFetch('/' + accountId + '/insights?fields=' + fields + ',campaign_id,campaign_name&date_preset=last_7d&level=campaign&limit=50&', token),
    metaFetch('/' + accountId + '/insights?fields=' + fields + ',adset_id,adset_name,campaign_name,targeting&date_preset=last_7d&level=adset&sort=spend_descending&limit=30&', token),
    metaFetch('/' + accountId + '/insights?fields=ad_id,ad_name,adset_id,adset_name,campaign_name,' + fields + '&date_preset=last_7d&level=ad&sort=spend_descending&limit=25&', token),
  ]);
...
Gasto: $${Math.round(metricsData.account.spend||0)} | Impresiones: ${metricsData.account.impressions||0} | Alcance: ${metricsData.account.reach||0}
CTR: ${(metricsData.account.ctr||0).toFixed(2)}% | CPM: $${Math.round(metricsData.account.cpm||0)} | Frecuencia: ${(metricsData.account.frequency||0).toFixed(1)}
ROAS: ${metricsData.account.roas ? metricsData.account.roas.toFixed(2)+'x' : 'N/A'}
Mensajería: ${metricsData.account.messagingConn||0} conexiones | ${metricsData.account.messagingFirstReply||0} primeras respuestas | Costo x msg: $${Math.round(metricsData.account.cpmsg||0)}
Leads: ${metricsData.account.leads||0} | Compras: ${metricsData.account.purchases||0} | Valor: $${Math.round(metricsData.account.purchaseValue||0)}
Nuevos seguidores: ${metricsData.account.newFollowersPage||0} (pág) + ${metricsData.account.newFollowersIG||0} (IG)
Compartidos: ${metricsData.account.postShares||0} | Guardados: ${metricsData.account.postSaves||0}
Engagement posts: ${metricsData.account.postEngagement||0} | Video views: ${metricsData.account.videoViews||0}

CAMPAÑAS (${metricsData.campaigns.length}):
${JSON.stringify(metricsData.campaigns.slice(0,12).map(c=>({
...
CONJUNTOS DE ANUNCIOS / AUDIENCIAS (${metricsData.adsets.length}):
${JSON.stringify(metricsData.adsets.slice(0,15).map(a=>({
...
ANUNCIOS TOP (${metricsData.ads.length}):
${JSON.stringify(metricsData.ads.slice(0,12).map(a=>({
