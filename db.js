...
// ── JWT_SECRET — obligatorio para que el server arranque ──
// Antes de esto, "estar logueado" era el navegador reenviando el password
// real en cada request (guardado en texto plano en localStorage). Ahora el
// login sigue verificando el password real UNA sola vez (bcrypt, como
// siempre) y a partir de ahí se emite un token firmado que es lo único que
// circula. Si falta la env var, preferimos que el server no arranque a que
// arranque sin poder emitir/validar tokens.
const JWT_SECRET = process.env.JWT_SECRET;
if (!JWT_SECRET) {
  throw new Error('Falta la env var JWT_SECRET (Render → Environment). Generar un valor random largo antes de desplegar.');
}
const CLIENT_TOKEN_TTL = '30d'; // mismo horizonte que la sesión vieja en localStorage
const ADMIN_TOKEN_TTL = '12h';  // el admin nunca persistía sesión entre recargas — se mantiene ese comportamiento

function generateClientToken(slug) {
  return jwt.sign({ slug, type: 'client' }, JWT_SECRET, { expiresIn: CLIENT_TOKEN_TTL });
}
function verifyClientToken(token, expectedSlug) {
  ...
  try {
    ...
    return payload;
  } catch (e) { return null; } // expirado, firma inválida, malformado, etc.
}
function generateAdminToken() {
  return jwt.sign({ type: 'admin' }, JWT_SECRET, { expiresIn: ADMIN_TOKEN_TTL });
}
...
// ── Mismo traer-cliente-por-slug, pero SIN filtrar por activo — uso
// exclusivo del panel de administración (/api/admin/*). loadClientDB()
// sigue filtrando por active=true para todo lo que es login/dashboard/
// crons, que sí deben bloquear a un cliente desactivado. Antes de esto,
// el admin usaba loadClientDB() también para editar clientes, así que un
// cliente desactivado dejaba de ser "visible" para el propio admin y
// nunca se podía volver a reactivar. ──
async function loadClientDBAny(slug) {
  const { data, error } = await supabase
    .from('clients')
    .select('*')
    .eq('slug', slug)
...
// ── Atajo usado por TODOS los endpoints ya autenticados del dashboard ──
// (los ~104 que reciben "password" por query string o body, uno por uno).
// IMPORTANTE — este atajo YA NO compara el password real con bcrypt en cada
// llamada. Desde el cambio a tokens de sesión, el segundo parámetro es el
// JWT que devolvió /api/:slug/login, no el password en texto plano. El
// nombre de la función y de la variable "password" en cada endpoint se
// dejaron sin tocar a propósito (son ~104 call sites) — lo único que cambió
// es qué significa ese valor. La verificación bcrypt contra el password
// real sigue existiendo, pero ahora vive SOLO en /api/:slug/login
// (verifyPassword de arriba), que es la única vez que el password real
// debería viajar por la red.
async function checkPassword(slug, sessionToken) {
  const payload = verifyClientToken(sessionToken, slug);
  return !!payload;
}
...
async function updateClientRecord(slug, fields) {
  const clientDB = await loadClientDBAny(slug);
  if (!clientDB) return false;
  const update = {};
  ...
  if (fields.enabledFeatures !== undefined) update.enabled_features = fields.enabledFeatures;
  if (fields.aiMonthlyLimit !== undefined) update.ai_monthly_limit = fields.aiMonthlyLimit;
  if (fields.aiFeatureLimits !== undefined) update.ai_feature_limits = fields.aiFeatureLimits;
  if (fields.reportFrioConfig !== undefined) update.report_frio_config = fields.reportFrioConfig;
...
// ── Límite de consultas mensuales al agente IA ────────────────────
// Un solo contador compartido entre Claude y GPT (misma "consulta" para
// el cliente, elija el botón que elija) — si más adelante se quiere
// separar en dos cupos independientes, es cuestión de duplicar la
// columna `period` con un sufijo por modelo.
function currentAiPeriod() {
  const d = new Date();
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}`;
}

// Cada función de IA del dashboard tiene su propio cupo independiente,
// identificado por `feature`. 'meta_analysis' es la original (botones
// Claude/GPT de Meta Ads) y sigue usando la columna clients.ai_monthly_limit
// para no romper lo ya configurado. Las funciones nuevas (Google Ads,
// eCommerce, Analytics, Creatividades) leen su límite de
// clients.ai_feature_limits (jsonb), una clave por feature — null/ausente
// = sin límite, nunca bloquea.
async function getAiFeatureLimit(clientDB, feature) {
  const limits = clientDB.ai_feature_limits || {};
  // Meta Ads se separó en dos cupos independientes (Claude/GPT). Si el
  // operador todavía no configuró ninguno de los dos por separado, se cae
  // al viejo ai_monthly_limit compartido — así un cliente ya configurado
  // antes de este cambio (ej. "3" compartido) sigue funcionando igual
  // hasta que el operador decida separarlos explícitamente en /admin.
  if (feature === 'meta_claude_analysis' || feature === 'meta_gpt_analysis') {
    if (limits[feature] !== undefined && limits[feature] !== null) return limits[feature];
    return (clientDB.ai_monthly_limit === null || clientDB.ai_monthly_limit === undefined) ? null : clientDB.ai_monthly_limit;
  }
  if (feature === 'meta_analysis') { // compatibilidad hacia atrás, ya no se usa en server.js
    return (clientDB.ai_monthly_limit === null || clientDB.ai_monthly_limit === undefined) ? null : clientDB.ai_monthly_limit;
  }
  return (limits[feature] === null || limits[feature] === undefined) ? null : limits[feature];
}

// Devuelve { used, limit, remaining, period } para una función de IA puntual.
async function getAiUsage(slug, feature = 'meta_analysis') {
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return null;
  const period = currentAiPeriod();
  const limit = await getAiFeatureLimit(clientDB, feature);
  const { data } = await supabase
    .from('client_ai_usage')
    .select('used_count')
    .eq('client_id', clientDB.id)
    .eq('feature', feature)
    .eq('period', period)
...
// Suma 1 al contador del mes en curso para esa feature puntual (crea la fila si no existe).
async function incrementAiUsage(clientId, feature = 'meta_analysis') {
  const period = currentAiPeriod();
  const { data: existing } = await supabase
    ...
    .maybeSingle();
  if (existing) {
    await supabase.from('client_ai_usage').update({ used_count: existing.used_count + 1, updated_at: new Date().toISOString() }).eq('id', existing.id);
  } else {
    await supabase.from('client_ai_usage').insert({ client_id: clientId, feature, period, used_count: 1 });
  }
}

module.exports = {
  supabase, loadClientDB, loadClientDBAny, verifyPassword, checkPassword,
  generateClientToken, verifyClientToken, generateAdminToken, verifyAdminToken,
