...
// Devuelve { used, limit, remaining, period }. limit=null significa
// sin límite (ilimitado) — nunca bloquea.
async function getAiUsage(slug) {
  const clientDB = await loadClientDB(slug);
  if (!clientDB) return null;
  const period = currentAiPeriod();
  const limit = (clientDB.ai_monthly_limit === null || clientDB.ai_monthly_limit === undefined) ? null : clientDB.ai_monthly_limit;
  const { data } = await supabase
...
// Suma 1 al contador del mes en curso (crea la fila si no existe todavía).
async function incrementAiUsage(clientId) {
  const period = currentAiPeriod();
  ...
  if (existing) {
    ...
    await supabase.from('client_ai_usage').insert({ client_id: clientId, period, used_count: 1 });
  }
}
