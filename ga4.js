...
// ── Fuente/Medio — igual que fetchGA4ChannelsByUsers pero con la dimensión
// cruda sessionSourceMedium en vez del canal agrupado por Google, para el
// gráfico nuevo de "Fuente/Medio" del Informe Frío. ──
async function fetchGA4SourceMedium(propertyId, since, until) {
  const auth = getGA4Auth();
  const analyticsData = google.analyticsdata({ version: 'v1beta', auth });
  const res = await analyticsData.properties.runReport({
    property: `properties/${propertyId}`,
  ...
  return (res.data.rows || []).map(r => ({
    source_medium: r.dimensionValues?.[0]?.value || '(sin datos)',
    users: Number(r.metricValues?.[0]?.value || 0),
  }));
}

// ── Serie diaria de Vistas y Usuarios — para el box "Evolución" del
// Informe Frío. A diferencia del resto de las funciones de este archivo,
// acá SÍ importa el desglose día por día (no solo el total del período).
async function fetchGA4DailySeries(propertyId, since, until) {
  const auth = getGA4Auth();
  const analyticsData = google.analyticsdata({ version: 'v1beta', auth });
  const res = await analyticsData.properties.runReport({
    property: `properties/${propertyId}`,
    requestBody: {
      ...
      metrics: [{ name: 'screenPageViews' }, { name: 'totalUsers' }, { name: 'newUsers' }, { name: 'averageSessionDuration' }, { name: 'bounceRate' }, { name: 'sessions' }],
      orderBys: [{ dimension: { dimensionName: 'date' } }],
    },
  });
  return (res.data.rows || []).map(r => {
    const m = i => Number(r.metricValues?.[i]?.value || 0);
    return {
      date: r.dimensionValues?.[0]?.value || '', // formato YYYYMMDD
      views: m(0), users: m(1), new_users: m(2), avg_engagement_seconds: m(3), bounce_rate: +(m(4) * 100).toFixed(2), sessions: m(5),
    };
  });
}

// ── Serie ANUAL (últimos 12 meses, fija — no depende del período elegido
// en el informe) — Usuarios mes a mes + Canales agregados del año. Se
// combinan en una sola consulta a GA4 cada una para no duplicar llamadas.
async function fetchGA4AnnualUsersByMonth(propertyId) {
  const auth = getGA4Auth();
  const analyticsData = google.analyticsdata({ version: 'v1beta', auth });
  const res = await analyticsData.properties.runReport({
    property: `properties/${propertyId}`,
  ...
  return (res.data.rows || []).map(r => ({
    month: r.dimensionValues?.[0]?.value || '', // formato YYYYMM
    users: Number(r.metricValues?.[0]?.value || 0),
  }));
}
async function fetchGA4AnnualChannels(propertyId) {
  const auth = getGA4Auth();
  const analyticsData = google.analyticsdata({ version: 'v1beta', auth });
  const res = await analyticsData.properties.runReport({
    property: `properties/${propertyId}`,
...
// ── Usuarios activos por ciudad — box nuevo del Informe Frío.
async function fetchGA4CityBreakdown(propertyId, since, until) {
  const auth = getGA4Auth();
  const analyticsData = google.analyticsdata({ version: 'v1beta', auth });
  const res = await analyticsData.properties.runReport({
    property: `properties/${propertyId}`,
  ...
  return (res.data.rows || []).map(r => ({
    city: r.dimensionValues?.[0]?.value || '(sin datos)',
    users: Number(r.metricValues?.[0]?.value || 0),
...
module.exports = {
  ...
  fetchGA4TopSiteSummary, fetchGA4ChannelsByUsers, fetchGA4SourceMedium, fetchGA4DailySeries,
  fetchGA4AnnualUsersByMonth, fetchGA4AnnualChannels, fetchGA4CityBreakdown,
};
