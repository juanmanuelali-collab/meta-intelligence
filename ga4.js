...
async function fetchGA4DeviceBreakdown(propertyId, since, until) {
  ...
  return (res.data.rows || []).map(r => ({
    device: r.dimensionValues?.[0]?.value || '(sin datos)',
...
async function fetchGA4Demographics(propertyId, since, until) {
  ...
  const [ageRes, genderRes] = await Promise.all([
    analyticsData.properties.runReport({
      property: `properties/${propertyId}`,
      requestBody: { dateRanges: [{ startDate: since, endDate: until }], dimensions: [{ name: 'userAgeBracket' }], metrics: [{ name: 'totalUsers' }] },
    }),
    analyticsData.properties.runReport({
      property: `properties/${propertyId}`,
      requestBody: { dateRanges: [{ startDate: since, endDate: until }], dimensions: [{ name: 'userGender' }], metrics: [{ name: 'totalUsers' }] },
    }),
  ]);
  const age = (ageRes.data.rows || []).map(r => ({ bracket: r.dimensionValues?.[0]?.value, users: Number(r.metricValues?.[0]?.value || 0) })).filter(r => r.bracket && r.bracket !== 'unknown');
  const gender = (genderRes.data.rows || []).map(r => ({ gender: r.dimensionValues?.[0]?.value, users: Number(r.metricValues?.[0]?.value || 0) })).filter(r => r.gender && r.gender !== 'unknown');
